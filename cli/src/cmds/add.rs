...
use clap::Args;
use core::{RegistryClient, CacheManager, LockfileManager};
use std::env;
...
pub async fn run(args: &AddArgs) -> Result<()> {
    ...
    let resolved_pack = registry_client.resolve(&pack_id, version.as_deref()).await
        .with_context(|| format!("Failed to resolve rpack '{}'", pack_id))?;
    
    println!("Found rpack '{}' version {}", resolved_pack.id, resolved_pack.version);
    
    // Download and cache the pack
    println!("Downloading rpack...");
    let cached_pack = cache_manager.ensure(&resolved_pack).await
    ...
    println!("✅ Successfully added rpack '{}' version {}", 
    ...
    if let Some(manifest) = &cached_pack.manifest {
        if let Some(templates) = &manifest.templates {
            if let Some(entrypoints) = &templates.entrypoints {
                if !entrypoints.is_empty() {
                    println!("\nAvailable templates:");
                    for template in entrypoints {
                        println!("  - {}:{}", pack_id, template);
                    }
                }
            }
        }
...
fn parse_rpack_spec(spec: &str) -> Result<(String, Option<String>)> {
    if let Some(at_pos) = spec.rfind('@') {
        ...
        if id.is_empty() || version.is_empty() {
            bail!("Invalid rpack spec: '{}'. Expected format: <rpack-id>[@version]", spec);
        }
