...
pub async fn run(args: &FrontmatterArgs) -> Result<()> {
    println!("📋 Generating frontmatter...");
    println!("Description: {}", args.description);

    // Generate frontmatter following core team best practices
    let frontmatter_content = format!(
        r#"title: "{}"
description: "Generated frontmatter for {}"
date: {}
author: "Generated by ggen"
tags:
  - generated
  - frontmatter
keywords: "frontmatter, {}, generated"
---
Generated content for: {}"#,
        args.description,
        args.description,
        chrono::Utc::now().format("%Y-%m-%d"),
        args.description.to_lowercase(),
        args.description
    );
    ...
    if let Some(output_path) = &args.output {
        std::fs::write(output_path, &frontmatter_content).map_err(|e| {
            ggen_utils::error::Error::new(&format!("Failed to write frontmatter to disk: {}", e))
        })?;
        println!("📁 Saved to: {}", output_path);
    } else {
        println!("📄 Generated frontmatter:");
        println!("{}", frontmatter_content);
    }
