# Implementation Plan: FMEA & Poka-Yoke for mcpp
...
Implement the behavior for **already-defined mcpp.toml config sections**:
...
### Config Schema (mcpp-config/src/schema.rs)
...
### CODEOWNERS Generator (mcpp-core/src/types/codeowners.rs)
...
**Location**: `crates/mcpp-domain/src/template.rs`
...
**Location**: `crates/mcpp-domain/src/template.rs` (or new `generation/headers.rs`)
...
fn inject_header(content: &str, ext: &str, config: &GenerationSafetyConfig) -> String {
    ...
    let header = config.generated_header.as_deref()
        .unwrap_or("// DO NOT EDIT - Generated by mcpp");
...
**Location**: `crates/mcpp-cli/src/cmds/marketplace.rs`

**Changes**:
1. During `mcpp marketplace install`, load package.toml
...
**Location**: `crates/mcpp-domain/src/template.rs` or generate command

**Changes**:
1. After template generation, if `codeowners.enabled`:
2. Use existing `CodeownersGenerator` from mcpp-core
...
|------|-------------|---------|
| `crates/mcpp-domain/src/template.rs` | MODIFY | Add path protection + header injection |
| `crates/mcpp-domain/src/generation/mod.rs` | CREATE | Module for generation utilities |
| `crates/mcpp-domain/src/generation/headers.rs` | CREATE | Header injection logic |
| `crates/mcpp-domain/src/generation/protection.rs` | CREATE | Path protection logic |
| `crates/mcpp-cli/src/cmds/marketplace.rs` | MODIFY | Add FMEA validation to install |
...
- Existing `mcpp-core/src/types/codeowners.rs` (already implemented)
- Existing `mcpp-config/src/schema.rs` (config structs ready)
