...
**Prerequisites**: mcpp installed, existing marketplace package

## Overview

This guide shows how to add enterprise-grade FMEA (Failure Mode and Effects Analysis) and Poka-Yoke (error-proofing) controls to your mcpp marketplace package.
...
**Key principle**: Your business logic goes in `src/domain/`. Generated scaffolding goes in `src/generated/`. Running `mcpp generate` will ONLY touch `src/generated/`.
...
//
// Regenerate with:
//   mcpp generate --template my-template --domain my-cli.rdf
//
...
[[fmea.controls]]
id = "F2"
mode = "mcpp generate overwrites domain implementations"
...
mcpp marketplace validate --strict

# Generate FMEA report
mcpp marketplace validate --fmea-report fmea-report.json --format json
```
...
impl UserTrait for UserDomain {
    fn create(&self, name: String, email: String) -> Result<User, DomainError> {
        ...
        // This code will NEVER be overwritten by mcpp generate
...
2. **Set up CI validation**: Add `mcpp marketplace validate --strict` to your CI pipeline
