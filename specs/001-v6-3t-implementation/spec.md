# Feature Specification: mcpp v6 - 3T (TOML, Tera, Turtle) Only Implementation
...
A developer creates a new software project using mcpp v6 where the entire system specification exists only as TOML configuration, Turtle ontologies, and Tera templates - with zero hand-written code in the generated output.
...
**Independent Test**: Can be fully tested by initializing a new v6 project with `mcpp init --v6 project-name`, writing a minimal ontology (defining one type), and verifying that `mcpp sync` generates complete, compilable code without any hand-written source files in the output directory.

**Acceptance Scenarios**:

1. **Given** a developer has mcpp v6 installed, **When** they run `mcpp init --v6 my-project`, **Then** a project structure is created containing only `mcpp.toml`, `ontology/` directory with schema files, and `templates/` directory with rendering templates
2. **Given** a new v6 project, **When** the developer defines a simple type in `ontology/domain.ttl`, **Then** running `mcpp sync` generates complete source code in `generated/` without requiring any hand-written code
3. **Given** a v6 project with ontology definitions, **When** the developer regenerates with `mcpp sync`, **Then** the output is bit-for-bit identical to the previous generation (deterministic)
...
**Independent Test**: Can be tested by running `mcpp sync` multiple times and verifying bit-for-bit identical output (idempotence), attempting to modify files in `generated/` and having the system reject them or warn on next sync (no-edit law), and verifying that hash chains in receipts prove output provenance from ontology.

**Acceptance Scenarios**:

1. **Given** a v6 project, **When** `mcpp sync` is run twice without ontology changes, **Then** the second run produces no file changes (idempotence: μ∘μ = μ)
2. **Given** artifacts in `generated/`, **When** a developer manually edits a generated file, **Then** the next `mcpp sync` either overwrites the change or fails with a warning about violated no-edit law
3. **Given** a project on different platforms (Linux, macOS, Windows), **When** `mcpp sync` runs on each, **Then** artifacts are bit-for-bit identical (determinism)
...
5. **Given** an ontology change, **When** `mcpp sync` runs, **Then** all affected artifacts regenerate and the receipt updates to reflect new provenance chain

---

### User Story 4 - Vocabulary Governance and Validation (Priority: P2)

Developers define allowed RDF namespaces in `mcpp.toml` and the system enforces vocabulary constraints via SHACL shapes, preventing namespace pollution and ensuring architectural coherence at compile-time.
...
**Independent Test**: Can be tested by configuring allowed vocabularies in `mcpp.toml`, adding triples from unauthorized namespaces to the ontology, and verifying that `mcpp sync` fails validation during μ₁ (Normalization Pass) with clear error messages identifying the violation.

**Acceptance Scenarios**:

1. **Given** a `mcpp.toml` with `allowed_vocabularies = ["http://schema.org/", "http://mcpp.dev/"]`, **When** an ontology uses triples from `http://example.org/`, **Then** `mcpp sync` fails during μ₁ with a vocabulary violation error
...
The system enforces guards (forbidden output patterns) to prevent accidental secret leaks, enforce path constraints, and maintain security policies defined in `mcpp.toml`.
...
**Independent Test**: Can be tested by defining a secret guard pattern in `mcpp.toml`, creating an ontology that would generate code containing that pattern, and verifying that `mcpp sync` fails during μ₅ (Receipt Pass) with a guard violation error.

**Acceptance Scenarios**:

1. **Given** a guard configured as `secret = { pattern = "(?i)(password|secret|api.?key)", action = "reject" }`, **When** an ontology would generate code containing "API_KEY = 'xyz'", **Then** `mcpp sync` fails with a secret guard violation before writing files
...
The v6 system includes its own type definitions, compilation rules, and projection logic in its ontology, enabling it to regenerate itself by running `mcpp sync` on the mcpp-core ontology.
...
**Independent Test**: Can be tested by loading the mcpp-core ontology (which defines the v6 compiler itself), running `mcpp sync` on it, and verifying that the generated Rust code compiles into a working mcpp binary that is functionally equivalent to the original.

**Acceptance Scenarios**:

1. **Given** the mcpp-core ontology defining the v6 compiler, **When** `mcpp sync` runs on it, **Then** complete Rust source code for mcpp is generated in `generated/`
2. **Given** generated mcpp source, **When** it's compiled with `cargo build`, **Then** it produces a binary that can successfully compile other v6 projects
3. **Given** a self-generated mcpp binary, **When** it runs `mcpp sync` on its own ontology, **Then** it produces identical output to the original (self-rendering closure)
...
- **What happens when SPARQL queries in `mcpp.toml` return no results?** μ₂ produces empty bindings, μ₃ renders templates with empty data (templates must handle empty cases gracefully), and the system completes successfully with a warning
...
- **What happens when the receipt file is manually deleted?** Next `mcpp sync` regenerates it from scratch based on current artifact hashes
- **What happens when a user modifies `generated/` files and runs `mcpp sync`?** System either overwrites changes (default) or fails with a diff showing violations of the no-edit law (configurable via `mcpp.toml`)
...
- **FR-001**: System MUST parse TOML configuration files conforming to the mcpp v6 schema, validating all required sections (`[project]`, `[v6]`, `[v6.passes]`, `[v6.invariants]`)
...
- **FR-012**: System MUST be idempotent: running `mcpp sync` twice without ontology changes produces no file modifications
...
- **FR-017**: System MUST execute the five passes in strict order (μ₁ → μ₂ → μ₃ → μ₄ → μ₅) with no parallelization within a single `mcpp sync` invocation
- **FR-018**: System MUST support `mcpp init --v6 <project-name>` to scaffold a new v6 project with minimal ontology, templates, and configuration
...
- **FR-020**: System MUST provide a `mcpp verify` command that validates receipt integrity by recomputing hashes and comparing against receipt
...
- **Projection Function (μ)**: The deterministic transformation pipeline consisting of five passes (μ₁-μ₅). Takes ontology as input, produces artifacts as output. Implemented by the mcpp v6 compiler.
...
- **Configuration (mcpp.toml)**: The TOML file defining pipeline behavior, allowed vocabularies, guards, generation rules (SPARQL queries + template mappings), and constitutional invariant settings.
...
- **SC-003**: Running `mcpp sync` twice without ontology changes produces zero file modifications (100% idempotence validation)
...
- **SC-007**: The mcpp-core ontology (defining the v6 compiler itself) successfully generates a compilable Rust codebase in under 10 seconds
- **SC-008**: Self-generated mcpp binaries produce identical output to the original compiler when run on the same ontology (meta-circular closure)
...
- **SC-010**: Developers can verify artifact provenance using `mcpp verify` in under 1 second for projects with 100+ generated files
...
- **D-001**: Rust 1.75+ toolchain for building the mcpp v6 compiler itself
...
- **D-008**: TOML parser for loading `mcpp.toml` configuration
...
- **NG-006**: Incremental compilation or caching of intermediate results - each `mcpp sync` runs the full μ₁-μ₅ pipeline
