# Feature Specification: ggen v26.5.19 - 3T (TOML, Tera, Turtle) Only Implementation

**Feature Branch**: `001-v26.5.19-3t-implementation`
**Created**: 2025-12-19
**Status**: Draft
**Input**: User description: "create v26.5.19 using 3T (TOML, Tera, Turtle) only"

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Project Initialization with Pure 3T (Priority: P1)

A developer creates a new software project using ggen v26.5.19 where the entire system specification exists only as TOML configuration, Turtle ontologies, and Tera templates - with zero hand-written code in the generated output.

**Why this priority**: This is the foundational capability that enables all other v26.5.19 features. Without the ability to initialize and configure a pure 3T project, the entire v26.5.19 paradigm cannot function.

**Independent Test**: Can be fully tested by initializing a new v26.5.19 project with `ggen init --v26.5.19 project-name`, writing a minimal ontology (defining one type), and verifying that `ggen sync` generates complete, compilable code without any hand-written source files in the output directory.

**Acceptance Scenarios**:

1. **Given** a developer has ggen v26.5.19 installed, **When** they run `ggen init --v26.5.19 my-project`, **Then** a project structure is created containing only `ggen.toml`, `ontology/` directory with schema files, and `templates/` directory with rendering templates
2. **Given** a new v26.5.19 project, **When** the developer defines a simple type in `ontology/domain.ttl`, **Then** running `ggen sync` generates complete source code in `generated/` without requiring any hand-written code
3. **Given** a v26.5.19 project with ontology definitions, **When** the developer regenerates with `ggen sync`, **Then** the output is bit-for-bit identical to the previous generation (deterministic)
4. **Given** a v26.5.19 project, **When** the developer inspects the repository, **Then** the `generated/` directory is gitignored and only 3T files (TOML, Tera, Turtle) are version-controlled
...
**Why this priority**: The staged pipeline is the core mechanism of v26.5.19's A = μ(O) constitutional equation. Without this, v26.5.19 is just another template-based code generator rather than an ontology-first system.
...
**Why this priority**: Constitutional invariants are what distinguish v26.5.19 from previous versions and traditional build systems. They guarantee zero-drift by construction rather than by discipline.
...
1. **Given** a v26.5.19 project, **When** `ggen sync` is run twice without ontology changes, **Then** the second run produces no file changes (idempotence: μ∘μ = μ)
...
**Why this priority**: Guards are a security feature that becomes critical in production but isn't required for basic v26.5.19 functionality. Can be added after core pipeline works.
...
The v26.5.19 system includes its own type definitions, compilation rules, and projection logic in its ontology, enabling it to regenerate itself by running `ggen sync` on the ggen-core ontology.

**Why this priority**: This is the ultimate validation of v26.5.19's power and completeness, but it's a demonstration feature rather than essential functionality. Core v26.5.19 can work without self-generation capability initially.

**Independent Test**: Can be tested by loading the ggen-core ontology (which defines the v26.5.19 compiler itself), running `ggen sync` on it, and verifying that the generated Rust code compiles into a working ggen binary that is functionally equivalent to the original.

**Acceptance Scenarios**:

1. **Given** the ggen-core ontology defining the v26.5.19 compiler, **When** `ggen sync` runs on it, **Then** complete Rust source code for ggen is generated in `generated/`
2. **Given** generated ggen source, **When** it's compiled with `cargo build`, **Then** it produces a binary that can successfully compile other v26.5.19 projects
...
- **What happens when generated code doesn't compile?** This is outside v26.5.19's responsibility - the ontology author must ensure their ontology produces valid code for the target language (v26.5.19 guarantees deterministic projection, not correctness)
...
- **FR-001**: System MUST parse TOML configuration files conforming to the ggen v26.5.19 schema, validating all required sections (`[project]`, `[v26.5.19]`, `[v26.5.19.passes]`, `[v26.5.19.invariants]`)
...
- **FR-016**: System MUST preserve TOML, Tera, and Turtle files as the only version-controlled files in a v26.5.19 project
...
- **FR-018**: System MUST support `ggen init --v26.5.19 <project-name>` to scaffold a new v26.5.19 project with minimal ontology, templates, and configuration
...
- **Ontology (O)**: The semantic substrate consisting of RDF triples in Turtle format. Represents the complete system specification including types, properties, constraints, and domain logic. The sole source of truth in v26.5.19.

- **Projection Function (μ)**: The deterministic transformation pipeline consisting of five passes (μ₁-μ₅). Takes ontology as input, produces artifacts as output. Implemented by the ggen v26.5.19 compiler.
...
- **SC-001**: Developers can initialize a new v26.5.19 project and generate working code from a minimal ontology (defining one type with properties) in under 3 commands and 2 minutes
...
- **SC-007**: The ggen-core ontology (defining the v26.5.19 compiler itself) successfully generates a compilable Rust codebase in under 10 seconds
...
- **A-003**: Ontologies authored by developers are semantically correct - v26.5.19 guarantees deterministic projection, not logical correctness of the specification
- **A-004**: Build systems for target languages (cargo, npm, etc.) are available to compile generated code - v26.5.19 only generates source files
...
- **A-007**: Projects using v26.5.19 can tolerate a paradigm shift from imperative programming to declarative ontology authoring

## Dependencies *(optional - only if relevant)*

- **D-001**: Rust 1.75+ toolchain for building the ggen v26.5.19 compiler itself
...
- **NG-001**: Runtime code execution or interpretation - v26.5.19 only generates static artifacts
- **NG-002**: Graphical ontology editors or visual programming - v26.5.19 assumes text-based Turtle editing
- **NG-003**: Language-specific syntax validation - if generated code doesn't compile, that's an ontology/template authoring error
- **NG-004**: Automatic ontology migration from v26.5.19 to v26.5.19 - migration requires manual rewriting
...
- **NG-008**: Package management or distribution of ontologies - v26.5.19 projects are standalone Git repositories
...
- **R-005**: **Risk**: Meta-circular self-generation creates bootstrapping complexity. **Mitigation**: Maintain a hand-written v26.5.19 compiler as the bootstrap until self-generation is proven stable.
