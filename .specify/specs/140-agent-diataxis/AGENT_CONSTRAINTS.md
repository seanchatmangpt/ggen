...
## `E0013`
**RULE:** SELECT queries in generation rules must include an ORDER BY clause.
**REASON:** The generator requires deterministic query output to guarantee reproducible template rendering.
**FIX:** Add an ORDER BY clause to the SELECT query in your generation rule.

## `E0014`
**RULE:** All packs referenced in a query source must be declared in the packs section.
**REASON:** The generator must know the package dependency layout to resolve template and query files.
**FIX:** Add the referenced pack under the `[[packs]]` array in your `ggen.toml`.
