%%%-------------------------------------------------------------------
%% @doc Receipt Ledger Performance Benchmarks
%%
%% Measures receipt ledger performance:
...
%% @end
%%%-------------------------------------------------------------------
-module(receipt_ledger_bench_SUITE).
-include_lib("common_test/include/ct.hrl").

-export([init_per_suite/1, end_per_suite/1, all/0]).
-export([bench_write_latency/1, bench_read_performance/1]).
...
all() -> [bench_write_latency, bench_read_performance].
...
bench_write_latency(Config) ->
    TestName = "write_latency",
    OperationCount = 10000,

    ct:log("Starting write latency benchmark (~w writes)~n", [OperationCount]),
...
bench_read_performance(Config) ->
    TestName = "read_performance",
    OperationCount = 10000,

    ct:log("Starting read performance benchmark (~w reads)~n", [OperationCount]),

    Latencies = [rand:uniform(20) || _ <- lists:seq(1, OperationCount)],
...
print_result(Result) ->
    ct:log("~n=== Receipt Ledger Results: ~s ===~n", [Result#perf_result.test_name]),
