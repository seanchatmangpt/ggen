...
handle_info(check_alerts, State) ->
    ...
    %% metrics_collector:get_metrics() returns {ok, [#metric_snapshot{}]}
    %% Access record fields directly since record is defined in metrics_collector module
    case metrics_collector:get_metrics() of
        {ok, [LatestSnapshot | _]} ->
            %% Access as record - record is defined in metrics_collector module
            Memory = element(4, LatestSnapshot),  % memory field
            Processes = element(5, LatestSnapshot),  % processes field
            MessageQueues = element(6, LatestSnapshot),  % message_queues field
            Errors = element(7, LatestSnapshot),  % errors field
            check_thresholds({Memory, Processes, MessageQueues, Errors});
        _ -> ok
    end,
