//! Simple test to run validate_pipeline quality gates on ggen project
...
fn main() {
    ...
    let manifest: GgenManifest = toml::from_str(&manifest_content)
        .expect("Failed to parse ggen.toml");
    ...
    match result {
        ...
        Err(e) => {
            ...
            for checkpoint in checkpoints {
                ...
                println!("  {} {} - {}", status.chars().next().unwrap(), checkpoint.name, status);
            }
        }
    }
}
