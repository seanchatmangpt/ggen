...
check_system_health() ->
    ...
    case cluster_mgr:get_cluster_status() of
        ...
        {error, ClusterReason} ->
            io:format("Failed to get cluster status: ~p~n", [ClusterReason])
    end,
    ...
    case metrics_collector:get_metrics() of
        ...
        {error, MetricsReason} ->
            io:format("Failed to get metrics: ~p~n", [MetricsReason])
    end,
    ...
    case alert_manager:get_active_alerts() of
        ...
        {error, AlertsReason} ->
            io:format("Failed to get alerts: ~p~n", [AlertsReason])
    end,
...
setup_alert_policies() ->
    ...
    case alert_manager:register_alert_policy("high_memory", HighMemoryPolicy) of
        ...
        {error, MemoryError} ->
            io:format("  ✗ Failed to register high memory alert: ~p~n", [MemoryError])
    end,
    ...
    case alert_manager:register_alert_policy("low_disk", LowDiskPolicy) of
        ...
        {error, DiskError} ->
            io:format("  ✗ Failed to register low disk alert: ~p~n", [DiskError])
    end,
