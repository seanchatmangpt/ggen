...
```bash
cd ~/.ggen/mcpp/tai-erlang-autonomics && rebar3 compile
...
```bash
cd ~/.ggen/mcpp/tai-erlang-autonomics && rebar3 release
...
grep -r "mock\|fake\|stub\|TODO.*implement\|FIXME" \
  ~/.ggen/mcpp/tai-erlang-autonomics/apps/tai_autonomics/src --include="*.erl"
```
...
```bash
cd ~/.ggen/mcpp/tai-erlang-autonomics && terraform validate
