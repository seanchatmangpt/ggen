...
int main(int argc, char **argv)
{
  ...
  knhks_hook_ir_t ask = {.op = KNHKS_OP_ASK_SP, .s = test_subj, .p = test_pred, .k = 0, .o = 0, .select_out = NULL, .select_capacity = 0};
  knhks_hook_ir_t ge = {.op = KNHKS_OP_COUNT_SP_GE, .s = test_subj, .p = test_pred, .k = 1, .o = 0, .select_out = NULL, .select_capacity = 0};
  ...
  // measure
  const int N = 200000;
  double ns_ask = knhks_bench_eval(&ctx, &ask, N);
  double ns_ge = knhks_bench_eval(&ctx, &ge, N);
