// rust/knhks-cli/src/commands/epoch.rs
...
pub fn create(id: String, tau: u32, lambda: String) -> Result<(), String> {
    ...
    // Call Erlang RC layer: knhks_epoch:schedule(Tau, Plan, CoverId)
    let plan_str = plan.join(",");
    let output = Command::new("erl")
        .args(&["-noshell", "-eval"])
        .arg(format!(
            "knhks_epoch:schedule(\"{}\", {}, [{}], undefined).",
            id, tau, plan_str
        ))
        .arg("-s")
        .arg("init")
...
pub fn run(id: String) -> Result<(), String> {
    ...
    let output = Command::new("erl")
        .args(&["-noshell", "-eval"])
        .arg(format!(
            "{{Actions, Receipt}} = knhks_epoch:run(\"{}\"), io:format(\"Actions: ~p~nReceipt: ~p~n\", [Actions, Receipt]).",
            id
        ))
        .arg("-s")
        .arg("init")
...
pub fn list() -> Result<(), String> {
    ...
    let output = Command::new("erl")
        .args(&["-noshell", "-eval"])
        .arg("io:format(\"~p~n\", [knhks_epoch:list()]).")
        .arg("-s")
        .arg("init")
        .arg("stop")
        .output();
