...
static int test_aot_guard_invalid_run_length(void) {
    ...
    assert(knhk_aot_validate_ir(KNHK_OP_ASK_SP, 9, 0) == false);
    
    // Run length = 0 is valid (empty run)
    assert(knhk_aot_validate_ir(KNHK_OP_ASK_SP, 0, 0) == true);
...
static int test_aot_guard_unique_sp_constraint(void) {
    ...
    assert(knhk_aot_validate_ir(KNHK_OP_UNIQUE_SP, 1, 0) == true);
    assert(knhk_aot_validate_ir(KNHK_OP_UNIQUE_SP, 2, 0) == false);
...
static int test_aot_guard_count_constraint(void) {
    ...
    assert(knhk_aot_validate_ir(KNHK_OP_COUNT_SP_GE, 8, 8) == true);
    assert(knhk_aot_validate_ir(KNHK_OP_COUNT_SP_GE, 8, 9) == false);
...
static int test_aot_guard_run_validation(void) {
    ...
    knhk_pred_run_t valid_run = { .pred = 0xC0FFEE, .off = 0, .len = 8 };
    assert(knhk_aot_validate_run(valid_run) == true);
    
    knhk_pred_run_t invalid_run = { .pred = 0xC0FFEE, .off = 0, .len = 9 };
    assert(knhk_aot_validate_run(invalid_run) == false);
...
static int test_aot_guard_hot_path_integration(void) {
    ...
    assert(knhk_aot_validate_run(run) == true);
...
static int test_aot_guard_routes_to_cold_path(void) {
    ...
    reset_test_data();
    // Note: knhk_pin_run is void, validation happens in AOT guard
    // Hot path will reject invalid runs internally
    knhk_pin_run(&ctx, invalid_run);
...
static int test_all_hot_path_ops_pass_aot(void) {
    ...
    for (size_t i = 0; i < num_ops; i++) {
        ...
        uint64_t run_len = (op == KNHK_OP_UNIQUE_SP) ? 1 : 8;
        uint64_t k = (op == KNHK_OP_UNIQUE_SP) ? 0 : 5;
        
        bool valid = knhk_aot_validate_ir(op, run_len, k);
        assert(valid == true);
    }
...
static int test_guard_enforcement_pipeline(void) {
    ...
    knhk_pred_run_t valid_run = { .pred = 0xC0FFEE, .off = 0, .len = 1 };
    assert(knhk_aot_validate_run(valid_run) == true);
    knhk_pin_run(&ctx, valid_run);
    ...
    assert(knhk_aot_validate_ir(ir.op, valid_run.len, ir.k) == true);
