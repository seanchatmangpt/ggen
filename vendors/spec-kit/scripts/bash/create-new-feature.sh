...
if [ "$FORMAT" = "rdf" ]; then
    ...
    # Symlink schema from vendors/spec-kit-3t
    SCHEMA_SOURCE="$REPO_ROOT/vendors/spec-kit-3t/ontology/spec-kit-schema.ttl"
    ...
    # Copy Tera templates
    TEMPLATE_SOURCE="$REPO_ROOT/vendors/spec-kit/templates/spec.tera"
...
EOF
    ...
    # Generate ggen.toml
    GGEN_CONFIG_SCRIPT="$REPO_ROOT/vendors/spec-kit/scripts/bash/generate-ggen-config.sh"
...
EOF
    ...
    if $JSON_MODE; then
        ...
        echo "  2. Use RDF helper templates in vendors/spec-kit/templates/rdf-helpers/"
