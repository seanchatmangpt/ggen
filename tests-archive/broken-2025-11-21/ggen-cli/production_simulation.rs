...
impl ProductionTestEnv {
    ...
    fn set_local_registry(&self, registry_file: &Path) {
        std::env::set_var(
            "GGEN_REGISTRY_URL",
            registry_file.to_string_lossy().as_ref(),
        );
    }
}
...
async fn test_production_search_workflow() -> Result<()> {
    ...
    assert!(
        !first_result.name.is_empty(),
        "Package name should not be empty"
    );
    assert!(
        !first_result.description.is_empty(),
        "Description should not be empty"
    );
    assert!(
        !first_result.version.is_empty(),
...
async fn test_production_install_workflow() -> Result<()> {
    ...
    let install_input = ggen_domain::marketplace::InstallInput {
        ...
        force: options.force,
        no_dependencies: !options.with_dependencies,
        dry_run: options.dry_run,
    };

    let result = execute_install(install_input).await;

    // Assert: Verify installation process works
    assert!(
        result.is_ok(),
        "Installation should succeed in production environment"
    );
...
async fn test_production_search_pagination() -> Result<()> {
    ...
    assert!(page1.len() <= 2, "First page should respect limit");
...
async fn test_production_error_handling() -> Result<()> {
    ...
    std::env::set_var(
        "GGEN_REGISTRY_URL",
        "file:///nonexistent/path/registry.json",
    );
...
async fn test_production_concurrent_searches() -> Result<()> {
    ...
    assert!(results1.len() <= 5, "First search should respect limit");
    assert!(results2.len() <= 5, "Second search should respect limit");
    assert!(results3.len() <= 5, "Third search should respect limit");
...
async fn test_production_registry_fetch_simulation() -> Result<()> {
    ...
    assert!(!results.is_empty(), "Should load packages from registry");
...
async fn test_production_empty_query_returns_all() -> Result<()> {
    ...
    assert!(results.len() <= 100, "Results should respect limit");

    // In production, empty query returns all packages up to limit
    // We verify the limit is respected, not the exact count
    // (The actual count depends on registry contents)
