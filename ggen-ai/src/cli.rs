...
pub fn create_client_from_args(matches: &ArgMatches) -> Result<Box<dyn LlmClient>> {
    let config = extract_llm_config(matches)?;
    config.create_contextual_client()
}

/// Create a client with specific configuration
pub fn create_client_with_config(config: &GlobalLlmConfig) -> Result<Box<dyn LlmClient>> {
    config.create_contextual_client()
}
...
mod tests {
    ...
    fn test_cli_config_builder() {
        ...
        assert!(cmd.get_arguments().any(|arg| arg.get_id() == "llm-provider"));
    ...
    fn test_extract_llm_config() {
        ...
        let matches = cmd.try_get_matches_from(&["test", "--llm-provider", "ollama", "--model", "llama3.2"]).unwrap();
        let config = extract_llm_config(&matches).unwrap();
