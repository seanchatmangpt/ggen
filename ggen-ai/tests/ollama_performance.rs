...
use ggen_ai::{
    generators::TemplateGenerator, skip_if_ollama_unavailable,
...
async fn test_ollama_performance_template_generation() {
    ...
    let test_cases = vec![
        ...
        (
            "REST API service",
            vec!["Use FastAPI", "Include CRUD", "Add validation"],
        ),
        (
    ...
    assert!(
        avg_time < Duration::from_secs(60),
        "Average generation time should be under 60s"
    );
}
...
async fn test_ollama_error_recovery() {
    ...
    assert!(
        recovered_errors < test_cases.len(),
        "Some edge cases should succeed"
    );
}
