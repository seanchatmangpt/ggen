...
async fn test_ollama_template_generation() {
    ...
    match result {
        Ok(Ok(template)) => {
            ...
            assert!(!template.body.is_empty(), "Template body should not be empty");
            // Note: front.to might be None for some templates, that's okay
            println!("📝 Generated template length: {} characters", template.body.len());
        }
...
async fn test_ollama_sparql_generation() {
    ...
    match result {
        Ok(Ok(query)) => {
            ...
            assert!(query.contains("SELECT"), "SPARQL query should contain SELECT");
...
async fn test_ollama_ontology_generation() {
    ...
    match result {
        Ok(Ok(ontology)) => {
            ...
            assert!(ontology.contains("@prefix"), "Ontology should contain prefixes");
            println!("🏗️ Generated ontology length: {} characters", ontology.len());
        }
...
async fn test_ollama_mcp_tools() {
    ...
    assert!(ai_tools.has_template_generator(), "Template generator should be initialized");
    assert!(ai_tools.has_sparql_generator(), "SPARQL generator should be initialized");
    assert!(ai_tools.has_ontology_generator(), "Ontology generator should be initialized");
    assert!(ai_tools.has_refactor_assistant(), "Refactor assistant should be initialized");
...
async fn test_ollama_cli_integration() {
    ...
    let output = tokio::process::Command::new("cargo")
        .args(&[
            "run", "--bin", "ggen", "--",
            "ai", "generate",
            "--description", "A Python web API with FastAPI",
            "--language", "python",
            "--framework", "fastapi",
    ...
    assert!(!content.is_empty(), "Generated template should not be empty");
    assert!(content.contains("FastAPI"), "Template should mention FastAPI");
...
async fn test_ollama_sparql_cli() {
    ...
    let output = tokio::process::Command::new("cargo")
        .args(&[
            "run", "--bin", "ggen", "--",
            "ai", "sparql",
            "--description", "Query for product inventory data",
            "--output", output_file.to_str().unwrap(),
        ])
        .current_dir("/Users/sac/ggen")
        .output()
    ...
    let content = std::fs::read_to_string(&output_file)
        .expect("Failed to read SPARQL output file");
...
async fn test_ollama_model_availability() {
    ...
    assert!(client.supports_model("qwen3-coder:30b"), "qwen3-coder:30b should be supported");
    
    // Test provider name
    assert_eq!(client.provider_name(), "ollama", "Provider name should be 'ollama'");
