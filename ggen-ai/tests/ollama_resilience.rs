...
use ggen_ai::{
    client::LlmClient, generators::TemplateGenerator, skip_if_ollama_unavailable,
...
async fn test_ollama_network_resilience() {
    ...
    assert!(
        ...
        "Should succeed with healthy Ollama service"
    );
}
...
async fn test_ollama_model_switching() {
    ...
    assert!(
        ...
        "Should work with at least one model"
    );
}
...
async fn test_ollama_resource_cleanup() {
    ...
    match final_result {
        Ok(Ok(template)) => {
            println!(
                "✅ Final template after cleanup: {} chars",
                template.body.len()
            );
        }
        Ok(Err(e)) => {
    ...
    assert!(
        templates.len() > 0,
        "Should generate templates before cleanup"
    );
}
...
async fn test_ollama_graceful_degradation() {
    ...
    assert!(
        graceful_degradations == scenarios.len(),
        "All scenarios should be handled gracefully"
    );
}
