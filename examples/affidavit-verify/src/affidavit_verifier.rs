...
//! Transcribed from `~/affidavit`'s `src/verifier.rs` (crates.io `affidavit`
//! v26.6.22), checksummed against
//! `packs/affidavit-pack/reference/affidavit_v26.6.22_src/verifier.rs`.
//! Every `stage_*` fn body and `verify()`'s dispatch loop are a direct,
//! real port -- not reinvented from `afd:rejectCondition`'s prose alone.
//!
//! decode -> check_format -> check_chain_integrity -> resolve_continuity ->
//! verify_commitments -> evaluate_profile -> emit Verdict.
//!
//! The pipeline never decides whether the underlying code was honest. It
//! checks a receipt (a witness) against a fixed format standard.
//!
//! Generated by `affidavit-pack`. Reference only -- this crate does not
//! invoke `affi`.

use crate::affidavit_chain::{recompute_chain, FORMAT_VERSION};
use crate::affidavit_types::{CheckOutcome, ProfileId, Receipt, Verdict};
...
// Verbatim body from `~/affidavit/src/verifier.rs`'s own
// `#[cfg(test)] mod tests` (checksummed original in
// `packs/affidavit-pack/reference/affidavit_v26.6.22_src/verifier.rs`,
// lines 215-317). The ONLY mechanical edit is `crate::types::` ->
// `crate::affidavit_types::`, forced by this pack's module-naming
// convention. Every assertion, helper fn, and test name is unchanged --
// this is the strongest fidelity proof this pack has: the reference
// author's own adversarial tests (tamper-a-commitment, seq-gap,
// wrong-format, duplicate-id) pass unmodified against ontology-generated
// code.
#[cfg(test)]
mod tests {
    use super::*;
    use crate::affidavit_types::{Blake3Hash, ObjectRef, OperationEvent};
