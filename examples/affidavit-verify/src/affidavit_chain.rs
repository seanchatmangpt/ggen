...
//! serialize and deserialize receipts. Transcribed from `~/affidavit`'s
//! `src/chain.rs` (crates.io `affidavit` v26.6.22) -- function bodies and
//! the chain rule are a direct, real port (not reinvented from the
//! README's prose description), checksummed against
...
use crate::affidavit_types::{canonical_bytes, Blake3Hash, OperationEvent, Receipt};
...
// Verbatim body from `~/affidavit/src/chain.rs`'s own
// `#[cfg(test)] mod tests` (checksummed original in
// `packs/affidavit-pack/reference/affidavit_v26.6.22_src/chain.rs`, lines
// 208-303). The ONLY mechanical edit is `crate::types::` ->
// `crate::affidavit_types::`, forced by this pack's module-naming
// convention (`affidavit_types` not `types`, to avoid collisions with
// other packs in a shared consumer -- see `affidavit_mod_wiring.rs.tmpl`'s
// note). Every assertion, helper fn, and test name is unchanged.
#[cfg(test)]
mod tests {
    use super::*;
    use crate::affidavit_types::{ObjectRef, OperationEvent};
