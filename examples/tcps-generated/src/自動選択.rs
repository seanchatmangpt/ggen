...
use crate::語彙::{
    方策番号, 時刻, 有無, 権限印, 準備印, 理由番号, 質量, 道具番号, 適格印, 経路番号,
};

const 印幅: usize = u64::BITS as usize;
...
pub fn 選択する<const 上限: usize>(
    ...
    if 方策が矛盾する(方策) {
        return 拒否を作る(方策, 要求, 拒否理由::方策矛盾, 0, 0, 入力要約);
    }
    if 方策.件数 == 0 {
        return 拒否を作る(
            方策,
    ...
    while 位置 < 方策.件数 {
        ...
        if (要求.取得権限 & 候補.必要権限) == 候補.必要権限 {
            権限候補 |= 印;

            if !要求.決定性必須 || 候補.決定的 {
                決定性候補 |= 印;

                if !要求.受領証必須 || 候補.受領証対応 {
                    受領証候補 |= 印;

                    if 候補.予測時間 <= 要求.最大時間 {
                        適格候補 |= 印;

                        if (要求.準備完了 & 候補.必要準備) == 候補.必要準備 {
                            準備候補 |= 印;
                            let 質量 = 候補.測度.乗法質量();
                            作業領域.質量[位置] = 質量;
                            if 質量 > 最良質量
    ...
    let 理由 = if 権限候補 == 0 {
        有無::有る(拒否理由::権限不足)
    } else if 要求.決定性必須 && 決定性候補 == 0 {
        有無::有る(拒否理由::決定性不足)
    } else if 要求.受領証必須 && 受領証候補 == 0 {
        有無::有る(拒否理由::受領証不足)
    } else if 適格候補 == 0 {
        有無::有る(拒否理由::時間超過)
    } else if 準備候補 == 0 {
...
fn 方策が矛盾する<const 上限: usize>(方策: &方策<上限>) -> bool {
    if 方策.件数 > 上限 || 方策.件数 > 印幅 {
        return true;
    }
...
fn 作業領域を消去する<const 上限: usize>(作業領域: &mut 作業領域<上限>) {
    let mut 位置 = 0usize;
    while 位置 < 上限 {
        作業領域.質量[位置] = 0;
        位置 += 1;
