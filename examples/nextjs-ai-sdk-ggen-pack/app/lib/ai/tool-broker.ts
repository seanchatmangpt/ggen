...
export class ToolRefusedError extends Error {
  ...
  constructor(readonly handlerKey: string) {
    super(`No admitted tool handler is registered for ${handlerKey}`);
  }
}
...
export class ToolBroker {
  ...
  register(handlerKey: string, handler: ToolHandler): void {
    if (this.#handlers.has(handlerKey)) {
      throw new Error(`duplicate tool handler registration: ${handlerKey}`);
    }
