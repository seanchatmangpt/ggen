...
fn validate_actions(
    ...
    let valid = match regime {
        PhaseRegime::Continuous => actions == [AutonomicAction::PreserveStandardWork],
        PhaseRegime::PhaseShift1000x => actions == PHASE_SHIFT_ACTIONS,
    };
    if valid {
        Ok(())
    } else {
