...
fn downstream_demand_flows_to_framework_receipted_execution() {
    ...
    let authorization = gate.authorize(&planned, "release-manager").expect("authorize");
    let authorized = planned.authorize(authorization).expect("bind authorization");
    ...
    assert_eq!(receipted.receipt().evidence_digest().len(), 64);
}
...
fn authorization_is_opaque_and_bound_to_one_plan() {
    ...
    let wrong = gate.authorize(&first, "release-manager").expect("authorize first");
    let refusal = second.authorize(wrong).expect_err("wrong plan must refuse");
    assert_eq!(refusal, Refusal::AuthorizationMismatch);
}

#[test]
fn praxis_cannot_authorize_a_plan_under_another_standard() {
    let (_, planned, _) = planned("change-4c");
    let wrong_gate = ReferencePraxisGate::new("standard-v2");
    let refusal = wrong_gate
        .authorize(&planned, "release-manager")
        .expect_err("wrong standard must refuse");
    assert_eq!(refusal, Refusal::StandardNotAdmitted);
}
