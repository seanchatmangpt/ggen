...
/-!
`rustStep` is the expected pure model of `aeneas-kernel/src/lib.rs`. Crown
standing additionally requires `scripts/verify-aeneas.sh` to regenerate the
model from Rust and prove the generated `step` definition extensionally equal to
`rustStep`. Extraction without that binding is explicitly insufficient.
-/
