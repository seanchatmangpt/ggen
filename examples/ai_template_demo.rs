...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let template_generator = TemplateGenerator::with_config(Box::new(template_client), config.clone());
    ...
    let template = template_generator.generate_template(
        "Generate a Rust data model for a User entity with id, name, and email fields",
        vec!["Include validation", "Use serde for serialization", "Add constructor method"]
    ).await?;
    
    println!("✅ Template generated successfully!");
    println!("Frontmatter: {:?}", template.front);
    println!("Body preview: {}", &template.body[..200.min(template.body.len())]);
...
ex:user2 a ex:User ; ex:name "Bob" ; ex:email "bob@example.com" ."#
    ...
    let sparql_query = sparql_generator.generate_query(
        &graph,
    ...
    let ontology = ontology_generator.generate_ontology(
        "User management domain",
        vec!["Include User class", "Add name and email properties", "Use OWL semantics"]
    ).await?;
