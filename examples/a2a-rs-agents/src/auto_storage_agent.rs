//! Auto Storage Agent — Demonstrates automatic storage creation from TOML config
...
impl AsyncMessageHandler for EchoHandler {
    async fn process_message(
        ...
        let response = Message::builder()
            .role(Role::Agent)
            .parts(vec![Part::text(format!("Auto-Storage Echo: {}", text))])
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Auto Storage Agent starting on http://127.0.0.1:3031");
    println!("Storage type configured in auto-storage-agent.toml");

    AgentBuilder::from_file("auto-storage-agent.toml")?
