...
use ggen_core::transport::streaming::{StreamBuilder, StreamControl, StreamMessage};
use ggen_core::transport::SessionId;
...
async fn test_streaming_control_pause_resume() {
    ...
    assert!(matches!(
        ...
        ggen_core::transport::streaming::StreamAction::Pause
    ));

    let resume_control = StreamControl::resume(session_id.clone(), Some(42));
    if let ggen_core::transport::streaming::StreamAction::Resume { from_position } = resume_control.action
    {
...
async fn test_streaming_control_cancel() {
    ...
    assert!(matches!(
        ...
        ggen_core::transport::streaming::StreamAction::Cancel
    ));
}
...
async fn test_streaming_control_acknowledge() {
    ...
    if let ggen_core::transport::streaming::StreamAction::Acknowledge { sequence } = ack_control.action {
...
async fn test_streaming_error_propagation() {
    use ggen_core::transport::TransportError;
