...
//!
//! ```ignore
//! use ggen_cli_lib::scaffolding::cli_generator::{WorkspaceGenerator, types::CliProject};
//! use std::path::Path;
//!
...
use crate::scaffolding::cli_generator::types::CliProject;
use std::path::Path;
...
///
/// ```rust,no_run
/// use ggen_cli_lib::scaffolding::cli_generator::workspace::WorkspaceGenerator;
/// use std::path::Path;
///
/// # fn main() -> ggen_cli_lib::error::Result<()> {
/// let generator = WorkspaceGenerator::new(Path::new("templates"))?;
...
impl WorkspaceGenerator {
    ...
    ///
    /// ```ignore
    /// use ggen_cli_lib::scaffolding::cli_generator::{workspace::WorkspaceGenerator, types::CliProject};
    /// use std::path::Path;
    ///
    /// # fn main() -> ggen_cli_lib::error::Result<()> {
    /// let generator = WorkspaceGenerator::new(Path::new("templates"))?;
    ...
    pub fn generate(&self, project: &CliProject, output_dir: &Path) -> Result<()> {
        ...
        let cli_crate = project.cli_crate.as_ref().ok_or_else(|| {
            GgenError::InvalidInput("cli_crate is required for workspace generation".to_string())
        })?;
        let core_crate = project.domain_crate.as_ref().ok_or_else(|| {
            GgenError::InvalidInput("domain_crate is required for workspace generation".to_string())
        })?;
