...
//!
//! **Deviation note (T030, `specs/014-ggen-core-replacement/tasks.md`)**: this file is
//! NOT one of the 11 files the port ticket
//! (`docs/jira/v26.7.16/07-PROJECT-SCAFFOLDING-PORT.md`) lists. `project_generator/mod.rs`
//! (one of the 11) calls `crate::security::command::SafeCommand` at 3 call sites (git
//! init, cargo fetch, npm install) — that's ggen-core's much larger `security` module
//! (12 files: intrusion detection, audit trail, alerting, etc.), which is out of this
//! ticket's scope and not otherwise needed by `ggen-cli`. Porting the whole `security`
//! module for 3 call sites would be scope creep in the other direction; dropping the
//! whitelist/shell-metacharacter validation entirely (falling back to bare
//! `std::process::Command`) would silently weaken a deliberate "SECURITY FIX (Week 4)"
//! hardening measure the original code chose. This file is a scoped, private copy of
//! just `ggen-core/src/security/command.rs`'s `SafeCommand`/`CommandError`, adapted to
//! `ggen-cli`'s own `GgenError`, kept `mod`-private to `project_generator` (not
//! re-exported), so it composes into this module's `Result` without creating a new
//! public surface.
...
impl From<CommandError> for GgenError {
    fn from(err: CommandError) -> Self {
        GgenError::ExternalServiceError(err.to_string())
    }
}
...
impl SafeCommand {
    ...
    pub fn current_dir(mut self, dir: &Path) -> Result<Self> {
        ...
        if !dir.exists() {
            return Err(GgenError::FileError(format!(
