...
//!
//! ```rust,no_run
//! use ggen_cli_lib::scaffolding::project_generator::common::validate_project_name;
//!
//! # fn main() -> ggen_cli_lib::error::Result<()> {
//! // Valid names
...
//!
//! ```rust,no_run
//! use ggen_cli_lib::scaffolding::project_generator::common::{generate_editorconfig, generate_prettierrc};
//!
...
pub fn validate_project_name(name: &str) -> Result<()> {
    if name.is_empty() {
        return Err(GgenError::ValidationError(
    ...
    if name.starts_with('-') || name.starts_with('_') {
        return Err(GgenError::ValidationError(
            "Project name cannot start with '-' or '_'".to_string(),
        ));
    }
