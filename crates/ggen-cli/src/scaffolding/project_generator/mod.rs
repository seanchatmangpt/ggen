...
//!
//! ```rust,no_run
//! use ggen_cli_lib::scaffolding::project_generator::{ProjectConfig, ProjectType, create_new_project};
...
impl DependencyInstaller {
    ...
    fn install_cargo_deps(&self, path: &Path) -> Result<()> {
        ...
        eprintln!("ℹ️  Installing Cargo dependencies...");
    ...
    fn install_npm_deps(&self, path: &Path) -> Result<()> {
        ...
        eprintln!("ℹ️  Installing npm dependencies...");
...
pub async fn create_new_project(config: &ProjectConfig) -> Result<()> {
    ...
    eprintln!("✅ Successfully created project: {}", config.name);
    eprintln!("ℹ️     Type: {}", config.project_type);
    eprintln!("ℹ️     Path: {}", project_path.display());
