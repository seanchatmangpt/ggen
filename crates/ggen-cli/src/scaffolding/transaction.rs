...
//!
//! Ported verbatim (module path only) from `ggen-core/src/codegen/transaction.rs` during the
//! ggen-core retirement migration (`specs/014-ggen-core-replacement`, Phase 3 "non-colliding
//! noun re-points"): `cmds/init.rs` is the sole caller of `FileTransaction` left depending on
//! ggen-core for this capability (`cmds/wizard.rs` also uses `FileTransaction` but is out of
//! scope for this port and keeps using `ggen_core::codegen::FileTransaction` directly, since
//! ggen-core stays on disk and is not being deleted). The only ggen-core-internal coupling in
//! the original was `crate::utils::error::{Error, Result}`, which ggen-cli already has its own
//! copy of (`crate::utils::error`, ported in task T035) — so this port swaps that one import
//! and is otherwise byte-for-byte identical, including its test suite.
