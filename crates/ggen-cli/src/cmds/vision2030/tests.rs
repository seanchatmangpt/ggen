...
// ---------------------------------------------------------------------------
// Real-pack tests: exercise the binary against packs/vision-2030-phase-change-pack
// exactly as shipped (no fixtures, no mocks). Chicago style: real files, real
// evaluator, state-based assertions.
// ---------------------------------------------------------------------------
...
/// The committed manifest was generated by the pack's Python tool
/// (`tools/catalog_to_manifest.py`). The Rust `project` verb is an
/// independent re-implementation (different language, no shared code). If the
/// two disagree on any field for any of the 52 capabilities, one of them is
/// wrong -- this is the cross-implementation falsifier for the projection, and
/// it also catches the committed manifest drifting from the catalog.
#[test]
fn project_matches_committed_python_generated_manifest() {
    let catalog = pack_root().join("catalog/vision-2030-capabilities.json");
    let committed = pack_root().join("catalog/vision-2030-program.manifest.json");
    let projected = project_catalog(&catalog).expect("project real catalog");
    let projected: Value = serde_json::to_value(&projected).expect("to value");
    let committed: Value =
        serde_json::from_slice(&fs::read(&committed).expect("read committed")).expect("json");
    assert_eq!(
        projected, committed,
        "Rust `project` and the committed Python-generated manifest disagree -- either the \
         catalog changed without re-running tools/catalog_to_manifest.py, or one projection \
         is wrong"
    );
    let count = projected["capabilities"].as_array().expect("array").len();
    assert_eq!(
        count, 52,
        "real catalog is expected to carry 52 capabilities"
    );
    assert!(
...
/// `project` must refuse anything that is not a ggen.vision2030.catalog.v1
/// document rather than emitting a plausible-looking manifest from it.
#[test]
fn project_refuses_non_catalog_input() {
    let directory = tempfile::tempdir().expect("tempdir");
    let not_a_catalog = directory.path().join("manifest.json");
    // A valid *manifest* is not a valid *catalog*: wrong schema id.
    fs::copy(
        pack_root().join("catalog/vision-2030-program.manifest.json"),
        &not_a_catalog,
    )
    .expect("copy");
    let error = project_catalog(&not_a_catalog).expect_err("must refuse");
    let text = error.to_string();
    assert!(
        text.contains("catalog.v1") || text.contains("not a ggen.vision2030.catalog.v1"),
        "refusal must name the expected schema, got: {text}"
    );
}

/// `report` renders the real pack manifest through the real pack template. The
/// rendered Markdown must carry one heading per capability and state the
/// evaluator's standing verbatim -- a template/Report field mismatch would
/// surface here as a render error or a wrong count, not as silent drift.
#[test]
fn report_renders_real_pack_manifest_as_designed() {
    let directory = tempfile::tempdir().expect("tempdir");
    let manifest = pack_root().join("catalog/vision-2030-program.manifest.json");
    let summary = report(
        manifest.to_string_lossy().to_string(),
        directory.path().to_string_lossy().to_string(),
    )
    .expect("report verb");
    assert_eq!(summary["standing"], "DESIGNED");
    assert_eq!(summary["achieved"], false);
    assert_eq!(summary["capabilities"], 52);
    let rendered =
        fs::read_to_string(directory.path().join("vision-2030-report.md")).expect("rendered md");
    assert!(
        rendered.starts_with("# Vision 2030 Capability Report"),
        "template header missing"
    );
    assert_eq!(
        rendered.lines().filter(|l| l.starts_with("### ")).count(),
        52,
        "one `### <id>` block per capability"
    );
    assert!(rendered.contains("**Standing:** `DESIGNED`"));
    // Every DESIGNED capability lists its violations; 52 x 8 lines in this pack today.
    assert_eq!(
        rendered.lines().filter(|l| l.starts_with("  - ")).count(),
        416,
        "every evaluator violation must be rendered, none dropped"
    );
    let digest = summary["report_markdown_digest"].as_str().expect("digest");
    assert_eq!(
        digest,
        format!("blake3:{}", digest_bytes(rendered.as_bytes())),
        "returned digest must be the digest of the bytes actually written"
    );
}
