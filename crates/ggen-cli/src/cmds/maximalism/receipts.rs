...
fn paths(output: &Path) -> (PathBuf, PathBuf, PathBuf) {
    (
        output.join("maximalism-report.json"),
        output.join("maximalism-intent.json"),
        output.join("maximalism-result.json"),
    )
}
...
pub(super) fn issue(manifest: &Path, output: &Path) -> Result<Value> {
    ...
    let intent = ProgramReceipt::issue(
        "maximalism-evaluate-intent",
        &report,
        &previous_digest(&result_path)?,
        vec!["maximalism-report.json".to_string()],
    )?;
    write_json(&intent_path, &intent)?;
    write_json(&report_path, &report)?;
    let result = ProgramReceipt::issue(
        "maximalism-evaluate-result",
        &report,
        &intent.digest,
        vec![
            "maximalism-report.json".to_string(),
            "maximalism-intent.json".to_string(),
        ],
    )?;
...
pub(super) fn replay(manifest: &Path, output: &Path) -> Result<Value> {
    ...
    Ok(json!({
        "schema": "ggen.vision2030.maximalism-replay.v1",
