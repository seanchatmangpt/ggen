...
fn load(file: String, format: Option<String>) -> Result<LoadOutput> {
    use mcpp_domain::graph::{execute_load, LoadInput};
    ...
    let result = crate::runtime::block_on(async move {
        execute_load(input)
            .await
            .map_err(|e| mcpp_utils::error::Error::new(&format!("Load failed: {}", e)))
    })
...
fn query(
    ...
    let result = crate::runtime::block_on(async move {
        execute_query(input)
            .await
            .map_err(|e| mcpp_utils::error::Error::new(&format!("Query failed: {}", e)))
    })
    .map_err(|e: mcpp_utils::Error| clap_noun_verb::NounVerbError::execution_error(e.to_string()))?
    .map_err(|e: mcpp_utils::Error| {
        clap_noun_verb::NounVerbError::execution_error(e.to_string())
...
fn export(input_file: String, output: String, format: String) -> Result<ExportOutput> {
    use mcpp_domain::graph::{execute_export, ExportInput};
    ...
    let result = crate::runtime::block_on(async move {
        execute_export(input_data)
            .await
            .map_err(|e| mcpp_utils::error::Error::new(&format!("Export failed: {}", e)))
    })
    .map_err(|e: mcpp_utils::Error| clap_noun_verb::NounVerbError::execution_error(e.to_string()))?
    .map_err(|e: mcpp_utils::Error| {
        clap_noun_verb::NounVerbError::execution_error(e.to_string())
...
fn visualize(input_file: String, format: Option<String>) -> Result<VisualizeOutput> {
    use mcpp_domain::graph::{execute_visualize, VisualizeInput};
    ...
    let result = crate::runtime::block_on(async move {
        execute_visualize(input_data)
            .await
            .map_err(|e| mcpp_utils::error::Error::new(&format!("Visualize failed: {}", e)))
    })
    .map_err(|e: mcpp_utils::Error| clap_noun_verb::NounVerbError::execution_error(e.to_string()))?
    .map_err(|e: mcpp_utils::Error| {
        clap_noun_verb::NounVerbError::execution_error(e.to_string())
