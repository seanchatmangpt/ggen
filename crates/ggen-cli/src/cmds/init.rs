//! Init Command - Initialize a new mcpp project
//!
//! `mcpp init` scaffolds a minimal, working mcpp project with hardcoded files.
...
/// Output for the `mcpp init` command
...
name = "my-mcpp-project"
version = "0.1.0"
description = "A mcpp project initialized with default templates"
authors = ["mcpp init"]
license = "MIT"

# BIG BANG 80/20: Specification Closure First
# Before using mcpp, confirm:
...
const DOMAIN_TTL: &str = r#"# mcpp v6: Schema.org Example Ontology
...
# 4. Run: mcpp sync --validate-only
"#;

const MAKEFILE: &str = r#".PHONY: help setup build clean

help:
	@echo "mcpp project - Available targets:"
	@echo "  make setup   - Run startup.sh to initialize project"
	@echo "  make build   - Generate code from ontology (mcpp sync)"
...
const EXAMPLE_TEMPLATE: &str = r#"# Example Tera Template for mcpp
# This template is called by the generation.rules in mcpp.toml
...
*Generated by mcpp v6*
"#;

const STARTUP_SH: &str = r#"#!/bin/bash
# Startup script for mcpp project initialization
...
echo "🚀 mcpp v6: BIG BANG 80/20 Screening Gate"
echo ""
...
read -r q1
if [[ "$q1" != "yes" ]]; then
    echo "❌ STOP. You need real data to validate with. Build MVP first, use mcpp after."
    exit 1
fi
...
echo "  4. Run 'mcpp sync --validate-only' to test without writing"
echo "  5. Run 'mcpp sync' to generate code"
...
echo "  - mcpp Docs: https://docs.mcpp.io"
...
/// Initialize a new mcpp project with default structure and scripts.
///
/// Creates a minimal, working mcpp project scaffold with:
...
fn perform_init(
    ...
    // List of mcpp-specific artifacts to check for
    let mcpp_artifacts = [
        "mcpp.toml",
        "Makefile",
        "schema/domain.ttl",
    ...
    // Check if mcpp has already been initialized in this directory
    let has_mcpp_artifacts = mcpp_artifacts.iter().any(|artifact| {
        let path = base_path.join(artifact);
        path.exists()
    });

    if has_mcpp_artifacts && !force {
        return Ok(InitOutput {
            ...
            error: Some(
                "mcpp project already initialized here. Use --force to reinitialize.".to_string(),
            ),
            warning: None,
    ...
    let preflight = mcpp_core::validation::PreFlightValidator::for_init(base_path);
    ...
    let temp_test = base_path.join(".mcpp_write_test");
    ...
    let dirs = vec!["schema", "templates", "scripts"];
    for dir in &dirs {
    ...
    tx.write_file(&toml_path, GGEN_TOML)
        .map_err(|e| GgenError::CommandError(format!("Failed to write mcpp.toml: {}", e)))?;
    ...
    if !gitignore_exists {
        let gitignore_content = "# mcpp outputs\n.mcpp/\n";
    ...
    let readme_content = r#"# My mcpp Project

Generated by `mcpp init`.
...
```bash
mcpp mcp init-config --force
```

To validate configuration:
```bash
mcpp mcp validate-config
```
...
mod tests {
    ...
    fn test_atomic_init_success() {
        ...
        assert!(
            result.directories_created.len() >= 3,
            "Should have created at least 3 directories"
        );

        // Verify actual files exist on disk
        let base = PathBuf::from(project_path);
        assert!(base.join("mcpp.toml").exists(), "mcpp.toml should exist");
        assert!(
    ...
    fn test_init_force_overwrites_files() {
        ...
        // Modify a file
        let toml_path = base.join("mcpp.toml");
        let original_content = fs::read_to_string(&toml_path).expect("Failed to read mcpp.toml");
        fs::write(&toml_path, "# Modified\n").expect("Failed to modify mcpp.toml");
        ...
        let restored_content = fs::read_to_string(&toml_path).expect("Failed to read mcpp.toml");
        assert_eq!(
