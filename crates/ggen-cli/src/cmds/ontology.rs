...
fn extract(
    ...
    let result = crate::runtime::block_on(async move {
        ...
        let file_content = std::fs::read_to_string(&ontology_path)
            .map_err(|e| ggen_utils::error::Error::new(&format!("Failed to read file: {}", e)))?;

        graph.insert_turtle(&file_content).map_err(|e| {
            ggen_utils::error::Error::new(&format!("Failed to load ontology: {}", e))
        })?;
    ...
    let result = crate::runtime::block_on(async move {
        ...
        if lang_clone == "typescript" {
            ...
            let interfaces = TypeScriptGenerator::generate_interfaces(&schema).map_err(|e| {
                ggen_utils::error::Error::new(&format!("Interface generation failed: {}", e))
            })?;

            let interfaces_path = format!("{}/types.ts", output_dir);
            std::fs::write(&interfaces_path, interfaces).map_err(|e| {
                ggen_utils::error::Error::new(&format!("Failed to write types: {}", e))
            })?;
            files_generated += 1;
            ...
            if zod {
                let zod_schemas =
                    TypeScriptGenerator::generate_zod_schemas(&schema).map_err(|e| {
                        ggen_utils::error::Error::new(&format!("Zod generation failed: {}", e))
                    })?;

                let zod_path = format!("{}/schemas.ts", output_dir);
                std::fs::write(&zod_path, zod_schemas).map_err(|e| {
                    ggen_utils::error::Error::new(&format!("Failed to write schemas: {}", e))
                })?;
                files_generated += 1;
            }
            ...
            if utilities {
                let utils = TypeScriptGenerator::generate_utility_types(&schema).map_err(|e| {
                    ggen_utils::error::Error::new(&format!("Utilities generation failed: {}", e))
                })?;

                let utils_path = format!("{}/utilities.ts", output_dir);
                std::fs::write(&utils_path, utils).map_err(|e| {
                    ggen_utils::error::Error::new(&format!("Failed to write utilities: {}", e))
                })?;
                files_generated += 1;
            }
        }
...
fn init(project_name: String, template: Option<String>) -> VerbResult<InitOutput> {
    ...
    let result = crate::runtime::block_on(async move {
        ...
        std::fs::create_dir_all(&proj_dir).map_err(|e| {
            ggen_utils::error::Error::new(&format!("Failed to create project: {}", e))
        })?;

        // Create subdirectories
        std::fs::create_dir_all(proj_dir.join("ontologies")).map_err(|e| {
            ggen_utils::error::Error::new(&format!("Failed to create ontologies dir: {}", e))
        })?;
        std::fs::create_dir_all(proj_dir.join("src")).map_err(|e| {
            ggen_utils::error::Error::new(&format!("Failed to create src dir: {}", e))
        })?;
        std::fs::create_dir_all(proj_dir.join("generated")).map_err(|e| {
            ggen_utils::error::Error::new(&format!("Failed to create generated dir: {}", e))
        })?;
...
"#,
        ...
        std::fs::write(&pkg_path, package_json).map_err(|e| {
            ggen_utils::error::Error::new(&format!("Failed to write package.json: {}", e))
        })?;
...
"#;
        ...
        std::fs::write(&config_path, config).map_err(|e| {
            ggen_utils::error::Error::new(&format!("Failed to write config: {}", e))
        })?;
        ...
        std::fs::write(&ontology_path, example_ttl).map_err(|e| {
            ggen_utils::error::Error::new(&format!("Failed to write example ontology: {}", e))
        })?;
...
"#,
        ...
        std::fs::write(&readme_path, readme).map_err(|e| {
            ggen_utils::error::Error::new(&format!("Failed to write README: {}", e))
        })?;
