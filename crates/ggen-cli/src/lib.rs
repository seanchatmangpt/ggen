...
use mcpp_utils::error::GgenError;
...
pub async fn cli_match() -> std::result::Result<(), GgenError> {
    // Use clap-noun-verb auto-discovery (handles --version automatically)
    clap_noun_verb::run()
        .map_err(|e| GgenError::msg(format!("CLI execution failed: {}", e)))?;
    Ok(())
}
...
pub async fn run_for_node(args: Vec<String>) -> std::result::Result<RunResult, GgenError> {
    use std::sync::Arc;
    use std::sync::Mutex;
