...
fn test_outdated_binary_warning_triggers() {
    ...
    assert!(current_bin.exists(), "Target binary must exist. Run cargo build first.");

    // 2. Set up a temporary directory
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let cloned_bin_path = temp_dir.path().join(if cfg!(windows) { "ggen.exe" } else { "ggen" });
    ...
    target_file.set_modified(SystemTime::now())
    ...
    assert!(
        ...
        "Warning was not found in stderr: {}", stderr
    );
    assert!(
        stderr.contains("current:"),
        "Current path was not found in stderr: {}", stderr
    );
    assert!(
        stderr.contains("latest:"),
        "Latest path was not found in stderr: {}", stderr
    );
    assert!(
...
fn test_outdated_binary_warning_skips_when_configured() {
    ...
    assert!(
        !stderr.contains("warning: running an outdated 'ggen' binary"),
        "Warning should not be displayed when bypassed: {}", stderr
    );
}
...
fn test_up_to_date_binary_displays_no_warning() {
    ...
    assert!(
        !stderr.contains("warning: running an outdated 'ggen' binary"),
        "Warning should not be displayed for up-to-date binary: {}", stderr
    );
}
