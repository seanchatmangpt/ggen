...
fn test_pack_cache_integration() {
    ...
    // Initialize cache
    let config = ggen_core::marketplace::cache::CacheConfig {
    ...
    let cache =
        ggen_core::marketplace::cache::PackCache::new(config).expect("Failed to initialize cache");
...
fn test_pack_cache_persistence() {
    ...
    {
        let cache1 = ggen_core::marketplace::cache::PackCache::new(config.clone())
