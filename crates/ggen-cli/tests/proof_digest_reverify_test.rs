...
fn test_sync_locked_passes_for_unmodified_pack() {
    ...
    let (_code, combined) = combined_output(world.sync().arg("--locked").arg("--dry-run").arg("true"));
...
fn test_sync_locked_fails_on_digest_mismatch_after_mutation() {
    ...
    let (code, combined) = combined_output(world.sync().arg("--locked").arg("--dry-run").arg("true"));
