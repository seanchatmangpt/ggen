...
fn test_packs_install_with_progress_feedback() {
    ...
    if output.status.success() {
        ...
        assert!(stdout.contains("📋 Installation Plan"), "Should show installation plan");
        assert!(stdout.contains("📦 Installing pack: startup-essentials"), "Should show progress indicators");
        assert!(stdout.contains("✅ Installation completed"), "Should show completion message");

        // Verify installation plan details
        assert!(stdout.contains("Total size:"), "Should show size information");
        assert!(stdout.contains("Dependencies:"), "Should show dependency count");
        assert!(stdout.contains("Estimated time:"), "Should show time estimate");

        println!("✅ Progress feedback test passed");
        println!("Output preview: {}", &stdout[..std::cmp::min(stdout.len(), 200)]);
        ...
        assert!(!stderr.contains("thread 'main' panicked"), "Command should not panic");
    }
}
...
fn test_packs_install_error_handling_improvements() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("Pack ID cannot be empty"), "Should show helpful error message");
...
fn test_packs_install_dry_run_mode() {
    ...
    if output.status.success() {
        ...
        assert!(stdout.contains("📋 Installation Plan"), "Should show installation plan");
        assert!(stdout.contains("Dry run completed"), "Should indicate dry run mode");
        assert!(stdout.contains("Would install"), "Should show what would be installed");

        // Verify no actual installation occurred
        assert!(!stdout.contains("Pack installed"), "Should not show completion");
...
fn test_packs_install_planning_phase() {
    ...
    if output.status.success() {
        ...
        assert!(stdout.contains("Starting installation"), "Should indicate installation start");

        // Verify step details
        assert!(stdout.contains("1. Validate package"), "Should show validation step");
        assert!(stdout.contains("2. Download main package"), "Should show download step");
        assert!(stdout.contains("3. Download dependencies"), "Should show dependency step");
        ...
        let stderr = String::from_utf8_lossy(&output.stderr);
        println!("Planning test failed (may be due to marketplace availability): {}", stderr);
...
fn test_packs_install_performance_indicators() {
    ...
    if output.status.success() {
        ...
        assert!(stdout.contains("Estimated time:"), "Should show estimated time");
        assert!(stdout.contains("Duration:"), "Should show actual duration");

        // Verify duration is reasonable (should be fast for dry run)
        assert!(duration.as_millis() < 30000, "Dry run should complete quickly (< 30s)");
...
fn test_packs_install_cache_status_indicators() {
    ...
    if output.status.success() {
        ...
        assert!(stdout.contains("Some packages cached") || stdout.contains("No cached packages"),
...
fn test_packs_install_step_by_step_progress() {
    ...
    if output.status.success() {
        ...
        assert!(stdout.contains("Starting installation"), "Should show installation start");
        assert!(stdout.contains("Installation completed"), "Should show completion");

        // Verify step details
        assert!(stdout.contains("1. Validate package"), "Should show step 1");
        assert!(stdout.contains("2. Download main package"), "Should show step 2");
        assert!(stdout.contains("3. Download dependencies"), "Should show step 3");
...
fn test_packs_install_error_recovery_suggestions() {
    ...
    assert!(stderr.contains("Pack ID cannot be empty") || stderr.contains("Pack not found"),
...
fn test_packs_install_concurrency_safety() {
    ...
    let output1 = child1.wait_with_output().expect("Failed to wait for first command");
    let output2 = child2.wait_with_output().expect("Failed to wait for second command");
    ...
    if output1.status.success() {
        ...
        assert!(stdout.contains("Installation Plan"), "First command should show planning");
    } else {
    ...
    if output2.status.success() {
        ...
        assert!(stdout.contains("Installation Plan"), "Second command should show planning");
    } else {
    ...
    println!("✅ Concurrency safety test passed ({} succeeded)", success_count);
}
...
fn test_packs_install_memory_efficiency() {
    ...
    assert!(output.status.code().unwrap_or(1) != 137, "Command should not be killed for excessive memory");
