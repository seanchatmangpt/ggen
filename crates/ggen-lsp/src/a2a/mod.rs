...
use crate::a2a_mcp::a2a_generated::adapter::{
...
pub fn dispatch_tool(tool: &str, arguments: &Value) -> Result<Value, AdapterError> {
    ...
    match tool {
        "ggen.lsp.repair_route" => crate::mcp::repair_route_result(args).map_err(to_adapter_err),
        "ggen.lsp.replay_case" => crate::mcp::replay_case_result(args).map_err(to_adapter_err),
        "ggen.lsp.metrics" => crate::mcp::metrics_result(args).map_err(to_adapter_err),
        ...
        if tool == "ggen.lsp.repair_route" {
            if let (Some(root), Some(file), Some(content)) = (
                ...
                crate::capture_request(
                    ...
                    file,
                    content,
                    &crate::Attribution::request(agent, "a2a"),
                );
            }
        }
        Ok(result)
