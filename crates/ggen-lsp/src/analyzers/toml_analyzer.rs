...
/// GGEN-SRC-001: output_file targets a source-caste directory.
/// RenderedSource is source — these path components are forbidden.
pub const GGEN_SRC_001: &str = "GGEN-SRC-001";

/// Forbidden path components that mark a source-caste directory.
const CASTE_DIRS: &[&str] = &[
    "generated",
    "output",
    "outputs",
    "gen",
...
/// Returns the violated caste directory name if `path` contains a forbidden component, else None.
#[must_use]
pub fn source_caste_path_violation(path: &str) -> Option<&'static str> {
    let lower = path.to_ascii_lowercase();
    for seg in lower.split('/') {
        for &caste in CASTE_DIRS {
            if seg == caste {
                return Some(caste);
            }
        }
...
impl TomlAnalyzer {
    ...
    pub fn diagnostics(&self) -> Vec<lsp_max_protocol::MaxDiagnostic> {
        ...
        for (idx, text) in self.source.lines().enumerate() {
            ...
            // GGEN-SRC-001: output_file must not target a source-caste directory.
            ...
            if key == "output_file" && !value.is_empty() {
                if let Some(violated) = source_caste_path_violation(value) {
                    diags.push(diag::max_whole_line(
                        ...
                        Some(GGEN_SRC_001),
                        format!(
                            "GGEN-SRC-001 SECOND_CLASS_PATH: output_file `{value}` targets \
