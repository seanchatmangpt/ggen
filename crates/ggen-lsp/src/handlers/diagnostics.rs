...
pub async fn handle_code_action(server: &GgenLanguageServer, params: CodeActionParams) -> Result<Option<CodeActionResponse>> {
    ...
    for d in &params.context.diagnostics {
        ...
        let has_real_edit = edit.changes.as_ref().is_some_and(|c| {
            c.values().flatten().any(|e| !e.new_text.contains('{'))
        });
        if !has_real_edit {
            continue;
        }
        let title = route.steps.nodes.first()
