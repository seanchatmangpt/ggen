...
pub fn repair_route_captured(
    ...
    if let Some(root) = params.root.as_deref() {
        ...
        crate::capture_request(
            Path::new(root),
            &params.file_path,
            &params.file_content,
            &crate::Attribution::request(agent, "mcp"),
        );
    }
    Ok(result)
}
...
pub fn replay_case_result(
    ...
    let value = match case_id {
        Some(id) => serde_json::to_value(crate::replay_case(Path::new(&root), &id)),
        None => serde_json::to_value(crate::verify_promotion(Path::new(&root))),
    };
...
pub fn metrics_result(
    ...
    let root = validate_root(params.root.unwrap_or_else(|| ".".to_string()))?;
    serde_json::to_value(crate::compute_metrics(Path::new(&root)))
        .map_err(|e| McpError::internal_error(format!("serialization failed: {e}"), None))
}

/// Build the repair-route result for a file.
///
/// Emits the canonical [`crate::RouteEnvelope`] per routed diagnostic (byte-equivalent
/// to the LSP CodeAction `data`, the headless gate, and the A2A bridge) plus the shared
/// [`crate::RouteRefusal`] shape for uncovered diagnostics — so an agent never
...
pub fn build_repair_routes_in(
    ...
    let Some(analyzer) = crate::analyzers::build_analyzer(file_path, file_content) else {
    ...
    let registry = crate::RouteRegistry::seeded()
        .with_pack_routes(&crate::route::default_pack_routes_path(&pack_root));
    let diagnostics = analyzer.diagnostics();
    let mut envelopes = Vec::new();
    let mut refusals = Vec::new();
    for d in &diagnostics {
        match crate::envelope_for_diagnostic(&registry, &d.lsp, file_content, file_path) {
            Some(env) => envelopes.push(env),
            None => refusals.push(crate::RouteRefusal::from_target(
                &crate::route::DiagnosticRef::from_diagnostic(&d.lsp),
            )),
        }
    }
    // Bind the route response to the specific pack it was served from (if any),
    // so a remote agent's result is provenance-anchored.
    let pack_hash = crate::pack_hash_at(&pack_root);
    serde_json::json!({
        "file_path": file_path,
        "is_law_surface": true,
        "pack_hash": pack_hash,
