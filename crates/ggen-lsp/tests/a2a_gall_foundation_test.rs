...
//!   MCP : `ggen_lsp::mcp::build_repair_routes_in(root, path, content)`        (tool surface)
//!   A2A : `ggen_lsp::a2a::dispatch_tool("ggen.lsp.repair_route", {..})`       (bridge)
//!
...
use ggen_lsp::a2a::{agent_card, dispatch_tool, RepairRouteAdapter, TOOLS};
...
fn mcp_route(path: &str, content: &str) -> Option<(String, String)> {
    let v = ggen_lsp::mcp::build_repair_routes_in(Some(&root_str()), path, content);
    envelope_scalars(&v)
}
...
fn gall_mcp_tool_list_contains_repair_replay_metrics() {
    ...
    let args = json!({ "file_path": path, "file_content": content })
        ...
        .cloned();
    assert!(
        ggen_lsp::mcp::repair_route_result(args).is_ok(),
        "repair_route callable"
    );
    assert!(
        ggen_lsp::mcp::metrics_result(Some(
    ...
    assert!(
        ggen_lsp::mcp::replay_case_result(Some(
            json!({ "root": root_str() }).as_object().unwrap().clone()
        ))
        .is_ok(),
...
fn gall_a2a_bridge_matches_mcp_for_repair_route() {
    ...
    .expect("a2a dispatch");
    let mcp = ggen_lsp::mcp::build_repair_routes(&path, &content);
    assert_eq!(
        a2a, mcp,
...
async fn gall_replay_metrics_surface_is_reachable_for_captured_case() {
    ...
    let metrics =
        ggen_lsp::mcp::metrics_result(Some(json!({ "root": root }).as_object().unwrap().clone()))
    ...
    let replay = ggen_lsp::mcp::replay_case_result(Some(
