...
async fn ping_agent_200_returns_healthy() {
    ...
    let result = ggen_core::a2a_registry::ping_agent(&stored, Duration::from_secs(2))
        .await
        .unwrap();
...
async fn ping_agent_timeout_returns_error() {
    ...
    let result = ggen_core::a2a_registry::ping_agent(&stored, Duration::from_millis(50)).await;
