...
fn test_ethos_conformant_gate_e2e() {
    ...
    assert!(ethos_report.passed, "EthosConformant gate should pass. Message: {}", ethos_report.message);
    assert!(ethos_report.message.contains("fitness"), "Should report fitness. Message: {}", ethos_report.message);
    // Alpha discovery on A->B->C and alignment of the same trace should yield 1.0 fitness
    assert!(ethos_report.message.contains("1.00"), "Fitness should be 1.00 for perfectly matching log. Message: {}", ethos_report.message);
    ...
    let ethos_report_fail = reports_fail.iter()
        .find(|r| r.gate_type == ProofGateType::EthosConformant)
        .expect("EthosConformant gate not found");
        
    assert!(!ethos_report_fail.passed, "EthosConformant gate should fail after log deletion");
    assert!(ethos_report_fail.message.contains("FAILED: No runtime audit logs found"), 
            "Should report missing logs. Message: {}", ethos_report_fail.message);
