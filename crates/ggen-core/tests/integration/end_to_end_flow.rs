...
async_test_with_timeout!(test_complete_package_lifecycle, 30, async {
    ...
    let resolved_specific = client.resolve("test-package", Some("26.6.6")).await.unwrap();
    assert_eq!(resolved_specific.version, "26.6.6");
});
...
async_test_with_timeout!(test_package_update_flow, 30, async {
    ...
    // Act & Assert
    let update = client.check_updates("update-test", "26.6.6").await.unwrap();
