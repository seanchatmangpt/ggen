...
async fn test_multiple_registry_instances() -> Result<()> {
    ...
    versions1.insert("1.0.0".to_string(), VersionMetadata {
    ...
    packs1.insert("package-a".to_string(), PackMetadata {
    ...
    versions2.insert("2.0.0".to_string(), VersionMetadata {
    ...
    packs2.insert("package-b".to_string(), PackMetadata {
...
async fn test_registry_failover_simulation() -> Result<()> {
    ...
    packs.insert("shared-package".to_string(), PackMetadata {
    ...
    fs::write(&primary_index_path, serde_json::to_string_pretty(&primary_index)?)?;
    fs::write(&fallback_index_path, serde_json::to_string_pretty(&primary_index)?)?;
