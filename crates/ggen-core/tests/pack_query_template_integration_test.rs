...
use ggen_core::v6::passes::{ExtractionPass, EmissionPass};
...
fn test_pack_query_loading() {
    ...
    assert!(!resolved.queries.is_empty(), "Pack queries should be loaded");
    ...
    assert!(query.sparql.contains("CONSTRUCT"),
...
fn test_pack_template_loading() {
    ...
    assert!(template.content.contains("pub struct"),
...
fn test_emission_pass_extends_with_pack_templates() {
    ...
    assert!(
        rule.inline_template.as_ref().unwrap().contains("pub struct"),
...
fn test_pipeline_with_pack_queries_and_templates() {
    ...
    let has_pack_generated = generated_files
        .iter()
        .any(|path| {
...
fn test_pack_provenance_digest() {
    ...
    assert!(
        ...
        "Digest should use SHA-256"
    );
    assert!(
        digest.len() > "sha256:".len(),
        "Digest should contain hash"
    );
}
...
fn test_pack_query_and_template_names() {
    ...
    assert!(
        query_names.iter().any(|n| n.contains("extract-test-entities")),
