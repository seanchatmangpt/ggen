...
pub fn load_standard_vocabularies(graph: &Graph) -> Result<(), Error> {
    graph.insert_turtle(OWL_TTL)
        .map_err(|e| Error::new(&format!("Failed to load embedded OWL ontology: {}", e)))?;
        
    graph.insert_turtle(RDF_TTL)
        .map_err(|e| Error::new(&format!("Failed to load embedded RDF ontology: {}", e)))?;
        
    graph.insert_turtle(RDFS_TTL)
        .map_err(|e| Error::new(&format!("Failed to load embedded RDFS ontology: {}", e)))?;
        
    graph.insert_turtle(SCHEMA_TTL)
        .map_err(|e| Error::new(&format!("Failed to load embedded schema.org ontology: {}", e)))?;

    graph.insert_turtle(FOAF_TTL)
        .map_err(|e| Error::new(&format!("Failed to load embedded FOAF ontology: {}", e)))?;

    graph.insert_turtle(DUBLIN_CORE_TTL)
        .map_err(|e| Error::new(&format!("Failed to load embedded Dublin Core ontology: {}", e)))?;
...
mod tests {
    ...
    fn test_load_standard_vocabularies() {
        ...
        match results_foaf {
            oxigraph::sparql::QueryResults::Boolean(b) => assert!(b, "foaf:Person not found in graph"),
