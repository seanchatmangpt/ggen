...
impl DependencyInstaller {
    ...
    fn install_cargo_deps(&self, path: &Path) -> Result<()> {
        ...
        mcpp_utils::alert_info!("Installing Cargo dependencies...");
        ...
        if !output.status.success() {
            ...
            mcpp_utils::alert_warning!(&error_msg);
        }
    ...
    fn install_npm_deps(&self, path: &Path) -> Result<()> {
        ...
        mcpp_utils::alert_info!("Installing npm dependencies...");
...
pub async fn create_new_project(config: &ProjectConfig) -> Result<()> {
    ...
    mcpp_utils::alert_success!(&format!("Successfully created project: {}", config.name));
    mcpp_utils::alert_info!(&format!("   Type: {}", config.project_type));
    mcpp_utils::alert_info!(&format!("   Path: {}", project_path.display()));
