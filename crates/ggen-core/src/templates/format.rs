...
//!
//! ```rust,no_run
//! use mcpp_core::templates::format::{FileTreeNode, NodeType, TemplateFormat};
//! use serde_json::json;
//!
...
///
/// ```rust
/// use mcpp_core::templates::format::NodeType;
///
...
///
/// ```rust
/// use mcpp_core::templates::format::FileTreeNode;
///
    ...
    fn validate_nodes(&self, nodes: &[FileTreeNode]) -> Result<()> {
        ...
        for node in nodes {
            if node.name.is_empty() {
                return Err(mcpp_utils::error::Error::new("Node name cannot be empty"));
            }
...
impl FileTreeNode {
    ...
    ///
    /// ```rust
    /// use mcpp_core::templates::format::{FileTreeNode, NodeType};
    ///
    /// let dir = FileTreeNode::directory("src");
