...
///
/// ```rust,no_run
/// use mcpp_core::graph::{Graph, GraphStore};
///
...
impl GraphStore {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let store = Store::open(path.as_ref())
            .map_err(|e| mcpp_utils::error::Error::new(&format!("Failed to open store: {}", e)))?;
        Ok(Self {
            store: Arc::new(store),
        })
    }
    ...
    pub fn new() -> Result<Self> {
        ...
        let store = Store::new().map_err(|e| {
            mcpp_utils::error::Error::new(&format!("Failed to create store: {}", e))
        })?;
        Ok(Self {
            store: Arc::new(store),
        })
