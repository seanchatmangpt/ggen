...
///
/// ```no_run
/// use mcpp_core::poka_yoke::{DryRunMode, Operation, ValidatedPath};
///
...
impl DryRunMode {
    ...
    pub fn confirm(&self) -> Result<bool> {
        ...
        std::io::stdout().flush().map_err(|e| {
            mcpp_utils::error::Error::io_error(format!("Failed to flush stdout: {}", e))
        })?;

        let mut input = String::new();
        std::io::stdin().read_line(&mut input).map_err(|e| {
            mcpp_utils::error::Error::io_error(format!("Failed to read stdin: {}", e))
        })?;
    ...
    pub fn execute(&mut self) -> Result<()> {
        if self.executed {
            return Err(mcpp_utils::error::Error::new("Operations already executed"));
        }
        ...
        match op {
            Operation::FileCreate { path, .. } | Operation::FileWrite { path, .. } => {
                ...
                if let Some(parent) = path.as_path().parent() {
                    if !parent.exists() {
                        return Err(mcpp_utils::error::Error::invalid_input(format!(
        ...
        match op {
            Operation::FileCreate { path, .. } | Operation::FileWrite { path, .. } => {
                std::fs::write(path.as_path(), b"").map_err(|e| {
                    mcpp_utils::error::Error::io_error(format!("Failed to write file: {}", e))
                })?;
            }
            Operation::FileDelete { path } => {
                std::fs::remove_file(path.as_path()).map_err(|e| {
                    mcpp_utils::error::Error::io_error(format!("Failed to delete file: {}", e))
                })?;
            }
            Operation::CommandExec { command, args } => {
                let status = std::process::Command::new(command)
                    ...
                    .status()
                    .map_err(|e| {
                        mcpp_utils::error::Error::new(&format!("Failed to execute command: {}", e))
                    })?;
            ...
            Operation::DirCreate { path } => {
                std::fs::create_dir_all(path.as_path()).map_err(|e| {
                    mcpp_utils::error::Error::io_error(format!("Failed to create directory: {}", e))
                })?;
            }
            Operation::DirDelete { path } => {
                std::fs::remove_dir_all(path.as_path()).map_err(|e| {
                    mcpp_utils::error::Error::io_error(format!("Failed to delete directory: {}", e))
                })?;
            }
        }
        Ok(())
