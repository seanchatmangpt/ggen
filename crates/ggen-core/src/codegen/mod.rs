...
//!
//! ```rust,no_run
//! use mcpp_core::codegen::{GeneratorLanguage, generate_service};
//!
...
///
/// This enum covers all first-class generators in mcpp. For Docker/Kubernetes manifests,
...
pub fn generate_service(
    ...
    match language {
        ...
        GeneratorLanguage::Rust => {
            ...
            Ok(format!(
                "// Generated by mcpp — Rust service: {name}\n//\n// Default port: {port}\n\npub struct {name};\n\nimpl {name} {{\n    pub fn port() -> u16 {{ {port} }}\n}}\n",
                name = service_name,
                port = port,
            ))
        }
        GeneratorLanguage::TypeScript => {
            ...
            Ok(format!(
                "// Generated by mcpp — TypeScript service: {name}\n\
                 // Default port: {port}\n\
                 \n\
