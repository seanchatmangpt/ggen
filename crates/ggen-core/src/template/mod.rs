...
) -> mcpp_utils::error::Result<template_validation::TemplateValidationResult> {
...
pub fn extract_variables_from_sparql_results(results_path: &str) -> Result<HashSet<String>> {
    let content = std::fs::read_to_string(results_path).map_err(|e| {
        mcpp_utils::error::Error::new(&format!("Failed to read SPARQL results: {}", e))
    })?;
    let json: serde_json::Value = serde_json::from_str(&content).map_err(|e| {
        mcpp_utils::error::Error::new(&format!("Failed to parse SPARQL results JSON: {}", e))
    })?;
