...
//!
//! Implements the five-stage pipeline that drives `mcpp sync`:
//!
//! ```text
...
//!
//! ```rust,no_run
//! use mcpp_core::sync::{SyncConfig, SyncLanguage, sync};
...
/// Configuration for a single `mcpp sync` invocation.
#[derive(Debug, Clone)]
...
// Allow converting mcpp_utils errors so we can use ? in internal helpers.
...
fn elixir_genserver_template(module_name: &str) -> String {
  ...
  @moduledoc "Generated by mcpp sync"
  use GenServer
