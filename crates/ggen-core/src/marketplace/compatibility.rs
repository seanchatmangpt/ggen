...
impl CompatibilityChecker {
    ...
    fn check_validators(&self, packs: &[AtomicPackId]) -> Result<Vec<Conflict>> {
        use crate::marketplace::atomic::AtomicPackCategory;
        use crate::marketplace::ownership::{OwnershipClass, OwnershipTarget};
    ...
    fn load_ownership_from_toml(path: &std::path::Path) -> Result<Vec<OwnershipDeclaration>> {
        use crate::marketplace::ownership::{MergeStrategy, OwnershipClass};
        use std::io::Read;
    ...
    fn check_policies(&self, packs: &[AtomicPackId]) -> Result<Vec<Conflict>> {
        use crate::marketplace::atomic::{AtomicPackCategory, AtomicPackClass};
    ...
    fn load_receipt_schema_metadata(
        ...
        for receipt_pack in receipt_packs {
            ...
            let default_meta = match receipt_pack.class {
                crate::marketplace::atomic::AtomicPackClass::ReceiptChained => ReceiptSchemaMeta {
                    schema_version: "1".to_string(),
                    signature_algorithm: None,
                    hash_function: Some("sha256".to_string()),
                    chain_depth_max: None,
...
mod tests {
    ...
    fn test_compatibility_report_success() {
        let packs = vec![
            AtomicPackId::new(
                crate::marketplace::atomic::AtomicPackClass::CoreOntology,
                "pack1".to_string(),
            ),
            AtomicPackId::new(
                crate::marketplace::atomic::AtomicPackClass::CoreOntology,
    ...
    fn test_check_capabilities_no_conflicts() {
        use crate::marketplace::atomic::AtomicPackClass;
