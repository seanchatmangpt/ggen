...
impl CliLayerGenerator {
    ...
    pub fn generate(&self, project: &CliProject, output_dir: &Path) -> Result<()> {
        let cli_crate = project.cli_crate.as_ref().unwrap();
        ...
        std::fs::create_dir_all(&cli_src)
            .context("Failed to create CLI src directory")?;
        ...
        context.insert("nouns", &project.nouns.iter().map(|n| &n.name).collect::<Vec<_>>());
    ...
    fn generate_noun(
        ...
        context.insert("verbs", &noun.verbs.iter().map(|v| &v.name).collect::<Vec<_>>());
    ...
    fn generate_verb(
        ...
        let domain_function = verb.domain_function.as_ref()
            .cloned()
            .unwrap_or_else(|| {
                // Default: core_crate::noun::verb
                let core_crate = project.domain_crate.as_ref().unwrap().replace("-", "_");
