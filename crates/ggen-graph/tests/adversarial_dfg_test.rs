...
fn ev(id: &str, act: &str, secs: i64, case: &str) -> OcelEvent {
    OcelEvent {
        id: id.into(), activity: act.into(),
        timestamp: Utc.timestamp_opt(secs, 0).single().unwrap(),
        objects: vec![OcelObjectRef { id: case.into(), r#type: "diagnostic_code".into(), qualifier: Some("diag".into()) }],
...
fn distinct_cases_do_not_create_cross_edges() {
    ...
    let log = OcelLog { objects: vec![], events: vec![
        ev("1","A",10,"c1"), ev("2","X",15,"c2"), ev("3","B",20,"c1"), ev("4","Y",25,"c2"),
    ]};
    let g = DeterministicGraph::new().unwrap();
    EvidenceProjector::project_ocel(&g, &log).unwrap();
    let edges = discover_dfg(&g, Q).unwrap();
    assert!(edges.iter().any(|e| e.source=="A"&&e.target=="B"), "A->B within c1");
    assert!(edges.iter().any(|e| e.source=="X"&&e.target=="Y"), "X->Y within c2");
    // The critical assertion: no edge crosses cases despite time interleaving.
    assert!(!edges.iter().any(|e| (e.source=="A"&&e.target=="X")||(e.source=="X"&&e.target=="B")),
        "cross-case edges must not exist, got {edges:?}");
}

#[test]
fn single_event_has_no_edges() {
    let log = OcelLog { objects: vec![], events: vec![ev("1","Solo",10,"c1")] };
    let g = DeterministicGraph::new().unwrap();
    EvidenceProjector::project_ocel(&g, &log).unwrap();
    assert!(discover_dfg(&g, Q).unwrap().is_empty(), "one event => no DFG edges");
}
...
fn equal_timestamps_produce_no_direct_follow() {
    ...
    let log = OcelLog { objects: vec![], events: vec![ev("1","A",10,"c1"), ev("2","B",10,"c1")] };
    let g = DeterministicGraph::new().unwrap();
    EvidenceProjector::project_ocel(&g, &log).unwrap();
    let edges = discover_dfg(&g, Q).unwrap();
    assert!(!edges.iter().any(|e| e.source=="A"&&e.target=="B"), "equal ts => no strict follow");
}
...
fn lifecycle_requires_all_three_in_order() {
    ...
    let log = OcelLog { objects: vec![], events: vec![
        ev("1","DiagnosticRaised",10,"E1"), ev("3","GatePassed",30,"E1"),
    ]};
    let g = DeterministicGraph::new().unwrap();
    EvidenceProjector::project_ocel(&g, &log).unwrap();
    let ok = check_lifecycle_order(&g, Q, &["DiagnosticRaised","RepairApplied","GatePassed"]).unwrap();
