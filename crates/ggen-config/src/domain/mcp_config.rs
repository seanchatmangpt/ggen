...
//!
//! Ported from `ggen-core/src/domain/mcp_config.rs` (specs/014-ggen-core-replacement,
//! docs/jira/v26.7.16/12-OPEN-QUESTIONS.md item 2). `crate::utils::error::Error`
//! (`ggen_core`'s ad-hoc string-message type, out of this port's scope) rewritten to this
//! crate's own `crate::config_lib::{ConfigError, Result}`, by call-site meaning: every original
//! `GgenError::invalid_input(...)` call (file/JSON/TOML read-write failures, all free-form
//! messages with no field name) → `ConfigError::Validation(...)`; the one
//! `GgenError::feature_not_enabled(...)` call (Windows process-uptime stub) →
//! `ConfigError::Other(...)`, preserving the original message text verbatim (same "wrap the
//! original text, change the variant" technique used in `domain::error`, this crate's sibling
//! port of the same ticket item).

use crate::config_lib::{ConfigError, Result};
...
impl McpServerConfig {
    ...
    ///
    /// # Errors
    /// Returns `Err(McpValidationError)` when the command is empty, the timeout is zero, or
    /// the command matches a known-dangerous pattern (e.g. `rm -rf`).
    pub fn validate(&self) -> std::result::Result<(), McpValidationError> {
...
impl A2aConfig {
    ...
    ///
    /// # Errors
    /// Returns `Err(A2aValidationError)` when the port or timeout is zero, or TLS is enabled
    /// without both a certificate and a key path.
    pub fn validate(&self) -> std::result::Result<(), A2aValidationError> {
...
///
/// # Errors
/// Returns `Err(ConfigError)` if any discovered config file exists but fails to read or parse.
pub fn load_config(
...
///
/// # Errors
/// Returns `Err(ConfigError::Validation)` if the file cannot be read or its JSON cannot be parsed.
pub fn load_mcp_from_file(path: &Path) -> Result<McpConfigFile> {
...
///
/// # Errors
/// Returns `Err(ConfigError::Validation)` if the file cannot be read or its TOML cannot be parsed.
pub fn load_a2a_from_file(path: &Path) -> Result<A2aConfig> {
    let content = fs::read_to_string(path).map_err(|e| {
        ConfigError::Validation(format!(
...
///
/// # Errors
/// Returns `Err(ConfigError)` only via [`load_mcp_from_file`] when `GGEN_MCP_CONFIG` points at
/// an existing-but-unparseable file.
pub fn load_mcp_from_env() -> Result<Option<McpConfigFile>> {
...
///
/// # Errors
/// Returns `Err(ConfigError)` only via [`load_a2a_from_file`] when `GGEN_A2A_CONFIG` points at
/// an existing-but-unparseable file.
pub fn load_a2a_from_env() -> Result<Option<A2aConfig>> {
...
///
/// # Errors
/// Returns `Err(ConfigError)` if the parent directory cannot be created or the config cannot
/// be serialized/written.
pub fn init_mcp_config(path: &Path, include_examples: bool) -> Result<McpConfigFile> {
...
/// Returns `Err(ConfigError)` if the parent directory cannot be created or the config cannot
/// be serialized/written.
pub fn init_a2a_config(path: &Path) -> Result<A2aConfig> {
...
///
/// # Errors
/// Returns `Err(ConfigError)` if the parent directory cannot be created, the config cannot be
/// serialized to JSON, or the file cannot be written.
pub fn write_mcp_config(path: &Path, config: &McpConfigFile) -> Result<()> {
...
pub fn write_a2a_config(path: &Path, config: &A2aConfig) -> Result<()> {
    // Create parent directory if needed
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|e| {
            ConfigError::Validation(format!(
...
///
/// # Errors
/// This function itself is infallible in practice (`Result` is used only for signature
/// consistency with the rest of this module) — every per-server validation failure is recorded
/// in the returned `Vec<ValidationResult>`, not propagated as `Err`.
pub fn validate_mcp_config(config: &McpConfigFile) -> Result<Vec<ValidationResult>> {
...
///
/// # Errors
/// Returns `Err(ConfigError)` if a PID file exists but cannot be read or its content is not a
/// valid PID.
pub fn get_server_status(project_dir: Option<&Path>) -> Result<ServerStatus> {
...
fn get_process_uptime(pid: u32) -> Result<u64> {
    ...
    let output = Command::new("ps")
        ...
        .output()
        .map_err(|e| ConfigError::Validation(format!("Failed to query process: {}", e)))?;
...
fn get_process_uptime(_pid: u32) -> Result<u64> {
    ...
    Err(ConfigError::Other(
        "Feature 'Process uptime not available on Windows' not enabled. ".to_string(),
    ))
}
...
///
/// # Errors
/// Returns `Err(ConfigError)` if the server's PID cannot be determined or the process cannot
/// be terminated.
pub fn stop_server(project_dir: Option<&Path>, force: bool) -> Result<bool> {
    ...
    let pid = status.pid.ok_or_else(|| {
        ConfigError::Validation("Server is running but PID is unknown".to_string())
    })?;
...
///
/// # Errors
/// Returns `Err(ConfigError)` if the parent directory cannot be created or the PID file cannot
/// be written.
pub fn write_pid_file(project_dir: Option<&Path>, pid: u32) -> Result<()> {
    let pid_file = project_dir
        .unwrap_or_else(|| Path::new("."))
        .join(MCP_SERVER_PID_FILE);
