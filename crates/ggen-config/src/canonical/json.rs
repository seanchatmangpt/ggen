...
//!
//! Ported from `ggen-core::canonical::json` during the ggen-core retirement migration
//! (see `specs/014-ggen-core-replacement/tasks.md`). Logic is unchanged from the original —
//! only the `use` path for the supporting `Canonical`/`CanonicalError`/`Canonicalizer`/`Result`
//! types moved from `crate::canonical` (ggen-core) to `crate::canonical` (this crate,
//! ggen-config).
//!
