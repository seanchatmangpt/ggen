...
fn test_extract_class_basic_info() {
    ...
    let fixture_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/simple_bond.ttl");
...
fn test_extract_class_with_label() {
    ...
    // Act
    let bond_class = extractor
        .extract_class("http://example.com/test#Bond")
        .unwrap();
...
fn test_extract_empty_class() {
    ...
    // Act
    let empty_class = extractor
        .extract_class("http://example.com/test#EmptyClass")
        .unwrap();
...
fn test_extract_datatype_property() {
    ...
    assert!(
        bond_class.properties.len() >= 1,
        "Should have at least one property"
    );

    let has_isin = bond_class.properties.iter().find(|p| match p {
        OWLProperty::DatatypeProperty { uri, .. } => uri.as_str().contains("hasISIN"),
        _ => false,
    });
...
fn test_extract_multiple_datatype_properties() {
    ...
    let datatype_props: Vec<_> = bond_class
        .properties
        .iter()
        .filter(|p| matches!(p, OWLProperty::DatatypeProperty { .. }))
        .collect();

    assert!(
        datatype_props.len() >= 4,
        "Should have at least 4 datatype properties"
    );
}
...
fn test_extract_object_property() {
    ...
    let has_issuer = bond_class.properties.iter().find(|p| match p {
        OWLProperty::ObjectProperty { uri, .. } => uri.as_str().contains("hasIssuer"),
        _ => false,
    });

    assert!(
        has_issuer.is_some(),
        "Should find hasIssuer object property"
    );
        ...
        match prop {
            OWLProperty::DatatypeProperty { uri, range, .. } => {
                if uri.as_str().contains("hasMaturityDate") {
                    assert!(
                        range.as_str().contains("date"),
                        "Maturity date should be xsd:date"
                    );
                } else if uri.as_str().contains("hasCouponRate")
                    || uri.as_str().contains("hasParValue")
                {
                    assert!(
                        range.as_str().contains("decimal"),
                        "Numeric fields should be xsd:decimal"
                    );
                } else if uri.as_str().contains("ISIN") {
                    assert!(
                        range.as_str().contains("string"),
                        "ISIN should be xsd:string"
                    );
                }
            }
            _ => {}
...
fn test_extract_exact_cardinality_restriction() {
    ...
    // Act
    let bond_class = extractor
        .extract_class("http://example.com/test#BondWithRestrictions")
        .unwrap();

    // Assert: Has exact cardinality restriction on hasISIN
    let isin_restriction = bond_class.restrictions.iter().find(|r| match r {
        OWLRestriction::Cardinality { property, .. } => property.as_str().contains("hasISIN"),
        _ => false,
    });

    assert!(
        isin_restriction.is_some(),
        "Should find cardinality restriction on hasISIN"
    );
...
fn test_extract_min_cardinality_restriction() {
    ...
    let maturity_restriction = bond_class.restrictions.iter().find(|r| match r {
    ...
    assert!(
        maturity_restriction.is_some(),
        "Should find cardinality restriction on hasMaturityDate"
    );
...
fn test_extract_max_cardinality_restriction() {
    ...
    let coupon_restriction = bond_class.restrictions.iter().find(|r| match r {
        OWLRestriction::Cardinality { property, .. } => property.as_str().contains("hasCouponRate"),
        _ => false,
    });

    assert!(
        coupon_restriction.is_some(),
        "Should find cardinality restriction on hasCouponRate"
    );
...
fn test_multiple_cardinality_restrictions() {
    ...
    let cardinality_restrictions: Vec<_> = bond_class
        .restrictions
        .iter()
        .filter(|r| matches!(r, OWLRestriction::Cardinality { .. }))
        .collect();

    assert_eq!(
        cardinality_restrictions.len(),
        3,
        "Should have 3 cardinality restrictions"
    );
}
...
fn test_cardinality_property_references() {
    ...
    for restriction in &bond_class.restrictions {
        match restriction {
            OWLRestriction::Cardinality { property, .. } => {
                assert!(
                    !property.as_str().is_empty(),
                    "Property URI should not be empty"
                );
                assert!(
...
fn test_extract_pattern_restriction() {
    ...
    // Act
    let bond_class = extractor
        .extract_class("http://example.com/test#BondWithDatatypeRestrictions")
        .unwrap();

    // Assert: Has pattern restriction on hasISIN
    let datatype_restriction = bond_class
        .restrictions
        .iter()
        .find(|r| matches!(r, OWLRestriction::DatatypeRestriction { .. }));

    assert!(
        datatype_restriction.is_some(),
        "Should find datatype restriction"
    );

    match datatype_restriction.unwrap() {
        OWLRestriction::DatatypeRestriction { facets, .. } => {
            let has_pattern = facets
                .iter()
                .any(|f| matches!(f, DatatypeFacet::Pattern(_)));
            ...
            for facet in facets {
                if let DatatypeFacet::Pattern(pattern) = facet {
                    assert_eq!(
                        ...
                        "Pattern should match ISIN format"
                    );
                }
            }
        }
...
fn test_extract_length_restrictions() {
    ...
    match datatype_restriction.unwrap() {
        OWLRestriction::DatatypeRestriction { facets, .. } => {
            let has_min_length = facets
                .iter()
                .any(|f| matches!(f, DatatypeFacet::MinLength(12)));
            let has_max_length = facets
                .iter()
                .any(|f| matches!(f, DatatypeFacet::MaxLength(12)));
...
fn test_extract_numeric_range_restrictions() {
    ...
    // Act
    let bond_class = extractor
        .extract_class("http://example.com/test#BondWithNumericRestrictions")
        .unwrap();
    ...
    assert!(
        datatype_restriction.is_some(),
        "Should find numeric restriction"
    );
    ...
    match datatype_restriction.unwrap() {
        OWLRestriction::DatatypeRestriction { facets, .. } => {
            let has_min = facets
                .iter()
                .any(|f| matches!(f, DatatypeFacet::MinInclusive(0.0)));
            let has_max = facets
                .iter()
                .any(|f| matches!(f, DatatypeFacet::MaxInclusive(100.0)));
...
fn test_datatype_restriction_base_type() {
    ...
    match datatype_restriction.unwrap() {
        OWLRestriction::DatatypeRestriction { base_type, .. } => {
            assert!(
                base_type.as_str().contains("string"),
                "Base type should be xsd:string"
            );
        }
        _ => panic!("Expected DatatypeRestriction"),
    }
...
fn test_multiple_facets_in_restriction() {
    ...
    match datatype_restriction.unwrap() {
        OWLRestriction::DatatypeRestriction { facets, .. } => {
            assert!(
                facets.len() >= 3,
                "Should have at least 3 facets (pattern, minLength, maxLength)"
            );
        }
        _ => panic!("Expected DatatypeRestriction"),
    }
...
fn test_extract_all_values_from_restriction() {
    ...
    // Act
    let bond_class = extractor
        .extract_class("http://example.com/test#BondWithValueRestrictions")
        .unwrap();

    // Assert: Has allValuesFrom restriction
    let value_restriction = bond_class.restrictions.iter().find(|r| match r {
    ...
    assert!(
        value_restriction.is_some(),
        "Should find allValuesFrom restriction"
    );

    match value_restriction.unwrap() {
        OWLRestriction::ValueRestriction { value_type, .. } => match value_type {
            ValueRestrictionType::AllValuesFrom(class) => {
                assert!(
                    class.as_str().contains("Issuer"),
                    "Should reference Issuer class"
                );
            }
...
fn test_extract_some_values_from_restriction() {
    ...
    assert!(
        value_restriction.is_some(),
        "Should find someValuesFrom restriction"
    );
}
...
fn test_mixed_restriction_types() {
    ...
    let has_value_restrictions = bond_class
        .restrictions
        .iter()
        .any(|r| matches!(r, OWLRestriction::ValueRestriction { .. }));

    assert!(
        has_value_restrictions,
        "Should have at least one value restriction"
    );
}
...
fn test_extract_missing_class_returns_error() {
    ...
    assert!(
        result.is_err(),
        "Should return error for non-existent class"
    );
}
