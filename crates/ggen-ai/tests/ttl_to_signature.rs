...
fn test_basic_shape_extraction_finds_all_properties() {
    ...
    let classes = transpiler
        .find_classes_with_shapes(&store)
    ...
    assert_eq!(
        classes.len(),
        1,
        "Should find exactly one class with SHACL shape"
    );
    assert!(
...
fn test_basic_shape_extracts_field_names_correctly() {
    ...
    let properties = transpiler
        .find_property_shapes(&classes[0], &store)
...
fn test_parse_string_length_constraints() {
    ...
    let username_prop = properties
        .iter()
        .find(|p| p.path.contains("username"))
        .expect("Should find username property");

    assert_eq!(
        username_prop.path, "username",
        "Should extract correct property name"
    );
    assert!(
...
fn test_parse_datatype_constraints() {
    ...
    let signatures = transpiler
        .build_signatures(&store)
    ...
    let input_types: Vec<_> = sig.inputs.iter().map(|f| f.type_annotation()).collect();

    // Should have String, i32, bool, f32, f32, i32, f32 types
    assert!(
        !input_types.is_empty(),
        "Should have input fields with type annotations"
    );
}
...
fn test_identify_output_fields_with_explicit_marker() {
    ...
    assert!(
        !signatures.is_empty(),
        "Should create at least one signature"
    );
    let sig = &signatures[0];
    ...
    let output_names: Vec<_> = sig.outputs.iter().map(|f| f.name()).collect();

    // Should have output_result and processing_time
    assert!(
        output_names
            .iter()
            .any(|&n| n.contains("output_result") || n.contains("result")),
...
fn test_default_unmarked_fields_as_inputs() {
    ...
    let input_names: Vec<_> = sig.inputs.iter().map(|f| f.name()).collect();

    assert!(
        input_names
            .iter()
            .any(|&n| n.contains("input_text") || n.contains("text")),
...
fn test_signature_always_has_output_field() {
    ...
    assert!(!signatures.is_empty(), "Should create signature");
    assert!(
        !signatures[0].outputs.is_empty(),
        "Should have at least one output field"
    );
}
...
fn test_hyphenated_property_names_converted_to_snake_case() {
    ...
    for name in &input_names {
        assert!(
            !name.contains("-"),
            "Field '{}' should not contain hyphens",
            name
        );
    }
}
...
fn test_reserved_name_collision_detection() {
    ...
    assert_eq!(
        result1, "custom_class",
        "Should prefix 'class' with custom_"
    );
...
fn test_multiple_classes_in_single_ttl_file() {
    ...
    assert_eq!(
        classes.len(),
        3,
        "Should find three classes with SHACL shapes"
    );

    let class_strings = classes.iter().map(|c| c.to_string()).collect::<Vec<_>>();
...
fn test_signature_count_tracking() {
    ...
    assert_eq!(
        signatures.len(),
        count,
        "Signature count should match array length"
    );
}
...
fn test_skip_orphan_shapes_without_targetclass() {
    ...
    assert_eq!(
        classes.len(),
        0,
        "Should not find orphan shape without sh:targetClass"
    );
}
...
fn test_track_signature_generation_count() {
    ...
    let store = load_ttl_fixture("simple_shape.ttl");
    let _signatures = transpiler
        .build_signatures(&store)
        .expect("Failed to build signatures");

    // Assert: Count should be updated
    assert_eq!(
        transpiler.signature_count(),
        1,
        "Should increment count after generation"
    );
}
...
fn test_field_collision_tracking_reset_between_signatures() {
    ...
    for sig in signatures {
        ...
        assert_eq!(
            ...
            "Signature '{}' should have no duplicate field names",
            sig.name
        );
    }
}
...
fn test_full_pipeline_ttl_to_signature() {
    ...
    for input in &sig.inputs {
        ...
        assert!(
            !input.desc().is_empty(),
            "Input field should have description"
        );
        assert!(
            !input.type_annotation().is_empty(),
            "Input field should have type"
        );
    }
    ...
    for output in &sig.outputs {
        ...
        assert!(
            !output.desc().is_empty(),
            "Output field should have description"
        );
        assert!(
            !output.type_annotation().is_empty(),
            "Output field should have type"
        );
    }
}
...
fn test_all_rdf_properties_become_signature_fields() {
    ...
    assert!(
        total_fields >= 3,
        "Should have at least 3 fields (3 from TTL + 1 default output)"
    );
}
...
fn test_local_name_extraction_from_hash_iri() {
    ...
    assert_eq!(
        local_name, "MyClass",
        "Should extract local name after hash"
    );
}
...
fn test_local_name_extraction_from_slash_iri() {
    ...
    assert_eq!(
        local_name, "MyClass",
        "Should extract local name after slash"
    );
}
...
fn test_snake_case_handles_empty_string() {
    ...
    assert_eq!(
        ...
        "Empty string should default to unnamed_field"
    );
}
