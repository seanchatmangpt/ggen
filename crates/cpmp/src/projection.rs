...
pub fn generate_reports(out: &std::path::Path) -> Result<()> {
    ...
    std::fs::write(out.join("reports/PATTERN_ATLAS.md"), "# Pattern Atlas\n\nGenerated by cpmp.")?;
    std::fs::write(out.join("reports/NON_DELETION_RECEIPT.toml"), "status = \"VERIFIED\"")?;
    Ok(())
}

pub fn generate_rdf_fallback(files: &[FileEntry], caps: &[DetectedCapability], receipt: &Receipt, out: &std::path::Path) -> Result<()> {
    ...
    for f in files {
        ttl.push_str(&format!("<file://{}> a spdx:File ;\n  prov:wasGeneratedBy <urn:cpmp:scan:{}> .\n\n", f.path, receipt.id));
    }
    
    for c in caps {
        ttl.push_str(&format!("<urn:cpmp:capability:{}> a skos:Concept .\n", c.capability.replace(" ", "_")));
        ttl.push_str(&format!("<file://{}> cpmp:implementsCapability <urn:cpmp:capability:{}> .\n\n", c.file_path, c.capability.replace(" ", "_")));
    }
    
    std::fs::write(out.join("catalog/cpmp-catalog.ttl"), ttl)?;
    std::fs::write(out.join("catalog/cpmp-shapes.ttl"), "# SHACL Shapes for CPMP\n")?;
    std::fs::write(out.join("catalog/cpmp-catalog.nq"), "# N-Quads representation\n")?;
