...
pub fn insert_catalog(conn: &mut Connection, files: &[FileEntry], symbols: &[Symbol], caps: &[DetectedCapability]) -> Result<()> {
    let tx = conn.transaction()?;
    { let mut stmt = tx.prepare("INSERT OR REPLACE INTO files (path, hash, size_bytes, language, git_root, is_test, is_binary) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)")?; for f in files { stmt.execute((&f.path, &f.hash, f.size_bytes as i64, format!("{:?}", f.language), &f.git_root, f.is_test, f.is_binary))?; } }
    { let mut stmt = tx.prepare("INSERT INTO symbols (file_path, name, kind, line) VALUES (?1, ?2, ?3, ?4)")?; for s in symbols { stmt.execute((&s.file_path, &s.name, &s.kind, s.line as i64))?; } }
    { let mut stmt = tx.prepare("INSERT INTO capabilities (file_path, capability, matched_term, confidence, evidence_type, classification, line_number) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)")?; for c in caps { stmt.execute((&c.file_path, &c.capability, &c.matched_term, c.confidence, &c.evidence_type, &c.classification, c.line_number))?; } }
    tx.commit()?;
    Ok(())
}
