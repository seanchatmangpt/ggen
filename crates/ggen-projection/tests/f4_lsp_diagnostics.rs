...
fn test_f4_t2_wasm4pm_replay_deviation() {
    ...
    // Providing generic event_type without all checkpoints triggers DEVIATION from adapter
    client.did_open("file:///virtual/project/events.jsonl", "json", "{\"events\": [{\"id\": \"evt_1\", \"time\": \"2026-06-06T18:00:00Z\", \"event_type\": \"BoundaryDeclared\"}]}").unwrap();
    ...
    assert!(diags.iter().any(|d| d["code"] == "WASM4PM-VERDICT-DEVIATION"), "got {:?}", diags);
    shutdown_client(client);
}
...
fn test_f4_t2_wasm4pm_digest_chain_broken() {
    ...
    client.did_open("file:///virtual/project/events.jsonl", "json", "{\"events\": [{\"id\": \"evt_1\", \"time\": \"2026-06-06T18:00:00Z\", \"event_type\": \"checkpoint.admitted\", \"attributes\": [{\"name\": \"previous_receipt\", \"value\": \"tampered_uuid\"}]}]}").unwrap();
    let res = client.wait_for_notification_timeout("textDocument/publishDiagnostics", std::time::Duration::from_millis(2000));
    assert!(res.is_ok());
    let diags = res.unwrap()["params"]["diagnostics"].as_array().unwrap().clone();
    assert!(diags.iter().any(|d| d["code"] == "WASM4PM-VERDICT-BLOCKED"), "got {:?}", diags);
    shutdown_client(client);
}
...
fn test_f4_t2_wasm4pm_verdict_fit() {
    ...
    client.did_open(
        ...
        "json",
        "{\"events\": [{\"id\": \"evt_1\", \"time\": \"2026-06-06T18:00:00Z\", \"type\": \"checkpoint.admitted\", \"relationships\": [{\"objectId\": \"GALL-CHECKPOINT-001\"}, {\"objectId\": \"GALL-CHECKPOINT-002\"}, {\"objectId\": \"GALL-CHECKPOINT-003\"}, {\"objectId\": \"GALL-CHECKPOINT-004\"}]}]}"
    ).unwrap();
    let res = client.wait_for_notification_timeout("textDocument/publishDiagnostics", std::time::Duration::from_millis(2000));
    assert!(res.is_ok());
    let diags = res.unwrap()["params"]["diagnostics"].as_array().unwrap().clone();
