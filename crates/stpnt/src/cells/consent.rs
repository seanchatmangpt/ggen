...
mod tests {
    ...
    async fn test_consent_gate_refusal_law() {
        ...
        let (output_bytes_admitted, packet_admitted) = part.execute(input_bytes_admitted).await.unwrap();
        let output_admitted: ConsentOutput = serde_json::from_slice(&output_bytes_admitted).unwrap();
