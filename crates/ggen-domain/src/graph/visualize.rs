...
pub async fn visualize_graph(
    ...
    let graph = if graph_path.exists() {
        Graph::load_from_file(graph_path)
            .map_err(|e| ggen_core::utils::error::Error::new(&format!("Failed to load graph: {}", e)))?
    } else {
    ...
    if let Some(ref path) = output_path {
        fs::write(path, content).map_err(|e| {
            ggen_core::utils::error::Error::new(&format!("Failed to write visualization: {}", e))
        })?;
    }
...
fn extract_nodes(graph: &Graph, options: &VisualizeOptions) -> Result<Vec<(String, String)>> {
    ...
    let results = graph
        .query(&query)
        .map_err(|e| ggen_core::utils::error::Error::new(&format!("Failed to query graph: {}", e)))?;

    if let oxigraph::sparql::QueryResults::Solutions(solutions) = results {
        for solution in solutions {
            let solution = solution.map_err(|e| {
                ggen_core::utils::error::Error::new(&format!("Query solution error: {}", e))
            })?;
...
fn extract_label(graph: &Graph, node_id: &str) -> Result<String> {
    ...
    let results = graph
        .query(&query)
        .map_err(|e| ggen_core::utils::error::Error::new(&format!("Failed to query label: {}", e)))?;
...
pub fn generate_json(
    ...
    serde_json::to_string_pretty(&graph)
        .map_err(|e| ggen_core::utils::error::Error::new(&format!("JSON serialization failed: {}", e)))
}
...
pub async fn run(args: &VisualizeInput) -> Result<()> {
    ...
    ggen_core::utils::alert_info!("   Format: {}", output.format);
