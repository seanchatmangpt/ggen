...
pub fn run(args: &LintInput) -> Result<()> {
    ...
    ggen_core::utils::alert_info!("📋 Linting template: {}", output.template_path);

    if output.errors_found > 0 {
        let msg = format!("\nErrors found: {}", output.errors_found);
        ggen_core::utils::alert_critical!(&msg);
        for error in &output.report.errors {
            if let Some(line) = error.line {
                ggen_core::utils::alert_info!("  Line {}: {}", line, error.message);
            } else {
                ggen_core::utils::alert_info!("  {}", error.message);
            }
        }
    }
    ...
    if output.warnings_found > 0 {
        ...
        ggen_core::utils::alert_warning!(&msg);
        for warning in &output.report.warnings {
            if let Some(line) = warning.line {
                ggen_core::utils::alert_info!("  Line {}: {}", line, warning.message);
            } else {
                ggen_core::utils::alert_info!("  {}", warning.message);
            }
        }
    }

    if output.errors_found == 0 && output.warnings_found == 0 {
        ggen_core::utils::alert_success!("No issues found. Template is valid!");
    }

    // Return error if errors were found
    if output.errors_found > 0 {
        return Err(ggen_core::utils::error::Error::new("Template validation failed"));
    }
