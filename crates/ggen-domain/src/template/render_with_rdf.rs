...
pub fn render_with_rdf(options: &RenderWithRdfOptions) -> Result<RenderWithRdfResult> {
    ...
    if let Some(parent) = options.output_path.parent() {
        std::fs::create_dir_all(parent).map_err(|e| {
            mcpp_utils::error::Error::new(&format!("Failed to create output directory: {}", e))
        })?;
    }
    ...
    let mut template = if options.use_preprocessor {
        Template::parse_with_preprocessor(&template_content, &options.template_path, output_dir)
            .map_err(|e| {
                mcpp_utils::error::Error::new(&format!("Failed to parse template: {}", e))
            })?
    } else {
    ...
    let mut tera = mcpp_core::tera_env::build_tera_minimal().map_err(|e| {
        mcpp_utils::error::Error::new(&format!("Failed to create Tera instance: {}", e))
    })?;
    ...
    let mut graph = Graph::new().map_err(|e| {
        mcpp_utils::error::Error::new(&format!("Failed to create RDF graph: {}", e))
    })?;
    ...
    template
        ...
        .map_err(|e| {
            mcpp_utils::error::Error::new(&format!("Failed to render frontmatter: {}", e))
        })?;
    ...
    let rendered_content = if !options.rdf_files.is_empty() {
        ...
        template
            ...
            .map_err(|e| {
                mcpp_utils::error::Error::new(&format!("Template rendering failed: {}", e))
            })?
        ...
        template
            ...
            .map_err(|e| {
                mcpp_utils::error::Error::new(&format!("Failed to process graph: {}", e))
            })?;
    ...
    let (files_created, total_bytes, output_path) = if rendered_content.contains("{# FILE:") {
        ...
        std::fs::write(&options.output_path, &rendered_content).map_err(|e| {
            mcpp_utils::error::Error::new(&format!("Failed to write output: {}", e))
        })?;
...
pub fn generate_from_rdf(
    ...
    for rdf_file in &rdf_files {
        ...
        graph
            ...
            .map_err(|e| mcpp_utils::error::Error::new(&format!("Failed to parse RDF: {}", e)))?;
    }
    ...
    let query = r#"
        ...
        WHERE {
            ...
            OPTIONAL { ?template mcpp:templateDescription ?description }
            OPTIONAL { ?template mcpp:outputPath ?to }
        }
        LIMIT 1
    "#;
    ...
    let results = graph
        .query_cached(query)
        .map_err(|e| mcpp_utils::error::Error::new(&format!("SPARQL query failed: {}", e)))?;

    // Extract template metadata
    let (name, description, to) = match results {
        mcpp_core::graph::CachedResult::Solutions(rows) if !rows.is_empty() => {
            let row = &rows[0];
            let name = row
                .get("name")
    ...
    std::fs::write(&output_template_path, template_content)
        .map_err(|e| mcpp_utils::error::Error::new(&format!("Failed to write template: {}", e)))?;
...
<http://example.org/template1> a mcpp:Template ;
    mcpp:templateName "My Template" ;
    mcpp:templateDescription "A generated template" ;
    mcpp:outputPath "output.txt" .
"#;
...
fn split_file_markers(content: &str, base_dir: &Path) -> Result<Vec<(PathBuf, String)>> {
    let file_marker_re = Regex::new(r"\{#\s*FILE:\s*([^\s#]+)\s*#\}").map_err(|e| {
        mcpp_utils::error::Error::new(&format!("Failed to compile file marker regex: {}", e))
    })?;
