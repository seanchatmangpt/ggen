...
pub fn generate_file_tree(
    ...
    let template = TemplateParser::parse_file(template_path)
        .map_err(|e| ggen_core::utils::error::Error::new(&format!("Failed to parse template: {}", e)))?;
    ...
    let context = TemplateContext::from_map(var_map)
        .map_err(|e| ggen_core::utils::error::Error::new(&format!("Failed to create context: {}", e)))?;
    ...
    let result = ggen_core::templates::generate_file_tree(template, context, output_dir)
        .map_err(|e| ggen_core::utils::error::Error::new(&format!("Generation failed: {}", e)))?;
    ...
    fn check_nodes(
        ...
        for node in nodes {
            let rendered_name = context
                ...
                .map_err(|e| ggen_core::utils::error::Error::new(&format!("Failed to render: {}", e)))?;
...
pub fn run(args: &GenerateTreeInput) -> Result<()> {
    ...
    let runtime = tokio::runtime::Runtime::new()
        .map_err(|e| ggen_core::utils::error::Error::new(&format!("Failed to create runtime: {}", e)))?;

    let output = runtime.block_on(execute_generate_tree(args.clone()))?;

    ggen_core::utils::alert_success!("Generated file tree from: {}", args.template.display());
    ggen_core::utils::alert_info!("📁 Output directory: {}", output.output_path);
    ggen_core::utils::alert_info!("📄 Files created: {}", output.files_generated);
    ggen_core::utils::alert_info!("📂 Directories created: {}", output.directories_created);
