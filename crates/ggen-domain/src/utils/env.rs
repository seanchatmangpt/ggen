...
impl EnvironmentManager for DefaultEnvironmentManager {
    fn get_environment(&self) -> Result<GgenEnvironment> {
        ...
        Ok(GgenEnvironment {
            ...
            templates_dir: home_dir.join(".mcpp/templates"),
            cache_dir: home_dir.join(".mcpp/cache"),
            config_dir: home_dir.join(".mcpp/config"),
            config: GgenConfig {
                editor: None,
...
// CLI wrapper - MOVED TO mcpp-cli crate
// Domain layer must not depend on clap
// See mcpp-cli/src/cmds/utils_v3.rs for CLI Args definitions
/*
#[derive(Debug, Parser)]
pub struct EnvInput {
...
pub fn run_env_old(args: &EnvInput) -> Result<()> {
    ...
    if args.list {
        ...
        if vars.is_empty() {
            mcpp_utils::alert_info!("No GGEN environment variables found");
        } else {
            mcpp_utils::alert_info!("GGEN Environment Variables:");
            for (key, value) in vars {
                mcpp_utils::alert_info!("  {}={}", key, value);
            }
        }
    }
    ...
    if args.show_dirs {
        ...
        mcpp_utils::alert_info!("GGEN Directories:");
        mcpp_utils::alert_info!("  Home: {}", env.home_dir.display());
        mcpp_utils::alert_info!("  Templates: {}", env.templates_dir.display());
        mcpp_utils::alert_info!("  Cache: {}", env.cache_dir.display());
        mcpp_utils::alert_info!("  Config: {}", env.config_dir.display());
    }

    if args.ensure_dirs {
        manager.ensure_directories()?;
        mcpp_utils::alert_success!("All directories created");
    }

    if args.clear_cache {
        let count = manager.clear_cache()?;
        mcpp_utils::alert_success!("Cleared {} cache files", count);
    }

    if let Some(key) = &args.get {
        match manager.get_var(key) {
            Some(value) => mcpp_utils::alert_info!("{}={}", key, value),
            None => mcpp_utils::alert_info!("Variable '{}' not found", key),
        }
    }
    ...
    for set_arg in &args.set {
        if let Some((key, value)) = set_arg.split_once('=') {
            ...
            mcpp_utils::alert_success!("Set {}={}", key, value);
        } else {
            mcpp_utils::alert_warning!("Invalid format: {}. Use KEY=VALUE", set_arg);
        }
    }
    ...
    if !args.list && !args.show_dirs && !args.ensure_dirs && !args.clear_cache
        ...
        let env = manager.get_environment()?;
        mcpp_utils::alert_info!("GGEN Environment:");
        mcpp_utils::alert_info!("  Home: {}", env.home_dir.display());
        mcpp_utils::alert_info!("  Templates: {}", env.templates_dir.display());
        mcpp_utils::alert_info!("  Cache: {}", env.cache_dir.display());
        mcpp_utils::alert_info!("  Config: {}", env.config_dir.display());
