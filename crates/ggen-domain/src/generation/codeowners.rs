...
//!
//! This module wraps mcpp-core's CodeownersGenerator to provide domain-level
//! CODEOWNERS generation that integrates with mcpp.toml configuration.
//!
//! Implements `[codeowners]` behavior from mcpp.toml:
...
use mcpp_core::types::codeowners::CodeownersGenerator;
...
///
/// Local definition to avoid cyclic dependency with mcpp-config.
/// Mirrors [codeowners] from mcpp.toml.
...
///
/// This function reads the CODEOWNERS configuration from mcpp.toml and generates
...
pub fn generate_codeowners(
    ...
    if !config.enabled {
        return Err(mcpp_utils::error::Error::new(
            "CODEOWNERS generation is disabled in mcpp.toml. Set [codeowners].enabled = true",
        ));
    }
    ...
    let (output_path, entries_generated) = if let Some(ref custom_path) = config.output_path {
        ...
        std::fs::write(&path, &content).map_err(|e| {
            mcpp_utils::error::Error::new(&format!("Failed to write CODEOWNERS: {}", e))
        })?;
        (path, entries)
    } else {
...
///
/// Convenience function for simple use cases without mcpp.toml.
...
mod tests {
    ...
    fn test_generate_codeowners_with_owners() {
        ...
        assert!(content.contains("Auto-generated by mcpp"));
