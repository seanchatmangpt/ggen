//! Configuration loader for mcpp.toml

use super::validation::{ConfigValidator, ValidationResult};
use mcpp_utils::error::Error as DomainError;
use serde::{Deserialize, Serialize};
use std::collections::HashMap;
use std::path::{Path, PathBuf};

/// Complete mcpp.toml configuration structure
...
fn default_marketplace_url() -> String {
    "https://marketplace.mcpp.dev".to_string()
}
...
impl ConfigLoader {
    ...
    /// Load mcpp.toml with environment override
    pub fn load_with_env(
        ...
        let validation = self.validator.validate_mcpp_config(&config);
        Ok((config, validation))
    }

    /// Load mcpp.toml without environment override
...
mod tests {
    ...
    fn test_ontology_config_default() {
        ...
        assert_eq!(config.base_uri, "https://mcpp.dev/");
    ...
    fn test_marketplace_config_default() {
        ...
        assert_eq!(config.registry_url, "https://marketplace.mcpp.dev");
