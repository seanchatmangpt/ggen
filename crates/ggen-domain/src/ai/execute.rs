...
///
/// ```rust,no_run
/// # use ggen_core::domain::ai::execute;
...
pub async fn execute_generate(
    ...
    if prompt.trim().is_empty() {
        return Err(ggen_core::utils::error::Error::new("Prompt cannot be empty"));
    }
...
pub async fn execute_analyze(
    ...
    let analysis_text = if let Some(code_content) = code {
        ...
        if code_content.trim().is_empty() {
            return Err(ggen_core::utils::error::Error::new("Code cannot be empty"));
        }
        // #region agent log
        log_debug(
            "debug-session",
...
pub async fn execute_chat(message: &str, model: Option<&str>) -> Result<ExecuteChatResult> {
    ...
    if message.trim().is_empty() {
        return Err(ggen_core::utils::error::Error::new("Message cannot be empty"));
    }
    ...
    let client = GenAiClient::new(config)
        .map_err(|e| ggen_core::utils::error::Error::new(&format!("Failed to create LLM client: {e}")))?;
...
User: {}",
    ...
    let resp = client
        .complete(&prompt)
        .await
        .map_err(|e| ggen_core::utils::error::Error::new(&format!("Chat failed: {e}")))?;
