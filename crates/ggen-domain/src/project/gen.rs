...
pub async fn execute_gen(input: GenInput) -> Result<GenerationResult> {
    use mcpp_core::{CacheManager, LockfileManager, TemplateResolver};
    ...
    // Step 3: Resolve template using mcpp-core
    // Create managers with defaults
    let _cache_manager = CacheManager::new().map_err(|e| {
        mcpp_utils::error::Error::new(&format!("Failed to create cache manager: {}", e))
    })?;
    ...
    let template_source = resolver.resolve(&input.template_ref).map_err(|e| {
        mcpp_utils::error::Error::new(&format!("Failed to resolve template: {}", e))
    })?;

    let template_path = template_source.template_path;

    // Step 4: Generate using mcpp-core Generator
    ...
    let _output_path = generator.generate().map_err(|e| {
        mcpp_utils::error::Error::new(&format!("Failed to generate project: {}", e))
    })?;
    ...
    if !input.dry_run {
        ...
        for file_path in files {
            let content = std::fs::read_to_string(&file_path).map_err(|e| {
                mcpp_utils::error::Error::new(&format!("Failed to read generated file: {}", e))
            })?;
