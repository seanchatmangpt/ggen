...
pub fn apply_plan(args: &ApplyInput) -> Result<ApplicationResult> {
    ...
    let content = fs::read_to_string(plan_path).map_err(ggen_core::utils::error::Error::from)?;

    let plan: GenerationPlan = match plan_path.extension().and_then(|ext| ext.to_str()) {
        Some("json") => serde_json::from_str(&content).map_err(ggen_core::utils::error::Error::from)?,
        Some("yaml") | Some("yml") => {
            serde_yaml::from_str(&content).map_err(ggen_core::utils::error::Error::from)?
        }
        Some("toml") => toml::from_str(&content).map_err(ggen_core::utils::error::Error::from)?,
        _ => {
            return Err(ggen_core::utils::error::Error::new_fmt(format_args!(
    ...
    // Show plan summary
    ggen_core::utils::alert_info!("📋 Plan Summary:");
    ggen_core::utils::alert_info!("  Template: {}", plan.template_ref);
    ggen_core::utils::alert_info!("  Variables: {}", plan.variables.len());
    ...
    if args.dry_run {
        ggen_core::utils::alert_info!("🔍 Dry run mode - no changes will be applied");
    ...
    if !args.auto_confirm {
        ggen_core::utils::alert_warning!("\nThis will apply the generation plan to your project.");
        ggen_core::utils::alert_info!("Continue? [y/N]: ");

        let mut input = String::new();
        std::io::stdin()
            .read_line(&mut input)
            .map_err(ggen_core::utils::error::Error::from)?;

        if !input.trim().to_lowercase().starts_with('y') {
            ggen_core::utils::alert_info!("Plan application cancelled by user");
            return Ok(ApplicationResult {
                operations_count: 0,
                plan_file: args.plan_file.clone(),
            });
