...
pub fn create_plan(args: &PlanInput) -> Result<PlanResult> {
    ...
    let output_path = match &args.output {
        ...
        None => Path::new("mcpp-plan").with_extension(&args.format),
    };

    // Serialize plan
    let content = match args.format.as_str() {
        "json" => serde_json::to_string_pretty(&plan).map_err(mcpp_utils::error::Error::from)?,
        "yaml" => serde_yaml::to_string(&plan).map_err(mcpp_utils::error::Error::from)?,
        "toml" => toml::to_string_pretty(&plan).map_err(|e| {
            mcpp_utils::error::Error::new_fmt(format_args!("TOML serialization failed: {}", e))
        })?,
        _ => {
            return Err(mcpp_utils::error::Error::new_fmt(format_args!(
    ...
    fs::write(&output_path, content).map_err(mcpp_utils::error::Error::from)?;
