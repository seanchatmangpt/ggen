...
mod tests {
    ...
    fn test_full_mape_k_loop() {
        ...
        // Simulate tick budget violations
        for i in 0..5 {
            let obs = Observation {
                id: format!("obs-ticks-{}", i),
                obs_type: ObservationType::Metric,
                timestamp: now - (5000 - i as u64 * 100),
                data: serde_json::json!({"pattern": "expensive_op", "ticks": 15}),
        ...
        // Check we found tick budget violations
        let has_tick_findings = findings
            .iter()
            .any(|f| f.kind == FindingKind::TickBudgetViolation);
        assert!(has_tick_findings);

        // Phase 7: Plan - Generate overlay proposals
        let mut planner = PlanEngine::new();
        let proposals = planner.plan(&findings);
        assert!(!proposals.is_empty());

        // Phase 8a: Execute - Validate and promote
        ...
        // Phase 8b: Knowledge - Store everything
        ...
        assert_eq!(knowledge.all_observations().len(), 5);
        assert!(!knowledge.all_findings().is_empty());
        assert!(!knowledge.all_overlays().is_empty());

        let stats = knowledge.statistics();
        assert_eq!(stats.total_observations, 5);
        assert!(stats.total_findings > 0);
        assert!(stats.total_overlays > 0);
    }
    ...
    fn test_monitor_aggregate_queries() {
        ...
        // Add observations with tick budget violations
        for i in 0..3 {
            monitor.ingest_observation(Observation {
                id: format!("tick-violation-{}", i),
                obs_type: ObservationType::Metric,
                timestamp: now - (3000 - i as u64 * 100),
                data: serde_json::json!({"pattern": format!("pattern_{}", i), "ticks": 10 + i * 2}),
                source: "monitor".to_string(),
            });
        }
    ...
    fn test_analyze_detection_types() {
        ...
        monitor.ingest_observation(Observation {
            ...
            obs_type: ObservationType::Metric,
            timestamp: now,
            data: serde_json::json!({"type": "latency", "value": 150}),
            source: "monitor".to_string(),
        });
        ...
        monitor.ingest_observation(Observation {
            ...
            timestamp: now,
            data: serde_json::json!({"type": "drift"}),
            source: "monitor".to_string(),
        });
