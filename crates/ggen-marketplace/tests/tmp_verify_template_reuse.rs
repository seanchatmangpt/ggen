...
async fn test_workflow_5_template_reuse_with_variables() {
    ...
    let Some(pack_with_templates) = packs.iter().find(|p| !p.templates.is_empty()) else {
        println!("No packs with templates found");
        return;
    };

    let pack_id = &pack_with_templates.id;
    let Ok(pack) = show_pack(pack_id) else {
        println!("Should show pack");
        return;
    };

    assert!(!pack.templates.is_empty(), "Pack should have templates");
    ...
    assert!(
        result1.is_ok(),
        "Should generate with variables: {result1:?}"
    );
    let Ok(output1) = result1 else { unreachable!() };
    ...
    assert!(
        result2.is_ok(),
        "Should reuse template with new variables: {result2:?}"
    );
    let Ok(output2) = result2 else { unreachable!() };
    ...
    for pack in packs {
        if !pack.templates.is_empty() {
            ...
            for template in &pack.templates {
                ...
                assert!(!template.path.is_empty(), "Template should have path");
            }
        }
    }
}
...
async fn test_generate_with_all_variables() {
    ...
    assert!(
        result.is_ok(),
        "Should generate with all variables provided"
    );
}
...
async fn test_variable_substitution_in_output() {
    ...
    let Some(pack) = packs.iter().find(|p| !p.templates.is_empty()) else {
        return;
    };
