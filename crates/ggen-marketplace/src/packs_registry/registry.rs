...
impl PackRegistry for InMemoryRegistry {
    async fn publish(&self, pack: &Pack, metadata: PublishMetadata) -> Result<PublishReceipt> {
        ...
        if pack.id.is_empty() || pack.name.is_empty() {
            return Err(Error::Other("Pack ID and name are required".to_string()));
        }
...
mod tests {
    use super::*;
    use crate::packs_registry::repository::FileSystemRepository;
    use crate::packs_registry::types::PackMetadata;
