...
use crate::models::{Category, Package, PackageId, PackageMetadata, PackageStats, Query, Version};
...
impl QueryRoot {
    ...
    async fn package(
        &self, ctx: &Context<'_>, namespace: String, name: String,
    ...
    async fn package_version(
        &self, ctx: &Context<'_>, namespace: String, name: String, version: String,
    ) -> GqlResult<PackageGQL> {
        let registry = ctx.data::<Arc<dyn Registry>>()?;
        let id = PackageId::new(namespace, name);
...
impl MutationRoot {
    ...
    async fn publish(&self, ctx: &Context<'_>, input: PublishInput) -> GqlResult<PackageGQL> {
        ...
        let version =
            Version::parse(&input.version).map_err(|e| format!("Invalid version: {}", e))?;
...
pub fn create_schema(
    registry: Arc<dyn Registry>, store: Arc<dyn PackageStore>,
) -> MarketplaceSchema {
