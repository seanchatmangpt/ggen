...
//!
//! Ported from `ggen-core/src/agent/receipt.rs` into `ggen-cli` during the
//! ggen-core retirement migration (`specs/014-ggen-core-replacement`, task
//! T041), then moved again into this crate (`ggen-marketplace/src/agent/`) as
//! tasks T048/T062, alongside `types.rs` and `facade.rs`, since none of the
//! three had any `ggen-cli`-specific dependency. Its own dependency
//! (`ggen_config::receipt::{hash_data, Receipt}`) was already ported to
//! `ggen-config` in an earlier bucket (T033) and stays an external-crate import
//! from here.
//!
