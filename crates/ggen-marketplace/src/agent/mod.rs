...
//!
//! Moved from `ggen-cli/src/agent/` to `ggen-marketplace/src/agent/` during the
//! ggen-core retirement migration (`specs/014-ggen-core-replacement`, tasks
//! T048/T062): this module's only real dependencies were already
//! `ggen_marketplace::*` / `ed25519_dalek` / `ggen_config::receipt` / `serde` /
//! `std`/`thiserror` (see `facade.rs` for the original T041 port history), so it
//! belongs alongside the marketplace subsystems it wraps rather than in the CLI
//! crate. `ggen-cli`'s `crate::agent` module is now a thin re-export shim over
//! this module so every existing `crate::agent::X` call site in `ggen-cli`
...
//!   the CLI uses (`ggen_marketplace::marketplace::install::install_pack_by_id`,
//!   the lockfile, the receipt emitter). It does not fork a second install path,
//!   so it can only *deepen* authority, never bypass it.
...
//!
//! ```no_run
//! use ggen_marketplace::agent::{PackAgent, InstallRequest};
//!
