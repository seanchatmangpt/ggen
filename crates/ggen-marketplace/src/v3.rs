...
impl V3OptimizedRegistry {
    ...
    fn rebuild_search_index(&self) -> Result<()> {
        ...
        let query = r"
            SELECT ?name ?package WHERE {
                ?package <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://mcpp.dev/ontology#Package> .
                ?package <http://mcpp.dev/ontology#name> ?name .
            }
        ";
    ...
    pub fn update_search_index(&self, package_id: &str, package_name: &str) -> Result<()> {
        let package_uri = format!("http://mcpp.dev/ontology#packages/{package_id}");
        let mut index = self.search_index.write();
