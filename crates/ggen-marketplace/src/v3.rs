...
impl V3OptimizedRegistry {
    ...
    fn rebuild_search_index(&self) -> Result<()> {
        ...
        let query = r"
            SELECT ?name ?package WHERE {
                ?package <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <https://mcpp.io/marketplace/classes/Package> .
                ?package <https://mcpp.io/marketplace/properties/name> ?name .
            }
        ";
    ...
    pub fn update_search_index(&self, package_id: &str, package_name: &str) -> Result<()> {
        let package_uri = format!("https://mcpp.io/marketplace/packages/{package_id}");
        let mut index = self.search_index.write();
