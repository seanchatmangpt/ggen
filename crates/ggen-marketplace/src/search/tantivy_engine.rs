...
impl TantivySearchEngine {
    ...
    fn package_to_doc(&self, package: &Package) -> TantivyDocument {
        ...
        doc.add_date(
            ...
            tantivy::DateTime::from_timestamp_secs(package.created_at.timestamp()),
        );
        doc.add_date(
            self.fields.updated_at,
            tantivy::DateTime::from_timestamp_secs(package.updated_at.timestamp()),
        );
...
impl SearchEngine for TantivySearchEngine {
    ...
    async fn search(&self, query: &SearchQuery) -> Result<SearchResults> {
        ...
        // Category facets
        let category_counts =
            self.collect_facet_counts(&searcher, &tantivy_query, self.fields.category)?;
        facets.insert("category".to_string(), category_counts);

        // Language facets
        let language_counts =
            self.collect_facet_counts(&searcher, &tantivy_query, self.fields.language)?;
        facets.insert("language".to_string(), language_counts);

        // License facets
        let license_counts =
            self.collect_facet_counts(&searcher, &tantivy_query, self.fields.license)?;
        facets.insert("license".to_string(), license_counts);

        let query_time_ms = start.elapsed().as_millis() as u64;

        // Record span attributes for OpenTelemetry
        Span::current().record("search.results_count", packages.len());
        Span::current().record("search.total_results", total);
        Span::current().record("search.query_time_ms", query_time_ms);
