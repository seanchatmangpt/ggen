//! Ontology source resolution — registry references and local filesystem paths.
//!
//! Ported verbatim from `ggen-core/src/ontology/resolver.rs` during the ggen-core retirement
//! migration (`specs/014-ggen-core-replacement`, Phase 3 "non-colliding noun re-points"): it
//! is `OntologyLoader`'s local-filesystem fallback and has no other ggen-core-internal
//! dependency (pure `std` + `walkdir`, already a direct `ggen-marketplace` dependency).
