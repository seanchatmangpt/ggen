...
//! These ontologies are always available without network access.
//!
//! Ported verbatim from `ggen-core/src/ontology/core_bundle.rs` during the ggen-core
//! retirement migration (`specs/014-ggen-core-replacement`, Phase 3 "non-colliding noun
//! re-points"): `ggen-cli`'s `cmds/ontology.rs` is the sole caller left depending on
//! ggen-core for this capability. Zero internal ggen-core dependencies in the original (pure
//! `std`, inline `&'static [u8]` byte literals) — this port is byte-for-byte identical,
