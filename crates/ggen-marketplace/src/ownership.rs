...
pub fn execute_merge_strategy(
    content_a: &str, content_b: &str, strategy: &MergeStrategy,
...
fn merge_concat(content_a: &str, content_b: &str) -> Result<MergeResult, MergeError> {
    if content_a.is_empty() {
        return Ok(MergeResult {
            ...
            had_conflict: false,
            description: "concat: content_a was empty, returned content_b".into(),
        });
    }
    if content_b.is_empty() {
        return Ok(MergeResult {
            ...
            had_conflict: false,
            description: "concat: content_b was empty, returned content_a".into(),
        });
    }
...
fn merge_last_writer_wins(content_a: &str, content_b: &str) -> Result<MergeResult, MergeError> {
    ...
    Ok(MergeResult {
        content: content_b.to_string(),
        had_conflict,
        description: if had_conflict {
            "last_writer_wins: content_b overrode content_a".into()
        } else {
            "last_writer_wins: contents identical, no override".into()
        },
    })
}
...
fn merge_first_writer_wins(content_a: &str, content_b: &str) -> Result<MergeResult, MergeError> {
    ...
    Ok(MergeResult {
        ...
        had_conflict,
        description: if had_conflict {
            "first_writer_wins: content_a preserved, content_b ignored".into()
        } else {
            "first_writer_wins: contents identical, no conflict".into()
        },
    })
}
...
fn merge_recursive(content_a: &str, content_b: &str) -> Result<MergeResult, MergeError> {
    ...
    if let (Some(ver_a), Some(ver_b)) = (
        semver::Version::parse(a_trimmed).ok(),
        semver::Version::parse(b_trimmed).ok(),
    ) {
    ...
    Ok(MergeResult {
        ...
        had_conflict,
        description: if had_conflict {
            format!("recursive: merged with {conflicts} duplicate line(s) deduplicated")
        } else {
            "recursive: merged without conflicts".into()
        },
    })
}
...
fn merge_semver(
    ver_a: &semver::Version, ver_b: &semver::Version,
) -> Result<MergeResult, MergeError> {
    let chosen = if ver_a > ver_b {
        ver_a
    } else if ver_b > ver_a {
        ver_b
    } else {
        return Ok(MergeResult {
            content: ver_a.to_string(),
            had_conflict: false,
            description: "semver: versions identical".into(),
        });
    };
    ...
    Ok(MergeResult {
        ...
        had_conflict,
        description: if had_conflict {
            format!("semver: selected {chosen} (higher of {ver_a} and {ver_b})")
        } else {
            "semver: versions identical".into()
        },
    })
}

fn merge_custom_sparql(
    content_a: &str, content_b: &str, query: &str,
) -> Result<MergeResult, MergeError> {
    ...
    {
        return Err(MergeError::InvalidSparqlQuery(format!(
            "query must contain a SPARQL keyword (SELECT, CONSTRUCT, ASK, DESCRIBE): {query}"
        )));
    }
    if !query_upper.contains("WHERE") && !query.contains('{') {
        return Err(MergeError::InvalidSparqlQuery(
            "query must contain WHERE clause or graph pattern".into(),
        ));
    }
    let had_conflict = content_a != content_b;
    Ok(MergeResult {
        content: if had_conflict {
            format!("{content_a}\n{content_b}")
        } else {
            content_a.to_string()
        },
        had_conflict,
        description: format!(
            "custom_sparql: query validated, merged {} bytes",
            if had_conflict {
                content_a.len() + content_b.len() + 1
            } else {
                content_a.len()
            }
        ),
...
fn merge_fail_on_conflict(content_a: &str, content_b: &str) -> Result<MergeResult, MergeError> {
    if content_a == content_b {
        return Ok(MergeResult {
            content: content_a.to_string(),
            had_conflict: false,
            description: "fail_on_conflict: contents identical, no conflict".into(),
        });
    }
    Err(MergeError::ConflictDetected(format!(
        "contents differ:\n  content_a ({} bytes): {}\n  content_b ({} bytes): {}",
        content_a.len(),
        truncate_preview(content_a, 80),
        content_b.len(),
        truncate_preview(content_b, 80)
    )))
}
    ...
    pub fn conflicts_with(&self, other: &Self) -> Option<String> {
        ...
        match (&self.class, &other.class) {
            ...
            (OwnershipClass::Mergeable, OwnershipClass::Mergeable) => {
                match (&self.merge_strategy, &other.merge_strategy) {
                    (Some(a), Some(b)) if a != b => Some(format!(
                        ...
                        self.target, self.owner_pack, a, other.owner_pack, b
                    )),
                    _ => None,
                }
            }
            (OwnershipClass::Overlay, OwnershipClass::Overlay) => Some(format!(
                "Overlay conflict: {:?} requires transfer between {} and {}",
                self.target, self.owner_pack, other.owner_pack
            )),
            ...
            (OwnershipClass::Exclusive, _) | (_, OwnershipClass::Exclusive) => Some(format!(
                "Exclusive violation: {:?} owned by {}, {} claims {:?}",
...
mod tests {
    ...
    #[test]
    fn test_ownership_class_requires_merge_strategy() {
        assert!(OwnershipClass::Mergeable.requires_merge_strategy());
        assert!(!OwnershipClass::Exclusive.requires_merge_strategy());
    }

    #[test]
    fn test_concat_basic() {
        let r = execute_merge_strategy("line1", "line2", &MergeStrategy::Concat).unwrap();
        assert_eq!(r.content, "line1\nline2");
        assert!(!r.had_conflict);
    }
    #[test]
    fn test_concat_empty_a() {
        let r = execute_merge_strategy("", "content", &MergeStrategy::Concat).unwrap();
        assert_eq!(r.content, "content");
    }
    #[test]
    fn test_concat_empty_b() {
        let r = execute_merge_strategy("content", "", &MergeStrategy::Concat).unwrap();
        assert_eq!(r.content, "content");
    }
    #[test]
    fn test_concat_both_empty() {
        let r = execute_merge_strategy("", "", &MergeStrategy::Concat).unwrap();
        assert_eq!(r.content, "");
    }

    #[test]
    fn test_last_writer_wins_basic() {
        let r = execute_merge_strategy("old", "new", &MergeStrategy::LastWriterWins).unwrap();
        assert_eq!(r.content, "new");
        assert!(r.had_conflict);
    }
    #[test]
    fn test_last_writer_wins_identical() {
        let r = execute_merge_strategy("same", "same", &MergeStrategy::LastWriterWins).unwrap();
        assert!(!r.had_conflict);
    }

    #[test]
    fn test_first_writer_wins_basic() {
        let r = execute_merge_strategy("first", "second", &MergeStrategy::FirstWriterWins).unwrap();
        assert_eq!(r.content, "first");
        assert!(r.had_conflict);
    }
    #[test]
    fn test_first_writer_wins_identical() {
        let r = execute_merge_strategy("same", "same", &MergeStrategy::FirstWriterWins).unwrap();
        assert!(!r.had_conflict);
    }
    ...
    #[test]
    fn test_recursive_semver_identical() {
        let r = execute_merge_strategy("1.2.3", "1.2.3", &MergeStrategy::Recursive).unwrap();
    ...
    fn test_recursive_line_by_line() {
        let r = execute_merge_strategy("a\nb", "c\nd", &MergeStrategy::Recursive).unwrap();
        assert_eq!(r.content, "a\nb\nc\nd");
        assert!(!r.had_conflict);
    }
    #[test]
    fn test_recursive_deduplication() {
        let r = execute_merge_strategy("a\nb", "b\nc", &MergeStrategy::Recursive).unwrap();
        assert_eq!(r.content, "a\nb\nc");
        assert!(r.had_conflict);
    }

    #[test]
    fn test_custom_sparql_valid() {
        let r = execute_merge_strategy(
            "a",
            "b",
            &MergeStrategy::CustomSparql {
                query: "SELECT ?x WHERE { ?x a :Thing }".into(),
            },
        )
        .unwrap();
        assert!(r.had_conflict);
    ...
    fn test_custom_sparql_invalid() {
        ...
        assert!(matches!(r.unwrap_err(), MergeError::InvalidSparqlQuery(_)));
    }

    #[test]
    fn test_fail_on_conflict_identical() {
        let r = execute_merge_strategy("same", "same", &MergeStrategy::FailOnConflict).unwrap();
        assert!(!r.had_conflict);
    }
    #[test]
    fn test_fail_on_conflict_differs() {
        let r = execute_merge_strategy("a", "b", &MergeStrategy::FailOnConflict);
        assert!(matches!(r.unwrap_err(), MergeError::ConflictDetected(_)));
    }

    #[test]
    fn test_merge_error_display() {
        assert!(MergeError::ConflictDetected("x".into())
            .to_string()
    ...
    fn test_merge_result_serialization() {
        ...
        assert_eq!(serde_json::from_str::<MergeResult>(&j).unwrap(), r);
    }
    #[test]
    fn test_merge_strategy_serialization() {
        for s in [
            ...
            MergeStrategy::FirstWriterWins,
            MergeStrategy::Recursive,
            MergeStrategy::FailOnConflict,
        ] {
            let j = serde_json::to_string(&s).unwrap();
            assert_eq!(serde_json::from_str::<MergeStrategy>(&j).unwrap(), s);
        }
    }
    #[test]
    ...
    fn test_exclusive_conflicts() {
        let t = OwnershipTarget::FilePath(PathBuf::from("/x"));
        let d1 = OwnershipDeclaration::exclusive(t.clone(), "a".into());
        let d2 = OwnershipDeclaration::exclusive(t, "b".into());
        assert!(d1.conflicts_with(&d2).is_some());
    }
    #[test]
    fn test_ownership_map_add() {
        let mut m = OwnershipMap::new();
        m.add(OwnershipDeclaration::exclusive(
            OwnershipTarget::FilePath(PathBuf::from("/x")),
            "a".into(),
        ))
        .unwrap();
        assert!(m.is_owned(&OwnershipTarget::FilePath(PathBuf::from("/x"))));
    }
    #[test]
    fn test_ownership_map_conflict_detection() {
        let mut m = OwnershipMap::new();
        let t = OwnershipTarget::FilePath(PathBuf::from("/x"));
        m.add(OwnershipDeclaration::exclusive(t.clone(), "a".into()))
            .unwrap();
        m.add(OwnershipDeclaration::exclusive(t, "b".into()))
            .unwrap();
        assert_eq!(m.check_conflicts().len(), 1);
    }
}
