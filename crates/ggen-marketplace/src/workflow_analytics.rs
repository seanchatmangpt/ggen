...
impl WorkflowLog {
    ...
    pub fn statistics(&self) -> ProcessStatistics {
        ...
        ProcessStatistics {
            ...
            most_common_variant: variant_counts.iter()
    ...
    fn duration_statistics(&self) -> (f64, f64) {
        ...
        let mut durations: Vec<i64> = self.traces
            .iter()
...
impl DirectlyFollowsGraph {
    ...
    pub fn pareto_edges(&self, percentile: f32) -> Vec<((WorkflowEventType, WorkflowEventType), usize)> {
        let total_follows: usize = self.follows.values().sum();
        let threshold = total_follows as f32 * percentile;

        let mut edges: Vec<_> = self.follows.iter()
            .map(|(k, v)| (k.clone(), *v))
            .collect();
