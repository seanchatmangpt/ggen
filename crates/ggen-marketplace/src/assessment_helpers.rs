...
pub fn filter_by_level(
    assessments: &[MaturityAssessment], level: MaturityLevel,
) -> Vec<&MaturityAssessment> {
    assessments.iter().filter(|a| a.level() == level).collect()
}

/// Filter assessments by score range
pub fn filter_by_score_range(
    assessments: &[MaturityAssessment], min_score: u32, max_score: u32,
) -> Vec<&MaturityAssessment> {
    assessments
        .iter()
        .filter(|a| {
...
pub fn filter_by_dimensions(
    assessments: &[MaturityAssessment], min_documentation: Option<u32>, min_testing: Option<u32>,
...
pub fn get_recommendations(
    use_case: &str, priority: Option<&str>, min_score: Option<u32>,
) -> Vec<String> {
    ...
    if let Some(pri) = priority {
        match pri {
            ...
            "documentation" => {
                guidance.push("Focus on packages with documentation score >= 15".to_string())
            }
            "performance" => {
                guidance.push("Focus on packages with performance score >= 12".to_string())
            }
            "adoption" => guidance.push("Focus on packages with adoption score >= 12".to_string()),
            "maintenance" => {
                guidance.push("Focus on packages with maintenance score >= 5".to_string())
            }
            _ => {}
        }
    }
...
pub fn compare_assessments(a: &MaturityAssessment, b: &MaturityAssessment) -> serde_json::Value {
...
pub fn find_for_use_case<'a>(
    assessments: &'a [MaturityAssessment], use_case: &str,
