...
use ggen_config::hash_data;
use tracing::{debug, instrument};

/// Re-export the canonical keypair generation from ggen-receipt.
pub use ggen_config::generate_keypair as generate_marketplace_keypair;
...
impl MarketplaceVerifier {
    ...
    pub fn from_public_key_hex(public_key_hex: &str) -> Result<Self> {
        let key_bytes = hex::decode(public_key_hex).map_err(|e| {
            crate::marketplace::error::Error::CryptoError(format!("Invalid public key hex: {}", e))
        })?;

        if key_bytes.len() != 32 {
            return Err(crate::marketplace::error::Error::CryptoError(
        ...
        let verifying_key = VerifyingKey::from_bytes(&key_array).map_err(|e| {
            crate::marketplace::error::Error::CryptoError(format!("Invalid verifying key: {}", e))
        })?;
    ...
    pub fn verify(&self, data: &[u8], signature: &MarketplaceSignature) -> Result<bool> {
        ...
        let sig_bytes = hex::decode(&signature.signature).map_err(|e| {
            crate::marketplace::error::Error::SignatureVerificationFailed {
...
impl Signable for MarketplaceVerifier {
    fn sign(&self, _data: &[u8]) -> Result<String> {
        ...
        Err(crate::marketplace::error::Error::CryptoError(
...
///
/// Kept for backward compatibility. Consider using ggen_config::receipt::hash_data directly.
...
mod tests {
    ...
    fn test_marketplace_signature_checksum() {
        ...
        // Checksum should match ggen_config::receipt::hash_data
