...
impl RdfMapper {
            ...
            if let oxigraph::sparql::QueryResults::Solutions(mut solutions) = results {
                if let Some(Ok(solution)) = solutions.next() {
                    ...
                    let quality_score = Self::extract_optional_literal(&solution, "quality")
                        ...
                        .and_then(|v| crate::marketplace::models::QualityScore::new(v).ok());
                    ...
                    let registry_type = Self::extract_optional_literal(&solution, "regType")
                        ...
                        .unwrap_or(crate::marketplace::trust::RegistryType::Ggen);
    ...
    ) -> Result<Vec<crate::marketplace::models::PackageDependency>> {
        let query = format!(
            r"
            ...
            for solution in solutions {
                if let Ok(solution) = solution {
                    ...
                    if let Ok(dep_id) = PackageId::new(dep_id_str) {
                        dependencies.push(crate::marketplace::models::PackageDependency {
                            id: dep_id,
                            version_req,
                            optional,
                        });
...
mod tests {
    use super::*;
    use crate::marketplace::models::{PackageMetadata, PackageVersion};
    use crate::marketplace::trust::RegistryType;
