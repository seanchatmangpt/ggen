...
//!
//! Ported from `ggen-core/src/domain/sync_profile.rs` (specs/014-ggen-core-replacement,
//! docs/jira/v26.7.16/12-OPEN-QUESTIONS.md item 2). All three of this module's
//! dependencies already live in `ggen-marketplace`: [`crate::marketplace::install::
//! compute_pack_digest`], [`crate::packs_registry::metadata::load_pack_metadata`], and
//! [`crate::packs::lockfile::{PackLockfile, PackSource}`] — this is a same-crate sibling
//! module, not a cross-crate port, so no error-type mapping was needed (the original
//! `Result<(), String>` signature is preserved verbatim).

use crate::marketplace::install::compute_pack_digest;
...
///
/// # Errors
/// Returns `Err(String)` with a human-readable explanation when the profile or
/// `--locked` requirements are not satisfied (missing lockfile, missing integrity
/// digest, or a digest mismatch against the pack on disk).
