...
async fn main() -> Result<()> {
    ...
    replication
        .initialize(node_id, peer_addresses.clone())
        .await?;
    ...
    let backup_store =
        S3PackageStore::new("marketplace-backup", "us-east-1", get_aws_credentials()).await?;
    ...
    println!(
        "✓ Downloaded from backup storage: {} bytes",
        downloaded.len()
    );
