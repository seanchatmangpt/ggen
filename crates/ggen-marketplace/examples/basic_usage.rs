...
fn main() {
    eprintln!("This example is disabled. See the file for details.");
    std::process::exit(1);
}
...
mod disabled_example {
    ...
    async fn main_disabled() -> Result<()> {
        ...
        let package_id = marketplace
            .publish(&signed_metadata, package_bytes.clone())
            .await?;
        println!(
            "✓ Published package: {} v{}\n",
        ...
        let versions = marketplace
            .list_versions("example-package", Some("examples"))
            .await?;
    ...
    fn print_dependency_tree(graph: &DependencyGraph, indent: usize) {
        ...
        println!(
            "{}└─ {} v{}",
            indent_str, graph.package.name, graph.package.version
        );
