...
impl SectorStack for ProcessIntelligenceSector {
    ...
    fn validate_artifact(&self, artifact: &Value) -> Result<(), String> {
        ...
        match artifact {
            ...
            Value::Array(_) => {
                Err("ProcessIntelligenceSector: artifact must be a JSON object, got array"
                    .to_string())
            }
            Value::Bool(_) => {
                Err("ProcessIntelligenceSector: artifact must be a JSON object, got bool"
                    .to_string())
            }
            Value::Number(_) => {
                Err("ProcessIntelligenceSector: artifact must be a JSON object, got number"
                    .to_string())
            }
            Value::String(_) => {
                Err("ProcessIntelligenceSector: artifact must be a JSON object, got string"
                    .to_string())
            }
