...
pub struct Pack {
    /// Consumer alias: the `[packs]` key in `ggen.toml` (RFC-GPACK-001 §9).
    /// Deliberately distinct from [`Pack::canonical_name`] — the alias is the
    /// consumer's local reference handle and stays the key of every legacy
    /// field (`ggen.lock` section names, `SyncReport::packs`), while the
    /// canonical identity travels beside it. The alias MUST NOT overwrite
    /// canonical pack identity in receipts (RFC §9); conversely, an alias
    /// differing from the canonical name is lawful (that is its purpose — §8
    /// identity correspondence is admission's business, not local
    /// composition, and is not enforced here).
    pub name: String,
    /// Canonical pack name, parsed from the pack's own `pack.toml`
    /// `[pack].name` (RFC-GPACK-001 §9). Recorded beside the alias in
    /// `ggen.lock` and the sync receipt so both identities survive
    /// resolution; never compared against the alias (an alias MAY differ
    /// lawfully).
...
fn resolve_pack_dir(name: &str, root: &Path) -> Result<Pack> {
    ...
    // RFC-GPACK-001 §9: keep BOTH identities. The `[packs]` key in ggen.toml
    // remains the authoritative resolution name (the consumer alias, and the
    // key of every legacy lock/receipt field); the manifest's own `name` is
    // the canonical pack identity, now retained instead of discarded so it
    // can be recorded beside the alias. The two are deliberately NOT
    // compared here: an alias differing from the canonical name is lawful
    // (that is the alias's purpose), and §8's `Project(I_S)=I_B`
    // correspondence is admission's business, not local composition.
    Ok(Pack {
        name: name.to_string(),
        canonical_name: manifest.pack.name,
...
/// One resolved lock entry: name, canonical name, `source` string, and
/// BLAKE3 hex.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LockEntry {
    /// Consumer alias (the `[packs]` key) — the legacy identity and the
    /// `ggen.lock` section key, unchanged (RFC-GPACK-001 §9 additivity:
    /// existing fields keep their meaning).
    pub name: String,
    /// Canonical pack name from the pack's own `pack.toml` `[pack].name`,
    /// recorded beside the alias (RFC-GPACK-001 §9). Never compared against
    /// `name` and never used as a lookup key — an alias MAY differ lawfully.
    pub canonical_name: String,
...
struct LockDocEntry {
    source: String,
    content_hash: String,
    /// Additive (RFC-GPACK-001 §9): lockfiles written before the alias/
    /// canonical-name distinction landed have no `canonical_name` key and
    /// still parse (`#[serde(default)]`); `check_lock`'s tamper contract
    /// remains content-hash-only, so a legacy lockfile checks exactly as
    /// before. Deserialization-only: the field's job is to admit the key
    /// past `deny_unknown_fields`, never to gate a check.
    #[serde(default)]
    #[allow(dead_code)]
...
pub fn lock_entries(config: &GgenConfig, packs: &[Pack]) -> Result<Vec<LockEntry>> {
    ...
    for pack in packs {
        ...
        entries.push(LockEntry {
            ...
            canonical_name: pack.canonical_name.clone(),
            source,
...
pub fn write_lock(root: &Path, entries: &[LockEntry]) -> Result<()> {
    ...
    for entry in sorted {
        // RFC-GPACK-001 §9: the section key stays the consumer alias (legacy
        // identity, unchanged); the canonical name from the pack's own
        // pack.toml is recorded additively beside it.
        let _ = write!(
            out,
            "\n[packs.{}]\nsource = \"{}\"\ncontent_hash = \"{}\"\ncanonical_name = \"{}\"\n",
            entry.name, entry.source, entry.content_hash, entry.canonical_name
        );
    }
    let lock_path = root.join(LOCK_FILE_NAME);
...
mod tests {
    ...
    fn entry(name: &str) -> LockEntry {
        LockEntry {
            name: name.to_string(),
            canonical_name: format!("{name}-pack"),
...
"#
    ...
    /// A local path pack whose on-disk directory name and pack.toml
    /// `[pack].name` are `acme-payments-pack`, aliased as `payments` in
    /// ggen.toml — the exact RFC-GPACK-001 §9 example shape.
    fn alias_fixture() -> (TempDir, crate::config::GgenConfig) {
        let base = TempDir::new().expect("tempdir");
        let pack_dir = base.path().join("packs/acme-payments-pack");
        std::fs::create_dir_all(pack_dir.join("templates")).expect("mkdir pack");
        std::fs::write(
            pack_dir.join("pack.toml"),
        ...
        .expect("write ontology.ttl");
        std::fs::write(
            pack_dir.join("templates/payment.rs.tmpl"),
            "---\nto: src/payment.rs\n---\n// generated\n",
        )
        .expect("write template");

        let project = base.path().join("project");
        std::fs::create_dir_all(project.join("templates")).expect("mkdir project");
        std::fs::write(project.join("ontology.ttl"), "").expect("write ontology");

        let config = crate::config::GgenConfig {
            project: crate::config::Project {
                name: "alias-fixture".to_string(),
            },
            ontology: crate::config::Ontology {
                source: PathBuf::from("ontology.ttl"),
                prefixes: std::collections::BTreeMap::new(),
            ...
            packs: std::collections::BTreeMap::from([(
                "payments".to_string(),
                PackRef::Path {
                    path: PathBuf::from("../packs/acme-payments-pack"),
    ...
    /// RFC-GPACK-001 §9 (T09 falsifier): resolution keeps BOTH identities —
    /// `consumer_alias` = the `[packs]` key (`payments`), `canonical_name` =
    /// the pack's own pack.toml `[pack].name` (`acme-payments-pack`). The
    /// trailing inversion assertion fails any implementation that conflates
    /// the two (writes the alias as the canonical name, or never parses
    /// pack.toml's name).
    #[test]
    fn resolve_keeps_consumer_alias_and_canonical_name_distinct() {
        let (base, config) = alias_fixture();
        let packs = resolve(&config, base.path().join("project").as_path())
            .expect("aliased path pack must resolve");
        assert_eq!(packs.len(), 1);
        assert_eq!(packs[0].name, "payments", "alias = the [packs] key");
        assert_eq!(
            packs[0].canonical_name, "acme-payments-pack",
            "canonical name = pack.toml [pack].name, recorded beside the alias"
        );
        // Inversion witness: a conflated implementation (canonical_name
        // copied from the alias, or the pack.toml name discarded) cannot
        // satisfy this.
        assert_ne!(
            packs[0].canonical_name, packs[0].name,
            "the alias and the canonical name must remain distinct when they differ on disk"
        );

        // Lock entries carry both identities, keyed by the alias (legacy
        // meaning unchanged).
        let entries = lock_entries(&config, &packs).expect("lock entries");
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].name, "payments");
        assert_eq!(entries[0].canonical_name, "acme-payments-pack");
        assert_ne!(entries[0].canonical_name, entries[0].name);
    }

    /// `write_lock` records the canonical name additively beside the alias:
    /// the section key stays the consumer alias (legacy identity), and a
    /// `canonical_name` line appears carrying the pack.toml name. Inversion
    /// witness: the recorded canonical name must not be the alias.
    #[test]
    fn write_lock_records_canonical_name_beside_the_alias() {
        let dir = TempDir::new().expect("tempdir");
        let entry = LockEntry {
            name: "payments".to_string(),
            canonical_name: "acme-payments-pack".to_string(),
            source: "path:../packs/acme-payments-pack".to_string(),
            content_hash: "blake3:deadbeef".to_string(),
        };
        write_lock(dir.path(), &[entry]).expect("write lock");

        let contents =
            std::fs::read_to_string(dir.path().join(LOCK_FILE_NAME)).expect("read lockfile");
        assert!(
            contents.contains("[packs.payments]"),
            "section key stays the consumer alias: {contents}"
        );
        assert!(
            contents.contains("canonical_name = \"acme-payments-pack\""),
            "canonical name recorded beside the alias: {contents}"
        );
        // Inversion witness: a conflated implementation that writes the
        // alias into the canonical_name field fails here.
        assert!(
            !contents.contains("canonical_name = \"payments\""),
            "canonical_name must never be the alias when they differ: {contents}"
        );
    }

    /// Compatibility falsifier (T09 scope 5): a `ggen.lock` written before
    /// the canonical_name field existed (source + content_hash only) must
    /// still parse and check exactly as before — the additive field is
    /// `#[serde(default)]` and `check_lock`'s tamper contract remains
    /// content-hash-only.
    #[test]
    fn check_lock_accepts_a_legacy_lockfile_without_canonical_name() {
        let dir = TempDir::new().expect("tempdir");
        std::fs::write(
            dir.path().join(LOCK_FILE_NAME),
            "# ggen.lock — generated by `ggen sync`. Do not edit.\n\n\
             [packs.payments]\n\
             source = \"path:../packs/acme-payments-pack\"\n\
             content_hash = \"blake3:deadbeef\"\n",
        )
        .expect("write legacy lockfile");

        let legacy_entry = LockEntry {
            name: "payments".to_string(),
            canonical_name: "acme-payments-pack".to_string(),
            source: "path:../packs/acme-payments-pack".to_string(),
            content_hash: "blake3:deadbeef".to_string(),
        };
        check_lock(dir.path(), &[legacy_entry])
            .expect("a pre-canonical_name lockfile must still check clean");
    }
}
