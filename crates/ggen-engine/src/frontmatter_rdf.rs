...
pub fn project_frontmatter(fm: &Frontmatter) -> crate::error::Result<String> {
    ...
    // `write!` to a String never fails (no I/O involved), so the Result is
    // discarded rather than propagated -- matches clippy's own suggested fix
    // for `format_push_string`.
