...
//! it projects only the 4 fields that carry a real SHACL shape
...
//! `unless_exists`, `force`, `when`, `skip_empty`, `from`, `backup`, `shape`,
...
/// carrying the 4 CP8-projected fields. Always produces at least the
/// required `fm:to` triple; `at_line`/`sh_before`/`sh_after` triples are
/// emitted only when the corresponding field is `Some`.
///
/// # Errors
