//! FIBO + TOGAF Phases A & B Self-Play Tests (Working Version)
//!
...
//!
//! **Key Fix:** Uses `Command` message type for Data content to avoid
//! handler collision with `TextContentHandler`.
...
/// **Important:** Uses `Command` type to avoid handler collision with `TextContentHandler`.
fn make_data_message(
    ...
    ConvergedMessage {
        ...
        envelope: MessageEnvelope {
            message_type: ConvergedMessageType::Command, // Use Command for Data content
    ...
    for turn in 1..=turns {
        let msg = match turn {
            ...
            3 => {
                ...
                let mut data = HashMap::new();
                data.insert("phase".to_string(), json!("A"));
                data.insert("turn".to_string(), json!(3));
                data.insert("action".to_string(), json!("arb_review"));
                data.insert(
                    "feedback".to_string(),
                    json!("Vision is too vague. Add specific metrics and timelines."),
                );
                data.insert("artifact_type".to_string(), json!("ARBReviewFeedback"));
            ...
            5 => {
                ...
                data.insert("artifact_type".to_string(), json!("ApprovalRecord"));
            ...
            _ => {
                ...
                data.insert("action".to_string(), json!("unknown"));
                make_data_message(
                    &format!("phase-a-turn-{}", turn),
                    "unknown",
                    data,
    ...
    for (idx, fibo_entity) in fibo_entities.iter().enumerate() {
        ...
        let msg = match fibo_entity.as_str() {
            ...
            "fibo-fnd:Organization" => {
                ...
                data.insert(
                    "description".to_string(),
                    json!("Organization represents business units and hierarchies"),
                );
