...
use ggen_dspy::{
    Module, Retrieve, MultiHopQA, SimplifiedBaleen,
    MultiHopConfig, BaleenConfig, InMemoryRetriever,
};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("- Retrieve: {}", if retrieve_result.is_ok() { "✓" } else { "✗" });
    println!("- MultiHopQA: {}", if multihop_result.is_ok() { "✓" } else { "✗" });
    println!("- SimplifiedBaleen: {}", if baleen_result.is_ok() { "✓" } else { "✗" });
