//! Fixed deterministic execution manager for cleanroom testing
//!
...
impl DeterministicManager {
    ...
    /// Create a new deterministic manager with specific seed
    ...
    /// Set the seed for deterministic execution
    pub fn set_seed(&self, seed: u64) {
        let mut rng = self.rng.try_write().unwrap();
    ...
    /// Generate a deterministic value based on a key
    pub fn generate_deterministic_value(&self, key: &str) -> u64 {
        let mut rng = self.rng.try_write().unwrap();
...
mod tests {
    ...
    fn test_deterministic_manager_with_seed() {
        let manager = DeterministicManager::new_with_seed(12345);
    ...
    fn test_set_seed() {
        ...
        assert_eq!(manager.get_current_seed(), 42); // Seed field doesn't change, only RNG
    }
    
    #[test]
    fn test_generate_deterministic_value() {
        let manager = DeterministicManager::new();
    ...
    #[test]
    fn test_deterministic_port_allocator() {
        let mut allocator = DeterministicPortAllocator::new(10000);
        
        let port1 = allocator.allocate_port().unwrap();
        let port2 = allocator.allocate_port().unwrap();
        
        assert_eq!(port1, 10000);
        assert_eq!(port2, 10001);
        
        allocator.release_port(port1).unwrap();
        let ports = allocator.get_allocated_ports();
        assert_eq!(ports.len(), 1);
        assert_eq!(ports[0], 10001);
    }

    #[test]
    fn test_deterministic_network_config() {
        let mut config = DeterministicNetworkConfig::new(42);
        
        config.add_port_mapping(5432, 15432);
        config.add_port_mapping(6379, 16379);
        
        assert_eq!(config.get_port_mapping(5432), Some(15432));
        assert_eq!(config.get_port_mapping(6379), Some(16379));
        assert_eq!(config.get_port_mapping(8080), None);
    }

    #[test]
    fn test_fs_operation_type_serialization() {
        let operation_types = vec![
            FsOperationType::CreateFile,
            FsOperationType::CreateDirectory,
            FsOperationType::ReadFile,
            FsOperationType::WriteFile,
            FsOperationType::DeleteFile,
            FsOperationType::DeleteDirectory,
            FsOperationType::ListDirectory,
            FsOperationType::GetMetadata,
        ];

        for operation_type in operation_types {
            let json = serde_json::to_string(&operation_type).unwrap();
            let deserialized: FsOperationType = serde_json::from_str(&json).unwrap();
            assert_eq!(operation_type, deserialized);
        }
    }

    #[test]
    fn test_deterministic_fs_operation_serialization() {
        let operation = DeterministicFsOperation {
            timestamp: SerializableInstant::from(Instant::now()),
            operation_type: FsOperationType::CreateFile,
            path: "/test/file.txt".to_string(),
        ...
        let deserialized: DeterministicFsOperation = serde_json::from_str(&json).unwrap();
        
        assert_eq!(operation.operation_type, deserialized.operation_type);
        assert_eq!(operation.path, deserialized.path);
        assert_eq!(operation.result, deserialized.result);
    }

    #[test]
    fn test_deterministic_state_summary_serialization() {
        let summary = DeterministicStateSummary {
            seed: 42,
            fs_operations_count: 5,
            network_name: "test-network".to_string(),
            allocated_ports_count: 3,
        };

        let json = serde_json::to_string(&summary).unwrap();
        let deserialized: DeterministicStateSummary = serde_json::from_str(&json).unwrap();
        
        assert_eq!(summary.seed, deserialized.seed);
        assert_eq!(summary.fs_operations_count, deserialized.fs_operations_count);
        assert_eq!(summary.network_name, deserialized.network_name);
        assert_eq!(summary.allocated_ports_count, deserialized.allocated_ports_count);
    }
}
