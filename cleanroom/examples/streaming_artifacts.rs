...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for artifact in filtering_collector.artifacts() {
        println!(
            "  - {}: {} bytes",
            artifact.metadata.artifact_type, artifact.metadata.size_bytes
        );
    }
    ...
    let stream = ArtifactStream::new(environment);
    let mut aggregating_collector = AggregatingCollector::new(100).aggregate_by_type();
    ...
    let stream = ArtifactStream::new(environment);
    let mut batching_collector = BatchingCollector::new(100, 2).with_batch_processor(|batch| {
    ...
    println!(
        "✓ Read {} bytes: {}",
        read,
        String::from_utf8_lossy(&read_buf[..read])
    );
    ...
    println!(
        "✓ Zero-copy collection: {:?} for {} artifacts",
        zero_copy_duration,
        zero_copy_collector.count()
    );
    ...
    println!(
        "✓ Owned content collection: {:?} for {} artifacts",
        owned_duration,
        owned_collector.count()
    );

    if zero_copy_duration < owned_duration {
        println!(
            "✓ Zero-copy is faster by {:?}",
            owned_duration - zero_copy_duration
        );
    } else {
        println!(
            "✓ Owned content is faster by {:?}",
            zero_copy_duration - owned_duration
        );
    }
...
mod tests {
    ...
    async fn test_basic_streaming() {
        ...
        stream
            ...
            .await
            .expect("Should stream artifacts");
    ...
    async fn test_streaming_collector() {
        ...
        let collector = stream
            .collect_into(StreamingCollector::with_zero_copy())
            .await
            .expect("Should collect artifacts");
    ...
    async fn test_filtering_collector() {
        let mut collector = FilteringCollector::new(100).filter_by_type(ArtifactType::Text);
    ...
    async fn test_aggregating_collector() {
        let mut collector = AggregatingCollector::new(100).aggregate_by_type();
    ...
    async fn test_transforming_collector() {
        let mut collector =
            TransformingCollector::new(100).transform_content(|content| content.to_uppercase());
    ...
    async fn test_batching_collector() {
        let mut collector = BatchingCollector::new(100, 2).with_batch_processor(|batch| {
        ...
        collector
            .add_artifact(artifact1)
            .expect("Should add artifact");
        assert_eq!(collector.current_batch_size(), 1);

        collector
            .add_artifact(artifact2)
            .expect("Should add artifact");
