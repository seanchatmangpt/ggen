...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let results = executor.context().run_concurrent(futures).await?;
    ...
    println!(
        "Fixed timeout (attempt 0): {:?}",
        fixed_strategy.timeout_for_attempt(0)
    );
    ...
    for i in 0..5 {
        println!(
            ...
            i,
            exp_strategy.timeout_for_attempt(i)
        );
    }

    // Adaptive timeout
    let adaptive_strategy =
        TimeoutStrategy::adaptive(Duration::from_millis(100), 1.5, Duration::from_secs(2));
    for i in 0..5 {
        println!(
            "Adaptive timeout (attempt {}): {:?}",
            i,
            adaptive_strategy.timeout_for_attempt(i)
        );
    }
    ...
    println!(
        "Final state: {:?}, Failures: {}, Successes: {}",
        circuit_breaker.state(),
    ...
    let result = executor_clone
        .run_async_with_cancellation(async {
            tokio::time::sleep(Duration::from_millis(200)).await;
            Ok::<i32, cleanroom::error::CleanroomError>(42)
        })
        .await;
...
mod tests {
    ...
    async fn test_async_executor_examples() {
        ...
        let result = executor
            .run_async(async { Ok::<i32, cleanroom::error::CleanroomError>(42) })
            .await
