...
async fn main() -> Result<()> {
    ...
    println!("\n1. Basic Scope Guard");
    {
        let _guard = ScopeGuard::new().add_cleanup_action(|| {
    ...
    let guard = ResourceGuard::new(resource).add_cleanup_action(|| {
    ...
    // Register container
    environment_arc
        .register_container(container_id.clone(), "container-123".to_string())
        .await?;
    ...
    println!(
        "  Is still registered: {}",
        environment_arc.is_container_registered(&container_id).await
    );
    ...
    let guard = SessionGuard::new(environment).add_cleanup_action(|| {
    ...
    let guard =
        TestGuard::new(environment_arc.clone(), test_name.clone()).add_cleanup_action(|| {
    ...
    // Register container
    environment_arc
        .register_container(container_id.clone(), "health-123".to_string())
        .await?;
    ...
    // Register container
    environment_arc
        .register_container(container_id.clone(), "resource-123".to_string())
        .await?;
    ...
    // Register container
    environment_arc
        .register_container(container_id.clone(), "lifecycle-123".to_string())
        .await?;
...
mod tests {
    ...
    async fn test_scope_guard() {
        ...
        let guard = ScopeGuard::new().add_cleanup_action(move || {
            counter_clone.fetch_add(1, Ordering::SeqCst);
        });
    ...
    async fn test_container_guard() {
        ...
        environment_arc
            .register_container(container_id.clone(), "test-123".to_string())
            .await
            .unwrap();
    ...
    async fn test_test_guard() {
        ...
        let guard = TestGuard::new(environment_arc, test_name.clone()).add_cleanup_action(|| {
            println!("Test cleanup executed");
        });
        ...
        let result = guard
            .execute_test(|| Ok::<i32, cleanroom::error::CleanroomError>(42))
            .await
            .unwrap();
        assert_eq!(result, 42);
