...
use cleanroom::{
    CleanroomConfig, CleanroomEnvironment, CleanroomError, CleanroomGuard, ContainerWrapper,
    CoverageTracker, DeterministicManager, GenericContainer, Policy, PostgresContainer,
    RedisContainer, ResourceLimits, SecurityLevel, ServiceContainer, SnapshotManager, TestReport,
    TracingManager, run, run_with_policy, new_cleanroom,
};
use std::sync::Arc;
...
async fn test_deterministic_execution() -> Result<(), Box<dyn std::error::Error>> {
    ...
    environment
        .set_deterministic_manager(deterministic_manager)
        .await?;
    ...
    let result1 = environment
        .execute_deterministic_test("test1", || Ok("deterministic_result"))
        .await?;
...
async fn test_snapshot_management() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let snapshot_id = environment
        .create_snapshot("test_snapshot", &snapshot_data)
        .await?;
    assert!(!snapshot_id.is_nil());

    // Verify snapshot
    let is_valid = environment
        .verify_snapshot("test_snapshot", &snapshot_data)
        .await?;
    assert!(is_valid);
