...
mod tests {
    ...
    fn test_mock_rpack_creation() {
        ...
        let manifest_content = std::fs::read_to_string(rpack_dir.join("templates").join("rgen.toml")).unwrap();
        assert!(manifest_content.contains("id = \"io.rgen.test\""));
        assert!(manifest_content.contains("version = \"0.1.0\""));
        
        // Test template content
        let template_content = std::fs::read_to_string(rpack_dir.join("templates").join("main.tmpl")).unwrap();
