...
class ObserverFalsifiers(unittest.TestCase):
    ...
    def test_unlisted_crate_is_observed_not_silently_ignored(self) -> None:
        root = self.make_repo()
    ...
    def test_duplicate_active_output_writers_are_refused_as_competing_actuators(self) -> None:
        root = self.make_repo()
        (root / "ggen.toml").write_text(
    ...
    def test_generated_marker_without_live_owner_is_a_blocking_finding(self) -> None:
        root = self.make_repo()
        (root / "orphan.rs").write_text("// GENERATED by absent machinery\n", encoding="utf-8")
        git(root, "add", ".")
