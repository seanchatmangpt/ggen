# Ontology Packs Documentation

Welcome to the Ontology Packs documentation. This guide follows the [Diataxis](https://diataxis.fr/) framework to help you learn, use, reference, and understand the ontology-as-packs system.

## Documentation Quadrants

### 🎓 [Tutorials](tutorials/) - Learning-Oriented
*Step-by-step lessons to build practical skills*

Perfect for newcomers who want hands-on experience:
- [Getting Started: Your First Ontology Pack](tutorials/01-getting-started.md)
- [Tutorial: Installing and Using SCHEMA.org](tutorials/02-using-schema-org.md)
- [Tutorial: Building Your Own Ontology Pack](tutorials/03-building-custom-pack.md)
- [Tutorial: Publishing to the Marketplace](tutorials/04-publishing-pack.md)

**Start here if:** You're new to ontology packs and want to learn by doing.

---

### 🛠️ [How-To Guides](how-to/) - Problem-Oriented
*Practical solutions to specific tasks*

For users who need to accomplish specific goals:
- [How to: Discover Available Ontologies](how-to/discover-ontologies.md)
- [How to: Generate TypeScript Types from FOAF](how-to/generate-typescript-types.md)
- [How to: Customize Code Generation](how-to/customize-generation.md)
- [How to: Compose Multiple Ontologies](how-to/compose-ontologies.md)
- [How to: Publish to Marketplace](how-to/publish-pack.md)
- [How to: Version and Update Packs](how-to/version-packs.md)
- [How to: Debug Pack Installation](how-to/debug-installation.md)

**Use this when:** You know what you want to do and need step-by-step instructions.

---

### 📚 [Reference](reference/) - Information-Oriented
*Complete technical specifications*

Detailed reference material for precision work:
- [Ontology Pack Metadata Format](reference/pack-metadata.md)
- [Template Variable Reference](reference/template-variables.md)
- [SPARQL Query Patterns](reference/sparql-patterns.md)
- [Supported Ontologies Registry](reference/ontologies-registry.md)
- [Code Generation Options](reference/generation-options.md)
- [CLI Command Reference](reference/cli-commands.md)
- [API Reference](reference/api.md)

**Consult this when:** You need precise details about syntax, options, or behavior.

---

### 💡 [Explanations](explanations/) - Understanding-Oriented
*Background knowledge and design decisions*

Deep dives into concepts and architecture:
- [Why Ontologies Matter](explanations/why-ontologies.md)
- [From Direct Generation to Template-Driven Design](explanations/case-study-evolution.md)
- [Understanding Pack Composition](explanations/pack-composition.md)
- [Architecture: Ontology Packs in the Marketplace](explanations/marketplace-architecture.md)
- [The Philosophy of Reusable Semantic Assets](explanations/philosophy.md)

**Read this when:** You want to understand the "why" behind the system.

---

## Quick Navigation

### By Role
- **First-time user?** → Start with [Getting Started](tutorials/01-getting-started.md)
- **Experienced developer?** → Jump to [How-To Guides](how-to/)
- **Integration work?** → Check [API Reference](reference/api.md)
- **Curious about design?** → Explore [Explanations](explanations/)

### By Task
- Installing a pack → [Tutorial: Using SCHEMA.org](tutorials/02-using-schema-org.md)
- Creating a pack → [Tutorial: Building Custom Pack](tutorials/03-building-custom-pack.md)
- Generating code → [How to: Generate TypeScript Types](how-to/generate-typescript-types.md)
- Publishing → [How to: Publish to Marketplace](how-to/publish-pack.md)
- Understanding metadata → [Pack Metadata Format](reference/pack-metadata.md)

---

## Document Evolution

This documentation is versioned alongside the ontology packs system. See [CONTRIBUTING.md](../../CONTRIBUTING.md) for guidelines on updating documentation.

**Current Version:** 1.0.0
**Last Updated:** 2025-11-18
