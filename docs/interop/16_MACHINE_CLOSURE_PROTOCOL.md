...
ggen handles contact.
Genesis handles consequence.
...
1. Genesis must remain pure.
2. ggen owns adapters, files, APIs, external formats, CLI, packaging, projections, and validation bridges.
3. AtomVM/WASM are runtime bodies for Genesis-bearing parts.
4. Truex is lifecycle and promotion, not the Genesis kernel.
5. Pair2 is not compressed RDF.
6. Pair2 is left byte + right byte under a predicate-fixed RelationPage.
7. Need9 means split.
8. Need257 means split.
9. No unsupported implementation claims.
10. No “done” claim without tests or explicit refusal.
...
3. PATCH
   ...
   - do not rename globally without a migration plan

4. VERIFY
   - compile
   - run relevant tests
   - run docs checks where available
   - run examples where available

5. RECEIPT
   - list changed files
   - list commands run
   - list pass/fail results
   - list remaining refusals
