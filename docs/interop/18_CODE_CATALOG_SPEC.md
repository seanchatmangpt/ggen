...
```
This yields a **File & Language index**. Tools like [`enry`](https://github.com/src-d/enry) or Linguist can auto-detect languages. We record for each file: path, size, language.
...
- **Static analysis tools**: For languages with no good LSP, use static analyzers (e.g. `doxygen` for C/C++, which can list classes and methods, or pyan for Python call graphs).  
...
- **Erlang/BEAM:** `erl_interface` or `rebar3 edoc` for functions; erl_ls LSP for symbols.  
- **WebAssembly:** Use WABT’s wasm-objdump to list functions/imports in a `.wasm`. The wasm-language-server provides static analysis and symbol info for WAT.  
...
- **Java:** Doxygen (call graphs via Graphviz), ProGuard (call graph), or SonarQube.  
...
Identify all **test files and runnable examples**. By convention: e.g. `tests/`, `*_test.py`, `*.spec.ts`, or CI scripts. Map each test to code under test (e.g. using import paths or known naming). Tools like `pytest --collect-only` can list tests. For languages with test frameworks (JUnit, Go’s `go test`, Rust’s `cargo test`), use those commands to enumerate tests and create a “test-to-file” map.
...
- **WASM**: use WABT’s `wasm-objdump` to disassemble and list imports/exports. `wasm2wat` makes text which can be parsed; the WASM language server provides static analysis (type checking, call hierarchy) for WASM text and can integrate with LSP tools.  
- **Container images**: tools like Syft can scan Docker/OCI images for files and produce SBOMs. We can mount (FUSE) the image layers and run `file`/`ctags` on embedded code.  
...
- **Symbol tables and exports:** As JSON, or as Language Server Protocol–style symbol lists. Could also populate an SQLite table (`symbols(name, kind, file, line)`).  
...
- **Receipts:** Tamper-evident logs of analysis (hashes of input files, tool versions, output hashes). Possibly store as COSE signatures or simple signed JSON.  
...
1. **Environment setup:** Choose a Unix-like OS (Linux/MacOS). Install tools: `universal-ctags`, `ripgrep`, `python3`, `sqlite3`, and language-specific tools (`rustup`, `go`, etc.).
2. **Clone repository:** `git clone <repo> project/`. Recursively include submodules.
3. **File & language index:** Run a script or use a library to detect languages.
4. **Symbol extraction:** Use Universal Ctags (`ctags --output-format=json -R ...`).
5. **Dependency extraction:** Ecosystem tools like `cargo metadata`, `go list`, `pipdeptree`.
6. **Call graph (approx.):** Tools like `pyan3` or `go-callvis`.
7. **Test discovery:** Map tests via frameworks like `pytest` or `cargo test`.
8. **License scan:** Run `scancode -cl --json licenses.json .`.
9. **WASM/binary inspection:** WABT tools like `wasm-objdump`.
10. **Index & database:** Load collected data into SQLite.
11. **Search index:** Use `ripgrep` for v0.1, Zoekt for v0.2.
12. **Run receipt:** Cryptographic logs of analysis commands.

## 11. Agent Prompts & Templates

To leverage the catalog, we define agent prompt patterns. Agents use the catalog data and must cite evidence (Non-Deletion Protocol rules). Classification uses the taxonomy: LIVE, PARTIAL, CAPABILITY_SEED, LEGACY_NAME, DORMANT, BROKEN_BUT_REAL, DOC_ONLY, TEST_ONLY, AMBIGUOUS.

- **Search code:** `[SYSTEM] Using the catalog, find files related to <feature> or containing symbols matching "<regex>". Cite the manifest entries.`
- **Extract functionality:** `[SYSTEM] List all functions exported by file path and classify their purpose. Provide evidence lines from the symbol table.`
- **Classify status:** `[SYSTEM] Using the Non-Deletion taxonomy, classify file f or symbol X. Cite the manifest or a commit log.`
- **Generate patch:** `[SYSTEM] Identify missing imports or unresolved references in file. Propose a patch with reasoning from the catalog symbol index.`
- **Verify build/test:** `[SYSTEM] The build command failed on module X. Find why by comparing code and tests from the catalog.`
- **Search patterns:** `[SYSTEM] Find all occurrences of the pattern "RelationPage" or "Pair2" in the codebase using the catalog.`

## 12. Validation & Metrics

To gauge effectiveness:
- **Symbol Recall:** Percentage of true symbols/definitions present in manifest vs. code. Aim >90%.  
- **False Positive Rate:** Instances where manifest claims a symbol exists but code changed/removed.  
- **Test Coverage (manifest):** Ratio of files/functions that have at least one associated test.  
- **Dependency Coverage:** Fraction of import statements mapped to entries in the dependency graph.  
- **Build Reproducibility:** Repeat build after cataloging; expect same binaries.  
- **Receipt Completeness:** Each analysis step logs a command and output hash.  
- **Performance:** Measure indexing time and catalog size.

## 13. Security & Privacy

- **Secrets scanning:** Run static scanners (TruffleHog, Gitleaks).  
- **Access controls:** Limit the catalogor’s permissions.  
- **Sandboxing:** Run any dynamic analysis in a container or VM.  
- **Provenance & Tamper Evidence:** Sign all outputs (JSON manifests, SQLite DB, receipt logs).  
- **Privacy:** Encrypt stored index or run offline for private code.  

## 14. ggen/Genesis/AtomVM/WASM Integration

To support our *parts-based architecture* (ggen/Genesis), the catalogor should specifically look for *RelationPage*, *Pair2*, and *Construct8* patterns in the code. Agents can then tag files or symbols with capabilities like `relation-indexer`, `pair2-builder`, or `construct8-serializer`. 

Integrate with AtomVM/WASM by indexing WASM modules, listing exported WASM functions that implement part interfaces.
