...
`PARTIAL_ALIVE` until repository CI compiles the Rust surface, regenerates both projections from RDF without byte drift, and validates the RDF instance against the SHACL law on the PR head.
...
The Crown example is a self-contained production cell:
...
├── crown-kernel.ttl
├── queries/passport.rq
├── templates/passport.json.tera
├── templates/nameplate.txt.tera
└── generated/
    ├── crown-kernel.json
    └── crown-kernel.nameplate.txt
```

The Turtle instance is the admitted source. SPARQL extracts the causal rating. ggen independently projects the machine JSON and the compact human nameplate. Neither generated file is hand-authored source law.
...
`.github/workflows/part-passport.yml` deletes both committed projections, executes this exact command, compares the regenerated bytes with the committed products, validates the JSON through the Rust passport, runs an independent SHACL verifier, and proves the double-insulation falsifier by changing `ambientIo` to `true`.
...
Manufactured machine passport:

```text
examples/part-passport/generated/crown-kernel.json
```

Manufactured human nameplate:

```text
examples/part-passport/generated/crown-kernel.nameplate.txt
```
...
9. rerunning ggen changes the committed projections without an admitted RDF change.
...
```bash
cargo run --manifest-path ../../Cargo.toml -p ggen-cli-lib --bin ggen -- sync run
cmp /tmp/crown-kernel.json generated/crown-kernel.json
cmp /tmp/crown-kernel.nameplate.txt generated/crown-kernel.nameplate.txt
cargo test -p ggen-marketplace part_passport
cargo test -p ggen-marketplace --test part_passport_fixture
python3 scripts/onto_shacl_check.py DATA SHAPES
```

Missing ggen, Rust, or SHACL tooling is `BLOCKED_TOOLCHAIN`, not success.
