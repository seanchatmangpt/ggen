...
|------|---------------|----------|----------|-------|-----------|---------|-----------------|
| `GGEN-TPL-001` | `unbound_projection` | `[ggen.toml, SPARQL, Tera]` | `error` | `source_law_repair` | `inspect_only` | `diagnostic_receipt` | **true** |
| `GGEN-HARNESS-001` | `harness_mismatch` | `[Cargo.toml, tests/proof, Makefile.toml]` | `release_blocking` | `proof_topology_repair` | `inspect_only` | `boundary_receipt` | **false** (Phase 2 metadata only) |

`GGEN-HARNESS-001` is metadata only — there is **no detector** for it; it locks the shape for Phase 2.

---

## 3. How GGEN-TPL-001 enters the existing observe→route→receipt loop

The existing living loop is in `crates/ggen-lsp/src/state.rs`:

- `ServerState::observe_diagnostics(&self, uri, new)` (`state.rs:127`) records the OCEL intel transitions `DiagnosticRaised → RouteSelected → RepairSuggested → RepairApplied → GatePassed → ReceiptEmitted` (BLAKE3-chained).
- The route registry is seeded at `state.rs:103` via `crate::route::RouteRegistry::seeded()`.

GGEN-TPL-001 attaches into this loop through the route layer (`crates/ggen-lsp/src/route/registry.rs`). Because `select_for_diagnostic` resolves **by `RepairFamily` only** (there is no per-code disambiguation in the real `route/model.rs`), GGEN-TPL-001 was given an exclusive family — `RepairFamily::DanglingReference` (unseeded otherwise, and semantically apt: an unbound projection is a dangling reference from template to a non-existent binding). `seed_routes()` registers the route `source-law.bind-projection` with three advisory `EditTemplate::NoOp` steps, one per source-law surface:

| step id | source-law surface |
|---------|--------------------|
| `edit-sparql-select` | SPARQL `SELECT` projection |
| `edit-template-ref` | Tera template variable reference |
| `inspect-ggen-toml-rule` | `ggen.toml` rule binding |

The load-bearing invariant — the route targets **only source law, never emitted output** — is enforced by the test `ggen_tpl_001_route_is_source_law_only` (every step is `NoOp`, no `out/`/`output/`/`dist/`/`gen/`/`emitted` marker, references a source-law surface), consistent with the `inspect_only` actuation boundary.

Once the live-trigger seam (§5) is wired, a raised GGEN-TPL-001 diagnostic flows: `detect_tpl_001` → `publishDiagnostics` → `observe_diagnostics` → `select_for_diagnostic` resolves `source-law.bind-projection` → advisory repair suggested → diagnostic receipt emitted. Today the detector and the route both exist and are tested; only the per-URI live invocation is deferred.
