...
- [ggen Rust Core Team — Master TODO List](#ggen-rust-core-team--master-todo-list)
  - [P0 — CRITICAL (Blocks other work or ships wrong behavior)](#p0--critical-blocks-other-work-or-ships-wrong-behavior)
    - [P0-01. SHACL validation is entirely stubbed — always passes](#p0-01-shacl-validation-is-entirely-stubbed--always-passes)
    - [P0-02. Production sync uses wrong pipeline](#p0-02-production-sync-uses-wrong-pipeline)
    - [P0-03. Three competing ontology namespaces cause silent data loss](#p0-03-three-competing-ontology-namespaces-cause-silent-data-loss)
    - [P0-04. CLI error type chaos — three Result types in prelude](#p0-04-cli-error-type-chaos--three-result-types-in-prelude)
    - [P0 Dependency Cascade](#p0-dependency-cascade)
  - [P1 — HIGH (Stubs that need real implementation)](#p1--high-stubs-that-need-real-implementation)
    - [P1-01. Construct command returns "not_implemented"](#p1-01-construct-command-returns-not_implemented)
    - [P1-02. Marketplace v3 registry — core methods return empty/error](#p1-02-marketplace-v3-registry--core-methods-return-emptyerror)
    - [P1-03. Marketplace RdfControlPlane — half the methods are stubs](#p1-03-marketplace-rdfcontrolplane--half-the-methods-are-stubs)
    - [P1-04. Pack install — 29 tests blocked on Phase 2](#p1-04-pack-install--29-tests-blocked-on-phase-2)
    - [P1-05. MCP tool execution returns placeholder JSON](#p1-05-mcp-tool-execution-returns-placeholder-json)
    - [P1-06. JSON Schema parser returns error](#p1-06-json-schema-parser-returns-error)
    - [P1-07. Rust code generation not implemented](#p1-07-rust-code-generation-not-implemented)
    - [P1-08. Graph evolution agent — AI client returns error](#p1-08-graph-evolution-agent--ai-client-returns-error)
    - [P1-09. PaaS deploy, logs follow, explain spec/pipeline — all not implemented](#p1-09-paas-deploy-logs-follow-explain-specpipeline--all-not-implemented)
    - [P1-10. YAWL watch mode returns error](#p1-10-yawl-watch-mode-returns-error)
    - [P1-11. MCP server background mode not implemented](#p1-11-mcp-server-background-mode-not-implemented)
    - [P1-12. Program of Thought — only JavaScript code generation works](#p1-12-program-of-thought--only-javascript-code-generation-works)
    - [P1-13. DSPy optimizers are pass-through stubs](#p1-13-dspy-optimizers-are-pass-through-stubs)
  - [P2 — MEDIUM (Mock implementations returning wrong answers)](#p2--medium-mock-implementations-returning-wrong-answers)
    - [P2-01. Sigma runtime invariants — 4 of 7 are empty branches](#p2-01-sigma-runtime-invariants--4-of-7-are-empty-branches)
    - [P2-02. Delta proposer returns mock proposals instead of calling LLM](#p2-02-delta-proposer-returns-mock-proposals-instead-of-calling-llm)
    - [P2-03. WeakProof::is_valid() always returns true](#p2-03-weakproofis_valid-always-returns-true)
    - [P2-04. Marketplace install conflict check always passes](#p2-04-marketplace-install-conflict-check-always-passes)
    - [P2-05. Marketplace README validator only checks description non-empty](#p2-05-marketplace-readme-validator-only-checks-description-non-empty)
    - [P2-06. Execution proof verification not implemented](#p2-06-execution-proof-verification-not-implemented)
    - [P2-07. Regeneration cache/dependency validation returns Ok/None](#p2-07-regeneration-cachedependency-validation-returns-oknone)
    - [P2-08. Template validation only checks for empty content](#p2-08-template-validation-only-checks-for-empty-content)
    - [P2-09. Pack generator logs "Would generate template" — no actual generation](#p2-09-pack-generator-logs-would-generate-template--no-actual-generation)
    - [P2-10. Pack composition Custom strategy returns error](#p2-10-pack-composition-custom-strategy-returns-error)
    - [P2-11. Pack registry unpublish just logs a warning](#p2-11-pack-registry-unpublish-just-logs-a-warning)
    - [P2-12. Graph export functions return hardcoded example data](#p2-12-graph-export-functions-return-hardcoded-example-data)
    - [P2-13. Bundle dependency resolution returns empty Vec](#p2-13-bundle-dependency-resolution-returns-empty-vec)
    - [P2-14. Windows lockfile not implemented](#p2-14-windows-lockfile-not-implemented)
  - [P3 — ARCHITECTURAL CLEANUP (Dead code, unused infrastructure, type debris)](#p3--architectural-cleanup-dead-code-unused-infrastructure-type-debris)
    - [P3-01. ggen-testing — complete framework, zero consumers](#p3-01-ggen-testing--complete-framework-zero-consumers)
    - [P3-02. ggen-macros — 3 of 5 macros dead, 2 never used](#p3-02-ggen-macros--3-of-5-macros-dead-2-never-used)
    - [P3-03. ~4,200 lines of dead AHI subsystem code in ggen-domain](#p3-03-4200-lines-of-dead-ahi-subsystem-code-in-ggen-domain)
    - [P3-04. Hive coordinator — entire module is dead code](#p3-04-hive-coordinator--entire-module-is-dead-code)
    - [P3-05. V6 pipeline types dead-code gated](#p3-05-v6-pipeline-types-dead-code-gated)
    - [P3-06. Lifecycle module has disabled submodules](#p3-06-lifecycle-module-has-disabled-submodules)
    - [P3-07. LoadConfigFromGgenToml trait — zero implementors](#p3-07-loadconfigfromggentoml-trait--zero-implementors)
    - [P3-08. ggen-domain marketplace module — all functions return errors](#p3-08-ggen-domain-marketplace-module--all-functions-return-errors)
    - [P3-09. ggen-core pack install — bails immediately](#p3-09-ggen-core-pack-install--bails-immediately)
    - [P3-10. 4 stale workspace exclude entries](#p3-10-4-stale-workspace-exclude-entries)
  - [P3 — TEST QUALITY (120 ignored tests, mock violations, 5K+ unwraps)](#p3--test-quality-120-ignored-tests-mock-violations-5k-unwraps)
    - [TQ-01. 120 `#[ignore]` tests — never run in CI](#tq-01-120-ignore-tests--never-run-in-ci)
    - [TQ-02. 8 files with hand-rolled mock structs (Chicago TDD violation)](#tq-02-8-files-with-hand-rolled-mock-structs-chicago-tdd-violation)
    - [TQ-03. 5,046 `.unwrap()` + 2,069 `.expect()` in test code](#tq-03-5046-unwrap--2069-expect-in-test-code)
  - [SUMMARY](#summary)
