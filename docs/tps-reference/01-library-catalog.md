...
- [Erlang/OTP Library Catalog for TPS Principles](#erlangotp-library-catalog-for-tps-principles)
  - [Executive Summary](#executive-summary)
  - [Dependency Graph: Which Libraries Work Together?](#dependency-graph-which-libraries-work-together)
  - [Library Catalog by TPS Principle](#library-catalog-by-tps-principle)
    - [JIT (Just-in-Time) Libraries](#jit-just-in-time-libraries)
      - [1. **RabbitMQ** (AMQP Message Broker)](#1-rabbitmq-amqp-message-broker)
      - [2. **NATS** (High-Performance Pub/Sub)](#2-nats-high-performance-pubsub)
      - [3. **poolboy** (Worker Pool Management)](#3-poolboy-worker-pool-management)
      - [4. **jobs** (Priority Job Queue)](#4-jobs-priority-job-queue)
    - [Jidoka (Autonomation) Libraries](#jidoka-autonomation-libraries)
      - [5. **fuse** (Circuit Breaker)](#5-fuse-circuit-breaker)
      - [6. **breaker** (Alternative Circuit Breaker)](#6-breaker-alternative-circuit-breaker)
      - [7. **supervisor** (Process Hierarchy)](#7-supervisor-process-hierarchy)
      - [8. **recon** (Runtime Diagnostics)](#8-recon-runtime-diagnostics)
    - [Kanban Libraries](#kanban-libraries)
      - [9. **RabbitMQ** (again, different use case)](#9-rabbitmq-again-different-use-case)
      - [10. **gnat** (NATS Erlang Client)](#10-gnat-nats-erlang-client)
      - [11. **queue** (OTP Module)](#11-queue-otp-module)
    - [Andon (Observability) Libraries](#andon-observability-libraries)
      - [12. **OpenTelemetry** (Distributed Tracing)](#12-opentelemetry-distributed-tracing)
      - [13. **Prometheus** (Metrics)](#13-prometheus-metrics)
      - [14. **logger** (OTP 21+ Structured Logging)](#14-logger-otp-21-structured-logging)
      - [15. **Lager** (Logging Before OTP 21)](#15-lager-logging-before-otp-21)
      - [16. **Grafana** (Dashboarding)](#16-grafana-dashboarding)
    - [Kaizen (Continuous Improvement) Libraries](#kaizen-continuous-improvement-libraries)
      - [17. **Prometheus** (again, different use case)](#17-prometheus-again-different-use-case)
      - [18. **OpenTelemetry** (again, for traces)](#18-opentelemetry-again-for-traces)
      - [19. **entop** (Erlang Process Top)](#19-entop-erlang-process-top)
      - [20. **folsom** (Lightweight Metrics)](#20-folsom-lightweight-metrics)
    - [Heijunka (Load Leveling) Libraries](#heijunka-load-leveling-libraries)
      - [21. **jobs** (again, for rate limiting)](#21-jobs-again-for-rate-limiting)
      - [22. **throttle** (Rate Limiting)](#22-throttle-rate-limiting)
      - [23. **poolboy** (again, for distribution)](#23-poolboy-again-for-distribution)
      - [24. **willow** (Load Balancer)](#24-willow-load-balancer)
  - [Recommended Library Stacks by Use Case](#recommended-library-stacks-by-use-case)
    - [Stack A: High-Throughput Messaging (Cost Governance)](#stack-a-high-throughput-messaging-cost-governance)
    - [Stack B: Real-Time Event Processing (Permission Drift)](#stack-b-real-time-event-processing-permission-drift)
    - [Stack C: Budget Spike Detection & Throttling](#stack-c-budget-spike-detection--throttling)
    - [Stack D: Compliance & Audit (ATO Guard)](#stack-d-compliance--audit-ato-guard)
  - [Library Quality Matrix](#library-quality-matrix)
    - [Scoring Rubric](#scoring-rubric)
    - [Library Scores](#library-scores)
  - [Security & Compliance Checklist](#security--compliance-checklist)
    - [Required Before Production Deployment](#required-before-production-deployment)
    - [Prohibited Libraries](#prohibited-libraries)
    - [Government-Approved Libraries (Pre-Vetted)](#government-approved-libraries-pre-vetted)
  - [Dependency Hell Prevention](#dependency-hell-prevention)
    - [Library Conflicts to Avoid](#library-conflicts-to-avoid)
  - [Version Pinning Strategy](#version-pinning-strategy)
    - [Critical Dependencies (pin exact version)](#critical-dependencies-pin-exact-version)
    - [Stable Dependencies (pin minor version)](#stable-dependencies-pin-minor-version)
    - [Safe Dependencies (allow patch updates)](#safe-dependencies-allow-patch-updates)
  - [Library Alternatives (Vendor Lock-In Prevention)](#library-alternatives-vendor-lock-in-prevention)
    - [If RabbitMQ Unavailable](#if-rabbitmq-unavailable)
    - [If Prometheus Unavailable](#if-prometheus-unavailable)
    - [If Grafana Unavailable](#if-grafana-unavailable)
  - [Cost Analysis: Commercial vs Open-Source](#cost-analysis-commercial-vs-open-source)
  - [Migration Path: Choosing Libraries Incrementally](#migration-path-choosing-libraries-incrementally)
    - [Phase 1: Core (Start Here)](#phase-1-core-start-here)
    - [Phase 2: Messaging](#phase-2-messaging)
    - [Phase 3: Reliability](#phase-3-reliability)
    - [Phase 4: Observability](#phase-4-observability)
    - [Phase 5: Advanced (Optional)](#phase-5-advanced-optional)
