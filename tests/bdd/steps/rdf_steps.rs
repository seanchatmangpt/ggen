...
fn should_see_query_results(world: &mut RgenWorld) {
    ...
    assert!(stdout.contains("ex:Person") || stdout.contains("ex:name"), 
        "Expected query results, but got: {}", stdout);
}
...
fn results_should_be_in_format(world: &mut RgenWorld, format: String) {
    ...
    match format.as_str() {
        "JSON" => assert!(stdout.contains("{") && stdout.contains("}"), 
            "Expected JSON format, but got: {}", stdout),
        "CSV" => assert!(stdout.contains(","), 
            "Expected CSV format, but got: {}", stdout),
        "TSV" => assert!(stdout.contains("\t"), 
            "Expected TSV format, but got: {}", stdout),
        _ => {} // Accept any format
    }
}