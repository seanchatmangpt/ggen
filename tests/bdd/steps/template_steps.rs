...
fn have_template_with_seed(world: &mut GgenWorld, seed: String) {
...
fn have_rdf_graph_data(world: &mut GgenWorld) {
...
#[when(regex = r"^I run ggen gen (.+) multiple times$")]
fn run_ggen_gen_multiple_times(world: &mut GgenWorld, template_path: String) {
...
fn generated_file_should_contain(
    world: &mut GgenWorld, file_path: String, expected_content: String,
) {
...
fn rdf_graph_should_be_processed(world: &mut GgenWorld) {
...
fn output_should_use_rdf_extracted_variables(world: &mut GgenWorld) {
...
fn sparql_variables_should_be_extracted(world: &mut GgenWorld) {
    // For BDD tests, we assume variables are extracted if the command succeeded
...
fn output_should_use_queried_values(world: &mut GgenWorld) {
...
fn all_outputs_should_be_byte_identical(world: &mut GgenWorld) {
...
fn file_should_be_generated_at(world: &mut GgenWorld, file_path: String) {
    let full_path = world.project_dir.join(&file_path);
    assert!(
        full_path.exists(),
