...
fn should_see_completion_script(world: &mut RgenWorld, shell: String) {
    ...
    match shell.as_str() {
        "bash" => assert!(
            stdout.contains("complete") || stdout.contains("_rgen"),
            "Expected bash completion script, but got: {}",
            stdout
        ),
        "zsh" => assert!(
            stdout.contains("compdef") || stdout.contains("_rgen"),
            "Expected zsh completion script, but got: {}",
            stdout
        ),
        "fish" => assert!(
            stdout.contains("complete") || stdout.contains("rgen"),
