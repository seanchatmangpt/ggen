...
fn test_simple_tracer_methods() {
    ...
    SimpleTracer::trace(
        core::simple_tracing::TraceLevel::Debug,
...
fn test_tracing_error_handling() {
    ...
    SimpleTracer::trace(
        core::simple_tracing::TraceLevel::Error,
        "Test error message",
        Some("test operation"),
    );
    ...
    SimpleTracer::trace(
        core::simple_tracing::TraceLevel::Warn,
...
fn test_tracing_memory_safety() {
    ...
    // Create many tracing events (optimized: reduced from 100 to 10)
    for i in 0..10 {
        SimpleTracer::performance(&format!("operation_{}", i), i as u64);
        SimpleTracer::sparql_query(
            &format!("SELECT * WHERE {{ ?s ?p ?o }} LIMIT {}", i),
            Some(i),
        );
    }
...
fn test_tracing_levels() -> Result<()> {
    ...
    for level in &test_levels {
        ...
        SimpleTracer::backup_created(
            std::path::Path::new("test.txt"),
            std::path::Path::new("backup.txt"),
        );
...
fn test_tracing_concurrent_access() -> Result<()> {
    ...
    // Test that tracing works with concurrent access (optimized: reduced from 10x10 to 2x5)
    let handles: Vec<_> = (0..2)
        .map(|i| {
            thread::spawn(move || {
                for j in 0..5 {
                    SimpleTracer::performance(
                        &format!("thread_{}_op_{}", i, j),
                        (i * 5 + j) as u64,
                    );
                }
            })
