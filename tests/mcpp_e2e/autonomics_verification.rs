...
fn test_second_order_autonomics_loop() {
    ...
    let receipt_history = vec![
        Receipt { id: "op1".into(), status: "failed".into() },
        Receipt { id: "op2".into(), status: "failed".into() },
        Receipt { id: "op3".into(), status: "success".into() },
    ...
    assert!(optimized_config.is_some(), "Meta-controller should have triggered adaptation");
    let new_config = optimized_config.unwrap();
    
    // Expected increase: (0.5 * 0.1) * 0.5 = 0.025 -> 0.525
    assert!(new_config.jidoka_sensitivity > 0.5, "Sensitivity should increase");
    assert!(new_config.jidoka_sensitivity < 0.55, "Sensitivity increase should be dampened");
    
    assert!(new_config.heijunka_leveling_factor < initial_config.heijunka_leveling_factor,
...
fn test_meta_loop_stability() {
    ...
    let stable_history = vec![
        Receipt { id: "op1".into(), status: "success".into() },
        Receipt { id: "op2".into(), status: "success".into() },
    ];

    let result = controller.evaluate_and_adapt(&stable_history);
    assert!(result.is_none(), "Stable system should not trigger policy adaptation");
