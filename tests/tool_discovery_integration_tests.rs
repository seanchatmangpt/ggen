...
impl MockA2AServer {
    ...
    pub fn get_tool(&self, name: &str) -> Result<ToolDefinition, DiscoveryError> {
        ...
        let tools = self.tools.lock().unwrap();
        tools.get(name).cloned().ok_or_else(|| DiscoveryError {
...
impl ToolDiscoveryManager {
    ...
    pub async fn discover_tools(&self) -> Result<DiscoveryResult, String> {
        ...
        Ok(DiscoveryResult {
            ...
            core_tools: tools
                .iter()
                .filter(|t| t.category == ToolCategory::Core)
                .count(),
            agent_tools: tools
                .iter()
                .filter(|t| t.category == ToolCategory::Agent)
                .count(),
            ...
            custom_tools: tools
                .iter()
                .filter(|t| matches!(&t.category, ToolCategory::Custom(_)))
                .count(),
        })
    }

    /// Discover tools with caching
    pub async fn discover_tools_with_cache(
        &self, cache_key: &str,
...
async fn td_001_basic_tool_discovery_empty_server_returns_zero_tools() {
    ...
    assert!(
        result.is_ok(),
        "Discovery should succeed even with no tools"
    );
    let discovery = result.unwrap();
...
async fn td_001_basic_tool_discovery_multiple_servers_aggregates_tools() {
    ...
    let discovery = result.unwrap();
    assert_eq!(
        discovery.total_count, 3,
        "Should discover 3 tools from all servers"
    );
    assert_eq!(discovery.core_tools, 2, "Should have 2 core tools");
    assert_eq!(discovery.agent_tools, 1, "Should have 1 agent tool");
}
...
async fn td_002_dynamic_tool_registration_new_tool_appears_in_discovery() {
    ...
    assert_eq!(
        ...
        "Should discover 2 tools after registration"
    );
    assert!(
        discovery.tools.iter().any(|t| t.name == "agent-status"),
...
async fn td_002_dynamic_tool_registration_batch_registration_all_discoverable() {
    ...
    assert_eq!(
        ...
        "Should discover all 5 registered tools"
    );
}
...
async fn td_002_dynamic_tool_registration_duplicate_tool_name_overwrites() {
    ...
    assert_eq!(
        ...
        "Should have 1 tool (not duplicate)"
    );
    let discovered = &discovery.tools[0];
    assert_eq!(
        discovered.description, "Updated description",
...
async fn td_002_dynamic_tool_registration_cannot_register_when_server_stopped() {
    ...
    assert!(
        result.is_err(),
        "Registration should fail when server is stopped"
    );
    assert!(
...
async fn td_003_tool_schema_validation_empty_name_fails() {
    ...
    assert!(
        validation.errors.iter().any(|e| e.contains("name")),
...
async fn td_003_tool_schema_validation_empty_description_fails() {
    ...
    assert!(!validation.is_valid, "Tool schema should be invalid");
    assert!(
        validation.errors.iter().any(|e| e.contains("description")),
...
async fn td_003_tool_schema_validation_invalid_schema_type_fails() {
    ...
    assert!(!validation.is_valid, "Tool schema should be invalid");
    assert!(
        validation.errors.iter().any(|e| e.contains("object")),
...
async fn td_003_tool_schema_validation_deprecated_tool_generates_warning() {
    ...
    assert!(
        validation.is_valid,
        "Deprecated tool schema should still be valid"
    );
    assert!(
    ...
    assert!(
        validation.warnings.iter().any(|w| w.contains("deprecated")),
...
async fn td_004_discovery_error_handling_server_not_running_returns_error() {
    ...
    assert!(
        result.is_err(),
        "Discovery should fail when server is not running"
    );
    assert!(
        result.unwrap_err().contains("not running") || result.unwrap_err().contains("Server"),
...
async fn td_004_discovery_error_handling_partial_server_failure_returns_error() {
    ...
    assert!(
        result.is_err(),
        "Discovery should fail when any server is unavailable"
    );
}
...
async fn td_004_discovery_error_handling_get_nonexistent_tool_returns_not_found() {
    ...
    assert!(
        error.message.contains("nonexistent-tool"),
...
async fn td_004_discovery_error_handling_error_preserved_for_inspection() {
    ...
    assert!(
        errors[0].message.contains("timed out"),
...
async fn td_004_discovery_error_handling_clear_errors_removes_all() {
    ...
    assert_eq!(
        server.errors().len(),
        2,
        "Should have 2 errors before clearing"
    );
...
async fn td_005_cached_discovery_first_call_fetches_from_server() {
    ...
    let client = MockMCPClient::new(vec![server.clone()], Duration::from_secs(60));
    ...
    assert_eq!(
        ...
        1,
        "Should have called server discovery once"
    );
}
...
async fn td_005_cached_discovery_expired_cache_refetches() {
    ...
    let client = MockMCPClient::new(vec![server.clone()], Duration::from_millis(100));
    let manager = ToolDiscoveryManager::new(client);
    ...
    assert_eq!(
        second_count, 2,
        "Second call should refetch from server after cache expires"
    );
}
...
async fn td_005_cached_discovery_manual_invalidate_clears_cache() {
    ...
    assert_eq!(
        second_count, 2,
        "After invalidation, should refetch from server"
    );
}
...
async fn integration_full_tool_discovery_workflow() {
    ...
    for tool in &result.tools {
        ...
        assert!(
            validation.unwrap().is_valid,
            "All discovered tools should have valid schemas"
        );
    }
}
...
async fn integration_concurrent_discoveries_handle_correctly() {
    ...
    let manager1 = Arc::new(manager);
    let manager1 = ToolDiscoveryManager::new(MockMCPClient::new(
        vec![server.clone()],
        Duration::from_secs(60),
    ));

    let task1 = tokio::spawn(async move {
        let m = ToolDiscoveryManager::new(MockMCPClient::new(vec![], Duration::from_secs(60)));
        m.discover_tools().await
    });
    ...
    // ASSERT: Both discoveries should complete
    let r1 = result1.unwrap();
    let r2 = result2.unwrap();
    // Empty server list is OK for this test - we're testing concurrency
}
