...
// `ggen_core::{utils::error::{Error, Result}, Graph}` directly. No ggen-engine/ggen-graph
// equivalent exists for this exact API (verified via workspace-wide search). This file was
// missed by the original 2026-07-16 `ggen-core-retired` gating pass (crates/ggen-cli/src/
// cmds/mod.rs's sibling files list did not include it), so it silently broke compilation
// the first time `--features integration` was actually exercised end-to-end (2026-08-03,
// TECH-DEBT-003 full test-integration run) -- `ggen_core` does not exist on disk at all.
// Gated as a whole file rather than deleted, per this project's fix-forward doctrine,
// matching the precedent already established for tests/fixture_validation_proof.rs,
