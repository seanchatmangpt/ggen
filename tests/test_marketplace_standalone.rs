...
/// Standalone test for marketplace search functionality
/// This tests the core search logic without depending on the full codebase
...
impl MockSearchEngine {
    ...
    fn search(&self, params: &SearchParams<'_>) -> Result<Vec<SearchResult>> {
...
fn test_popular_categories() -> Result<()> {
    ...
    let categories = engine.get_popular_categories()?;
    ...
    for i in 1..categories.len() {
        assert!(categories[i-1].1 >= categories[i].1);
    }
    
    // Check for specific categories
    let category_names: Vec<&String> = categories.iter().map(|(name, _)| name).collect();
    assert!(category_names.contains(&&"web".to_string()));
    assert!(category_names.contains(&&"rust".to_string()));
    assert!(category_names.contains(&&"python".to_string()));
...
fn test_popular_keywords() -> Result<()> {
    ...
    let keywords = engine.get_popular_keywords()?;
...
fn test_multiple_filters() -> Result<()> {
    ...
    // Should find the React component template (matches all filters)
    assert_eq!(results.len(), 1);
    assert_eq!(results[0].id, "io.rgen.web.react-component");
    assert_eq!(results[0].category, Some("web".to_string()));
    assert!(results[0].keywords.contains(&"testing".to_string()));
    assert_eq!(results[0].author, Some("frontend-team".to_string()));
    assert!(!results[0].latest_version.contains("beta"));
