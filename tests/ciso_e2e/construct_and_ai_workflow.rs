...
fn test_construct_create_with_nonexistent_spec() -> anyhow::Result<()> {
    ...
    assert_eq!(
        code, 0,
        "construct create should return JSON even for missing file"
    );
    let json = parse_json_output(&output);
    assert_eq!(
        json["status"].as_str().unwrap_or(""),
        "error",
        "Should report error status for missing spec file"
    );
    assert!(
...
fn test_construct_create_with_non_ttl_extension() -> anyhow::Result<()> {
    ...
    assert_eq!(
        json["status"].as_str().unwrap_or(""),
        "error",
        "Should report error for non-.ttl file"
    );
    assert!(
        json["message"].as_str().unwrap_or("").contains("Turtle"),
        "Error should mention Turtle format requirement"
    );
    Ok(())
}
...
"#,
    ...
    assert_eq!(
        json["status"].as_str().unwrap_or(""),
        "not_implemented",
        "Construct pipeline should report not_implemented status"
    );
    assert!(
        json.get("module_name").is_some(),
        "Output should contain inferred module name"
    );
    assert!(
        json.get("next_steps").is_some(),
...
fn test_construct_validate_with_module_name() -> anyhow::Result<()> {
    let workspace = create_temp_workspace()?;
    let (output, code) = run_ggen(
        &["construct", "validate", "--module_name", "test_module"],
        workspace.path(),
    )?;
    ...
    assert_eq!(
        json["status"].as_str().unwrap_or(""),
        "not_implemented",
        "Construct validate should report not_implemented"
    );
    assert_eq!(
        json["module_name"].as_str().unwrap_or(""),
        "test_module",
        "Output should echo module name"
    );
    Ok(())
}
...
fn test_ai_generate_without_api_key_fails_gracefully() -> anyhow::Result<()> {
    ...
    let (output, code) = run_ggen(
        &["ai", "generate", "--prompt", "Hello world"],
        workspace.path(),
    )?;
...
fn test_packs_list_shows_packs() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "packs list JSON should contain 'total' field"
    );
    assert!(
        json.get("packs").is_some(),
        "packs list JSON should contain 'packs' array"
    );
    Ok(())
}
...
fn test_packs_list_verbose() -> anyhow::Result<()> {
    ...
    assert!(
        json.get("total").is_some(),
        "packs list --verbose JSON should contain 'total' field"
    );
    Ok(())
}
...
fn test_packs_search_with_query() -> anyhow::Result<()> {
    ...
    assert!(
        json.get("total").is_some(),
        "packs search JSON should contain 'total' field"
    );
    assert!(
        json.get("results").is_some(),
        "packs search JSON should contain 'results' array"
    );
    Ok(())
}
...
fn test_policy_list_shows_profiles() -> anyhow::Result<()> {
    ...
    assert!(
        json.get("total").is_some(),
        "policy list JSON should contain 'total' field"
    );
    assert!(
        json.get("profiles").is_some(),
        "policy list JSON should contain 'profiles' array"
    );
    let total = json["total"].as_u64().unwrap_or(0);
    assert!(
        total > 0,
        "Should have at least one predefined policy profile"
    );
    Ok(())
}
