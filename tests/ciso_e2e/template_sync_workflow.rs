...
fn test_template_list_returns_json_output() {
    ...
    if code == 0 && !output.trim().is_empty() {
        ...
        assert!(
            ...
            "template list JSON should contain 'total' field"
        );
    }
}
...
fn test_template_new_creates_template() {
    ...
    let json = parse_json_output(&output);
    assert_eq!(
        json["template_name"].as_str().unwrap_or(""),
        "e2e-test-template",
        "Output should contain template name"
    );
    assert_eq!(
        json["template_type"].as_str().unwrap_or(""),
        "generic",
        "Output should contain template type"
    );
}
...
fn test_template_list_with_directory_flag() {
    ...
    if !output.trim().is_empty() {
        ...
        assert!(
            json.get("total").is_some(),
            "JSON output should have 'total' field"
        );
    }
}
...
fn test_init_creates_valid_project_structure() {
    ...
    assert_eq!(
        ...
        "success",
        "Init should report success status"
    );
    assert!(
    ...
    assert!(
        ...
        "schema/domain.ttl should exist after init"
    );
    assert!(
        workspace.path().join("Makefile").exists(),
        "Makefile should exist after init"
    );
    assert!(
        workspace.path().join("scripts/startup.sh").exists(),
        "scripts/startup.sh should exist after init"
    );
    assert!(
        workspace.path().join("templates/example.txt.tera").exists(),
        "templates/example.txt.tera should exist after init"
    );
    assert!(
        workspace.path().join("schema").is_dir(),
        "schema/ directory should exist"
    );
    assert!(
        workspace.path().join("templates").is_dir(),
        "templates/ directory should exist"
    );
    assert!(
        workspace.path().join("scripts").is_dir(),
...
fn test_init_twice_without_force_fails() {
    ...
    let (_output1, code1) = run_ggen(
        &["init", "--path", project_dir, "--skip_hooks", "true"],
        workspace.path(),
    )
    .unwrap();
    assert_eq!(code1, 0, "First init should succeed");
    let (output2, code2) = run_ggen(
        &["init", "--path", project_dir, "--skip_hooks", "true"],
        workspace.path(),
    )
    .unwrap();
    ...
    assert_eq!(
        json["status"].as_str().unwrap_or(""),
        "error",
        "Second init should report error status"
    );
    assert!(
        json["error"]
            .as_str()
...
fn test_init_force_reinitializes() {
    ...
    assert_eq!(
        json["status"].as_str().unwrap_or(""),
        "success",
        "Force re-init should report success"
    );
    let content = std::fs::read_to_string(&toml_path).unwrap();
    assert!(
        content.contains("[project]"),
        "ggen.toml should be restored to original content after force init"
    );
}
...
fn test_init_creates_transaction_receipt() {
    ...
    assert!(
        ...
        "Output should contain transaction info"
    );
    let tx = &json["transaction"];
    assert!(
        tx["committed"].as_bool().unwrap_or(false),
        "Transaction should be committed"
    );
    assert!(
        tx["total_files"].as_u64().unwrap_or(0) > 0,
        "Transaction should have created files"
    );
}
...
fn test_sync_dry_run_with_init_project() {
    let workspace = create_temp_workspace().unwrap();
    let project_dir = workspace.path().to_str().unwrap();
    let (init_output, init_code) = run_ggen(
        &["init", "--path", project_dir, "--skip_hooks", "true"],
        workspace.path(),
    )
    .unwrap();
...
"#,
    ...
    if code == 0 && !output.trim().is_empty() {
        let json = parse_json_output(&output);
        assert!(
            json.get("triples_loaded").is_some(),
            "graph load JSON should contain 'triples_loaded'"
        );
    }
}
...
fn test_version_returns_consistent_version() {
    ...
    assert_eq!(
        out1, out2,
        "Version output should be consistent across calls"
    );
