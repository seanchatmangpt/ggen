...
fn test_receipt_verify_help_shows_usage() -> anyhow::Result<()> {
    Command::cargo_bin("ggen")?.args(&["receipt", "verify", "--help"]).assert().success().stdout(predicate::str::contains("receipt")).stdout(predicate::str::contains("verify"));
    Ok(())
}
...
fn test_receipt_verify_invalid_format() -> anyhow::Result<()> {
    ...
    let (output, code) = run_ggen(&["receipt", "verify", bad_receipt.to_str().unwrap()], workspace.path()).unwrap();
...
fn test_receipt_verify_valid_json_wrong_schema() -> anyhow::Result<()> {
    ...
    let (output, code) = run_ggen(&["receipt", "verify", wrong_schema.to_str().unwrap()], workspace.path()).unwrap();
...
fn test_receipt_chain_verify_help_shows_usage() -> anyhow::Result<()> {
    Command::cargo_bin("ggen")?.args(&["receipt", "chain_verify", "--help"]).assert().success().stdout(predicate::str::contains("chain")).stdout(predicate::str::contains("verify"));
    Ok(())
}
...
fn test_receipt_chain_verify_nonexistent_file() -> anyhow::Result<()> {
    ...
    let (output, code) = run_ggen(&["receipt", "chain_verify", nonexistent, "--public-key", "/tmp/nonexistent-key.txt"], workspace.path())?;
    // CLI fails fast on missing files - that's acceptable CISO behavior
...
fn test_receipt_info_help_shows_usage() -> anyhow::Result<()> {
    Command::cargo_bin("ggen")?.args(&["receipt", "info", "--help"]).assert().success().stdout(predicate::str::contains("info"));
    Ok(())
}
...
fn test_capability_enable_generates_receipt() -> anyhow::Result<()> {
    ...
    if code == 0 && !output.trim().is_empty() {
        ...
        assert_eq!(json["status"].as_str().unwrap_or(""), "enabled", "Capability should be enabled");
        assert!(json.get("atomic_packs").is_some(), "Output should contain atomic_packs");
...
fn test_packs_install_generates_lockfile() -> anyhow::Result<()> {
    ...
    let (output, code) = run_ggen(&["packs", "install", "e2e-test-pack", "--force"], workspace.path())?;
    println!("packs install exit code: {}", code);
    println!("packs install output: {}", output.trim());
    if code == 0 {
        let json = parse_json_output(&output);
        assert_eq!(json["status"].as_str().unwrap_or(""), "installed", "Pack should be installed");
        ...
        if lockfile_path.exists() {
            ...
            assert!(lockfile_content.contains("e2e-test-pack"), "Lockfile should reference installed pack");
        }
    }
    Ok(())
}
...
fn test_receipt_help_shows_all_subcommands() -> anyhow::Result<()> {
    Command::cargo_bin("ggen")?.args(&["receipt", "--help"]).assert().success().stdout(predicate::str::contains("verify")).stdout(predicate::str::contains("info")).stdout(predicate::str::contains("chain_verify"));
    Ok(())
}

#[test]
fn test_receipt_subcommands_have_required_args() -> anyhow::Result<()> {
    Command::cargo_bin("ggen")?.args(&["receipt", "verify"]).assert().failure();
    Command::cargo_bin("ggen")?.args(&["receipt", "info"]).assert().failure();
    Command::cargo_bin("ggen")?.args(&["receipt", "chain_verify"]).assert().failure();
    Ok(())
}
