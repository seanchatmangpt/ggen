...
pub trait CryptoVerifierExt: super::CryptoVerifier {
    ...
    async fn verify_batch(&self, verifications: &[VerificationRequest]) -> Result<Vec<VerificationResult>>;

    /// Create a detached signature
    fn sign_detached(&self, content: &[u8]) -> Result<Vec<u8>>;

    /// Verify a detached signature
    fn verify_detached(&self, content: &[u8], signature: &[u8], public_key: &PublicKey) -> Result<bool>;

    /// Sign with timestamp
    fn sign_with_timestamp(&self, content: &[u8]) -> Result<TimestampedSignature>;

    /// Verify signature with timestamp validation
    fn verify_with_timestamp(&self, content: &[u8], signature: &TimestampedSignature) -> Result<bool>;
}
