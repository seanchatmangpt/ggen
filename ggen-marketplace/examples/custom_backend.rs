...
impl PackageStore for InMemoryPackageStore {
    ...
    async fn list(
        ...
        let mut hashes: Vec<_> = storage.keys()
            .filter(|hash| {
...
async fn main() -> Result<()> {
    ...
    println!("✓ Published package: {} v{}", package_id.name, package_id.version);
    ...
    // List versions
    let versions = marketplace.list_versions("custom-backend-demo", None).await?;
