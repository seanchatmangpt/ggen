...
use ggen_marketplace::models::{
    Category, ContentId, HashAlgorithm, Package, PackageId, PackageMetadata,
    PackageStats, Query, Version
};
...
async fn test_graphql_search_query() {
    ...
    assert!(result.errors.is_empty(), "GraphQL query should not have errors");
...
async fn test_graphql_package_query() {
    ...
    assert_eq!(pkg.get("name").and_then(|v| v.as_str()), Some("awesome-crate"));
...
async fn test_graphql_error_handling() {
    ...
    assert!(!result.errors.is_empty(), "Should have errors for non-existent package");
...
async fn test_graphql_delete_mutation() {
    ...
    assert_eq!(deleted, Some(true), "Should return true on successful delete");
}
