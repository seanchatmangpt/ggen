...
use ggen_marketplace::models::signature::{SignatureAlgorithm, PublicKey, Signature};
...
fn test_verify_invalid_signature_returns_false() {
    ...
    assert!(
        !result.unwrap(),
        "Invalid signature should verify as false"
    );
}
...
fn test_signature_verification_with_wrong_public_key_fails() {
    ...
    let verifier1 = Ed25519Verifier::with_keypair(keypair1);
    let mut signature = verifier1
        .sign(content)
        .expect("Should sign with key 1");
