...
async fn test_ed25519_sign_and_verify() {
    ...
    assert_eq!(signature.algorithm, ggen_marketplace::models::SignatureAlgorithm::Ed25519);
...
async fn test_ed25519_verify_wrong_content_fails() {
    ...
    let is_valid = verifier_with_key.verify(wrong_content, &signature).expect("should verify");
    assert!(!is_valid, "signature should not be valid for tampered content");
}
...
async fn test_ed25519_keypair_generation() {
    ...
    let keypair1 = verifier.generate_keypair().expect("should generate keypair");
    let keypair2 = verifier.generate_keypair().expect("should generate keypair");
...
async fn test_ed25519_export_import_public_key() {
    ...
    let pem = verifier.export_public_key(&keypair.public_key).expect("should export");
...
async fn test_p2p_publish_and_discover() {
    ...
    // Create test package
    let package = Package::builder(
        PackageId::new("test", "p2p-package"),
        Version::new(1, 0, 0),
    )
    ...
    .license("MIT")
    .tag("p2p")
    .content_id(ContentId::new("test-hash".to_string(), HashAlgorithm::Sha256))
    .build()
    .expect("package should build");

    // Publish to P2P network
    p2p_registry.publish(package.clone()).await.expect("publish should succeed");

    // Discover via P2P
    let results = p2p_registry.discover("p2p").await.expect("discover should succeed");
...
async fn test_p2p_retrieve_package() {
    ...
    let retrieved = p2p_registry.retrieve(&package_id).await.expect("retrieve should succeed");
...
async fn test_p2p_peer_management() {
    ...
    // Add peers
    p2p_registry.add_peer("peer1.example.com:8080".to_string()).await.expect("add peer should succeed");
    p2p_registry.add_peer("peer2.example.com:8080".to_string()).await.expect("add peer should succeed");
    p2p_registry.add_peer("peer3.example.com:8080".to_string()).await.expect("add peer should succeed");
...
async fn test_p2p_discover_multiple_packages() {
    ...
    let results = p2p_registry.discover("web").await.expect("discover should succeed");
...
async fn test_graphql_search_query() {
    ...
    .license("MIT")
    .tag("graphql")
    .content_id(ContentId::new("hash-gql".to_string(), HashAlgorithm::Sha256))
    .build()
    .expect("package should build");
...
async fn test_graphql_package_query() {
    ...
    .license("Apache-2.0")
    .content_id(ContentId::new("hash-specific".to_string(), HashAlgorithm::Sha256))
    .build()
    .expect("package should build");
...
async fn test_graphql_query_not_found() {
    ...
    let response = graphql_server.execute_query(query).await.expect("query should execute");
...
async fn test_full_stack_signed_p2p_package_via_graphql() {
    ...
    let (package, _content_bytes) = create_signed_package(
        "full-stack-package",
        "1.0.0",
        content,
    )
...
async fn test_signed_package_p2p_distribution() {
    ...
    let packages = vec![
        create_signed_package("auth-lib", "1.0.0", b"auth library").await.expect("should create"),
        create_signed_package("crypto-lib", "2.0.0", b"crypto library").await.expect("should create"),
        create_signed_package("web-lib", "1.5.0", b"web library").await.expect("should create"),
    ];
    ...
    // Discover via P2P
    let results = p2p_registry.discover("lib").await.expect("discover should succeed");
...
async fn test_graphql_query_signed_packages() {
    ...
    for i in 0..3 {
        ...
        let (package, _) = create_signed_package(
            &format!("signed-pkg-{}", i),
            "1.0.0",
            content.as_bytes(),
        )
...
async fn test_p2p_network_resilience() {
    ...
    let results = p2p_registry.discover("resilient").await.expect("discover should succeed");
...
async fn test_p2p_concurrent_operations() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            registry.publish(package).await.expect("publish should succeed");
        });
    ...
    let results = p2p_registry.discover("concurrent").await.expect("discover should succeed");
...
async fn test_large_scale_p2p_discovery() {
    ...
    let results = p2p_registry.discover("category-0").await.expect("discover should succeed");
...
async fn test_integration_with_local_registry() {
    ...
    .license("MIT")
    .content_id(ContentId::new("hash-integration".to_string(), HashAlgorithm::Sha256))
    .build()
    .expect("package should build");

    local_registry.publish(package.clone()).await.expect("publish should succeed");

    // Search in local registry
    let query = Query::new("integration");
    let results = local_registry.search(&query).await.expect("search should succeed");
