...
impl AiMcpTools {
    ...
    pub fn with_mock(mut self) -> Self {
        ...
        self.sparql_generator = Some(SparqlGenerator::with_client(Box::new(MockClient::with_response(r#"{"query_type":"SELECT","variables":["?s","?p","?o"],"patterns":[{"subject":"?s","predicate":"?p","object":"?o"}]}"#))));
        ...
        self.refactor_assistant = Some(RefactorAssistant::with_client(Box::new(MockClient::with_response("Refactoring suggestion"))));
