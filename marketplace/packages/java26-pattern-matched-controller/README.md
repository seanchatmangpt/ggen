...
```bash
mcpp sync --rule rest-controller-test
...
```bash
mcpp install java26-pattern-matched-controller
```
...
```bash
mcpp sync --rule rest-controller
mcpp sync --rule rest-controller-test
```
...
Re-run `mcpp sync` to regenerate. All switch expressions that don't handle `RefundedOrder`
