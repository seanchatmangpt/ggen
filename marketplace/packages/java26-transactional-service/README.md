...
```bash
cargo make test   # via mcpp test runner
# or directly:
...
### 1. Add to your mcpp project

```bash
mcpp install java26-transactional-service
```
...
```bash
mcpp sync --rule transactional-service
mcpp sync --rule transactional-service-test
```
...
Re-run `mcpp sync` to regenerate the command records with the new field.

---

*Generated by mcpp marketplace — Java 26 Pattern Language*
